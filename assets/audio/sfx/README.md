...
The WAV files used by Arch Rogue come from **Action RPG SFX Pack v2**, acquired
through the Unity Asset Store. They are third-party licensed material and are
not covered by this repository's Apache-2.0 license.
...
```bash
python3 tools/import_action_rpg_sfx.py \
  --source "/path/to/action-rpg-sfx-pack"
python3 tools/sfx_bundle.py verify
```

The importer requires `ffmpeg`. It uses the explicit source mapping and duration
policy in `tools/import_action_rpg_sfx.py`, then writes only game-ready PCM WAVs
