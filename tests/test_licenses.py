...
class AboutScreenLicensesTests(unittest.TestCase):
    ...
    def test_about_document_layout_is_reused_across_scroll_frames(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            game = make_game(tmpdir)
            game.state = "about"
            menu = game.menus
            with patch.object(
                menu, "wrap_text", wraps=menu.wrap_text
            ) as wrap_text:
                menu.draw_about_screen()
                self.assertGreater(wrap_text.call_count, 100)
                cached_document = menu._about_document_cache
                wrap_text.reset_mock()
                game.licenses_scroll += 1
                menu.draw_about_screen()
            self.assertEqual(wrap_text.call_count, 0)
            self.assertIs(menu._about_document_cache, cached_document)

    def test_desktop_about_retention_invalidates_for_scroll(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            game = make_game(tmpdir)
            game.state = "about"
            with (
                patch.object(
                    game,
                    "draw_about_screen",
                    wraps=game.draw_about_screen,
                ) as draw_about,
                patch.object(
                    game, "_present_frame", wraps=game._present_frame
                ) as present,
            ):
                game.draw()
                game.draw()
                self.assertEqual(draw_about.call_count, 1)
                self.assertEqual(present.call_count, 2)

                game.licenses_scroll += 1
                game.draw()
                self.assertEqual(draw_about.call_count, 2)

    def test_menu_text_cache_obeys_entry_and_byte_caps(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            game = make_game(tmpdir)
            menu = game.menus
            rect = pygame.Rect(0, 0, game.screen.get_width(), 24)
            for index in range(1400):
                menu.draw_text(
                    f"{index}: " + "Wide cached menu text " * 12,
                    game.small_font,
                    menu.TEXT,
                    rect,
                )
            self.assertLessEqual(
                len(menu._menu_text_cache),
                menu._MENU_TEXT_CACHE_MAX_ENTRIES,
            )
            self.assertLessEqual(
                menu._menu_text_cache_bytes,
                menu._MENU_TEXT_CACHE_MAX_BYTES,
            )
            menu.clear_render_caches()
            self.assertEqual(menu._menu_text_cache_bytes, 0)
