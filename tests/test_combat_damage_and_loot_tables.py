...
class CombatSkillsLoot22Tests(unittest.TestCase):
    ...
    def test_loot_drops_are_rarer_than_the_four_one_baseline(self) -> None:
        # 4.2 trimmed the on-kill loot roll from the 0.45 baseline to 0.36;
        # the 4.9.23 across-the-board rarity pass trims it again to 0.28. We
        # assert the constant indirectly by driving controlled rolls just
        # under and above the threshold.
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # rolls just under 0.28 must drop, rolls above it must not.
            class _Rng:
            ...
            # 0.28 < 0.28 is False, so use 0.27 to confirm a drop happens just
            # under the threshold and 0.30 to confirm it does not.
            game.rng = _Rng([0.27])
