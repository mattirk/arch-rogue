...
class FamiliarTests(unittest.TestCase):
    ...
    def test_acolyte_spirit_familiar_cannot_perceive_or_attack_through_walls(self) -> None:
        ...
        # Ranger Spirit Beast wall test above, exercised on the Acolyte's crow.
        with tempfile.TemporaryDirectory() as tmpdir:
    ...
    def test_spirit_branch_scales_hp_damage_count_and_flags(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Crow Companion: more HP (lifesteal moved to the Blood path in
            ...
            # Twin Crows: +1 familiar (count = 2) and more damage.
            ...
            # Crow Lord: lead familiar is a champion (taunts); the sprite
            # stays the big crow (variant 1) once Spirit Call is chosen.
            ...
            # Eternal Crows: +1 familiar (count = 3) and unkillable host.
    ...
    def test_two_familiar_sprite_states_exist_and_crow_is_larger(self) -> None:
        # The sprite atlas exposes two familiar states (small wisp + big crow),
        # both renderable, and the crow is clearly larger than the wisp.
