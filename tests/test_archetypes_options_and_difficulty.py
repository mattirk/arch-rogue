    ...
    def test_options_and_run_save_are_versioned_and_persistent(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                self.assertEqual(loaded_options["schema_version"], 10)
    ...
    def test_difficulty_cycles_and_hell_unlocks_after_first_clear(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                # 4.9: the final descent opens the Tenth Bell epilogue; ring
                # it through to complete the clear.
                self.assertEqual(game.state, "playing")
                assert game.active_cutscene is not None
                self.assertEqual(
                    game.active_cutscene.asset_id, "story_epilogue"
                )
