...
class StoryModeTests(unittest.TestCase):
    ...
    def test_story_guest_choice_updates_effects_run_stats_and_save_version(
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                # 4.9 budget: the prompt detail must survive the two-line
                # mobile clamp (~48 chars) without silent truncation.
                self.assertLessEqual(len(hint[2]), 48)
    ...
    def test_quest_cutscene_assets_and_guest_dialogue_lifecycle(self) -> None:
        ...
        self.assertIn("story_epilogue", library)
        # 4.9.x: no separate depth-1 prologue asset — every floor opens
        # straight on the relic-choice omen (Rue's greeting is folded into
        # the depth-1 omen body instead).
        self.assertNotIn("story_guest_omen_first", library)
        self.assertEqual(
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                # --- Cutscene asset binding (depth-1 omen) ---
                self.assertTrue(game.story_intro_pending)
                ...
                # Depth 1 opens straight on the relic choice; Rue's greeting
                # is the omen body's first line.
                self.assertIn("N. Rue", game.active_cutscene_text())
                ...
                self.assertEqual(
                    loaded.active_cutscene.asset_id, "story_guest_omen"
                )
