...
class GraphicsAnimation21Tests(unittest.TestCase):
    ...
    def test_wall_fog_edge_darkens_directionally_and_preserves_occlusion(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            game = self.make_game(tmpdir)
            ...
            game.dungeon.tiles[x - 1][y] = Tile.FLOOR
            game.dungeon.tiles[x][y - 1] = Tile.FLOOR
            game.revealed_tiles = {(x, y), (x - 1, y), (x, y - 1)}
            self.assertEqual(game._wall_fog_edge(x, y), 0)

            # Remembered solid rock does not visually support an elevated wall
            # face: only floor-like terrain removes the corresponding fog edge.
            game.dungeon.tiles[x - 1][y] = Tile.WALL
            self.assertEqual(game._wall_fog_edge(x, y), 1)
            game.dungeon.tiles[x][y - 1] = Tile.CLOSED_DOOR
            self.assertEqual(game._wall_fog_edge(x, y), 3)

            base = pygame.Surface((360, 440), pygame.SRCALPHA)
            base.fill((0, 0, 0, 0))
            pygame.draw.rect(base, (120, 150, 180, 255), (10, 10, 340, 420))
            both = game._fogged_wall_surface(base, 3)
            both_again = game._fogged_wall_surface(base, 3)
            self.assertIs(both_again, both)
            self.assertEqual(
                pygame.image.tobytes(base, "RGBA")[3::4],
                pygame.image.tobytes(both, "RGBA")[3::4],
            )

            center_x = both.get_width() // 2
            top = both.get_at((center_x, 20))
            middle = both.get_at((center_x, both.get_height() // 2))
            bottom_y = both.get_height() - 11
            bottom = both.get_at((center_x, bottom_y))
            self.assertLess(top.r, middle.r)
            self.assertLess(middle.r, bottom.r)
            self.assertEqual(bottom.r, 120)
            for py in (20, both.get_height() // 2, bottom_y):
                self.assertEqual(both.get_at((center_x, py)).a, 255)

            northwest = game._fogged_wall_surface(base, 1)
            northeast = game._fogged_wall_surface(base, 2)
            probe_y = 40
            left_x, right_x = 12, base.get_width() - 13
            self.assertLess(
                northwest.get_at((left_x, probe_y)).r,
                northwest.get_at((right_x, probe_y)).r,
            )
            self.assertEqual(
                northwest.get_at((left_x, probe_y)).r,
                northeast.get_at((right_x, probe_y)).r,
            )
            self.assertEqual(
                northwest.get_at((right_x, probe_y)).r,
                northeast.get_at((left_x, probe_y)).r,
            )

            keyed = pygame.Surface((32, 32))
            keyed.fill((255, 0, 255))
            keyed.set_colorkey((255, 0, 255), pygame.RLEACCEL)
            pygame.draw.rect(keyed, (120, 150, 180), (8, 8, 16, 16))
            keyed_fog = game._fogged_wall_surface(keyed, 3)
            self.assertEqual(keyed_fog.get_at((0, 0)).a, 0)
            self.assertEqual(keyed_fog.get_at((16, 16)).a, 255)
            self.assertEqual(
                pygame.mask.from_surface(keyed, 0).count(),
                pygame.mask.from_surface(keyed_fog, 0).count(),
            )

            game._frame_dark = True
            self.assertEqual(game._wall_fog_edge(x, y), 0)
    ...
    def test_modern_aim_cone_is_stronger_and_legacy_render_is_unchanged(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Compare the tiers at a zoom where both resolve the same world
            # render bucket (1.0). The HD default zoom sits on the 0.8 bucket
            # since 4.9.24, so tier geometry legitimately differs there by the
            # bucket ratio; this test pins alpha strength, not bucket choice.
