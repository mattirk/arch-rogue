...
class ValidationTests(unittest.TestCase):
    ...
    def test_intent_builder_clamps_and_validates(self) -> None:
        ...
        self.assertIn("minigame", INTENT_ACTIONS)
        mini_game = make_intent(
            input_seq=2,
            action="minigame",
            target="7:9:2",
        )
        self.assertEqual(mini_game["target"], "7:9:2")
        self.assertEqual(validate_client_message(mini_game), "")
        ready = make_intent(
            input_seq=3,
            action="minigame",
            target="7:ready",
        )
        self.assertEqual(ready["target"], "7:ready")
        self.assertEqual(validate_client_message(ready), "")
