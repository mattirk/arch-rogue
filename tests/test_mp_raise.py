...
class RaisePartnerTests(unittest.TestCase):
    ...
    def test_raise_charges_and_memory_tokens_replicate_in_both_sync_payloads(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            game = _make_host_game_with_partner(tmpdir)
            ...
            partner.memory_tokens = 4
            fast = sync.player_fast_dict(game, partner)
            self.assertEqual(fast["raises"], 2)
            self.assertEqual(fast["tokens"], 4)
            partner.raise_charges = 1
            partner.memory_tokens = 1
            sync.apply_player_fast(game, partner, fast)
            self.assertEqual(partner.raise_charges, 2)
            self.assertEqual(partner.memory_tokens, 4)
            full = sync.player_full_dict(game, partner)
            self.assertEqual(full["raise_charges"], 2)
            self.assertEqual(full["memory_tokens"], 4)
            self.assertEqual(full["mastery_tokens"], 4)
            rebuilt = sync.build_player_from_full(game, full)
            self.assertEqual(rebuilt.raise_charges, 2)
            self.assertEqual(rebuilt.memory_tokens, 4)

            legacy_full = dict(full)
            legacy_full.pop("memory_tokens")
            rebuilt_legacy = sync.build_player_from_full(game, legacy_full)
            self.assertEqual(rebuilt_legacy.memory_tokens, 4)

            conflicting_full = dict(full, memory_tokens=6, mastery_tokens=3)
            rebuilt_current = sync.build_player_from_full(game, conflicting_full)
            self.assertEqual(rebuilt_current.memory_tokens, 6)
