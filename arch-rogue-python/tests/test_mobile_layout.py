...
class MobileRenderQualityTests(unittest.TestCase):
    ...
    def test_mobile_performance_monitor_reports_phase_and_runtime_context(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # A 1100ms frame also trips the immediate hitch line, which is
            # emitted before the interval report.
            self.assertEqual(len(emitted), 2)
            self.assertTrue(emitted[0].startswith("ARCH_ROGUE_HITCH "))
            self.assertEqual(emitted[1], report)
    ...
    def test_hitch_lines_attribute_slow_frames_immediately(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            game = make_mobile_game(tmpdir, (780, 360))
            now = [0.0]
            emitted: list[str] = []
            monitor = MobilePerformanceMonitor(
                report_interval=60.0,  # keep the interval report out of the way
                clock=lambda: now[0],
                emit=emitted.append,
            )
            monitor.begin_frame(game)
            monitor.record_phase("update", 0.010)
            monitor.record_detail_phase("save", 0.180)
            now[0] = 0.2
            self.assertIsNone(monitor.finish_frame(game))
            self.assertEqual(len(emitted), 1)
            line = emitted[0]
            self.assertTrue(line.startswith("ARCH_ROGUE_HITCH frame=200.0ms"))
            self.assertIn("update:10.0", line)
            self.assertIn("save:180.0", line)
            self.assertIn("other:190.0", line)

            # Within the rate-limit gap a second slow frame stays silent...
            monitor.begin_frame(game)
            now[0] = 0.3
            self.assertIsNone(monitor.finish_frame(game))
            self.assertEqual(len(emitted), 1)
            # ...and a fast frame never logs regardless of elapsed time.
            monitor.begin_frame(game)
            now[0] = 10.31
            monitor._last_hitch_emitted = float("-inf")
            monitor.begin_frame(game)
            now[0] = 10.32
            self.assertIsNone(monitor.finish_frame(game))
            self.assertEqual(len(emitted), 1)

    def test_gc_probe_records_collector_pauses_and_detaches(self) -> None:
        import gc as gc_module

        now = [0.0]
        monitor = MobilePerformanceMonitor(clock=lambda: now[0])
        monitor.install_gc_probe()
        try:
            self.assertIn(monitor._on_gc_event, gc_module.callbacks)
            monitor.begin_frame(None)
            monitor._on_gc_event("start", {})
            now[0] = 0.033
            monitor._on_gc_event("stop", {})
            self.assertAlmostEqual(
                monitor._current_detail_phase_seconds["gc"], 0.033
            )
        finally:
            monitor.uninstall_gc_probe()
        self.assertNotIn(monitor._on_gc_event, gc_module.callbacks)
...
class MobileHudTests(unittest.TestCase):
    ...
    def test_memory_token_prompt_is_hidden_without_unspent_tokens(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertFalse(
                any(
                    target.context == "gameplay"
                    and target.command == Command.OPEN_DISCIPLINES
                    for target in game._mobile_touch_targets
                )
            )

    def test_memory_token_prompt_stays_clear_and_opens_disciplines_on_tap(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            game = make_mobile_game(tmpdir, (1280, 720), (42, 8, 18, 12))
            ...
            global_prompt = prompt.move(layout.world_viewport.topleft)
            self.assertTrue(layout.safe_rect.contains(global_prompt))
            self.assertFalse(global_prompt.colliderect(layout.left_rail))
            self.assertFalse(global_prompt.colliderect(layout.right_rail))
            self.assertFalse(global_prompt.colliderect(layout.joystick_rect))
            self.assertEqual(game._memory_token_prompt_layout["tokens"], 3)
            self.assertEqual(
                game._memory_token_prompt_layout["detail_text"],
                "Open Disciplines",
            )
            self.assertEqual(
                game._memory_token_prompt_layout["title_text"],
                "+3 memory tokens",
            )
            ...
            target = targets[0]
            self.assertEqual(target.label, "Open Disciplines")
            self.assertTrue(target.rect.contains(global_prompt))
            ...
            self.assertEqual(game.mobile_input_context(), "character")
...
class MobileTouchTests(unittest.TestCase):
                ...
                (
                    max(
                        ...
                        round(
                            ...
                            / (
                                game.view_zoom
                                / game.world_render_scale_bucket()
                            )
                        ),
                    ),
                    max(
    ...
    def test_joystick_world_aim_and_skill_fingers_can_coexist(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch.object(game, "player_big_hit") as big_hit:
                self.assertTrue(game.handle_mobile_finger_event(skill_down))
            big_hit.assert_called_once_with()
    ...
    def test_world_and_skill_fingers_can_coexist(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertTrue(game._mobile_touch_world_active)
            big_hit_calls = 0
            original = game.player_big_hit

            def big_hit() -> None:
                nonlocal big_hit_calls
                big_hit_calls += 1
                original()

            game.player_big_hit = big_hit  # type: ignore[assignment]
            self.assertTrue(game.handle_mobile_finger_event(skill_down))
            self.assertEqual(big_hit_calls, 1)
    ...
    def test_safe_area_title_row_tap_uses_rendered_hitbox(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            game.state = "title"
            game.draw()
            local_row = game._title_row_rects[4]
            global_point = (
    ...
    def test_inventory_rows_and_horizontal_swipes_replace_helper_buttons(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            sort_inventory.assert_called_once_with(forward=True)
