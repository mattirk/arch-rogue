    ...
    def test_floor_bosses_drop_notable_rewards_and_record_mastery(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                # The run-stats entry keeps the story nameplate for display, but
                # the lifetime ledger holds the canonical definition name --
                # boss achievements key on it (4.9.22 nameplate fix).
                self.assertIn(
                    canonical_boss_name(floor_boss.name),
                    game.meta_progress["bosses_defeated"],
                )
                self.assertNotEqual(
                    canonical_boss_name(floor_boss.name), floor_boss.name,
                    "expected a story-decorated nameplate; the scenario no "
                    "longer exercises the decoration path",
                )
