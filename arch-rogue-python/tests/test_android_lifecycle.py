...
class AndroidLifecycleTests(unittest.TestCase):
    ...
    def test_low_memory_clears_caches_without_crash(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            game.audio.sfx_cache["hit"] = object()
            game.audio.music_cache[123] = object()
            game.handle_mobile_lifecycle_event(
                pygame.event.Event(getattr(pygame, "APP_LOWMEMORY", -25))
            )
            self.assertIsNone(game._world_layer)
            self.assertEqual(game.audio.sfx_cache, {})
            self.assertEqual(game.audio.music_cache, {})
...
class AndroidStorageTests(unittest.TestCase):
    ...
    def test_desktop_uses_the_pref_dir_since_5_0_1(self) -> None:
        # Steam Auto-Cloud cannot address the bare Windows home directory,
        # so desktop joined mobile on the SDL pref path; the home dir is
        # only the fallback when SDL cannot supply one (covered in
        # tests/test_storage_migration.py).
        with patch.object(
            pygame.system, "get_pref_path", return_value="/tmp/pref-dir/"
        ):
            self.assertEqual(
                application_storage_directory(mobile=False),
                Path("/tmp/pref-dir/"),
            )
