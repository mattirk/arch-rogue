...
class SpriteAssetTests(unittest.TestCase):
    ...
    def test_world_tint_is_byte_identical_after_nearest_scale(self) -> None:
        sources = []
        for opaque in (False, True):
            source = pygame.Surface((29, 23), pygame.SRCALPHA, 32)
            for y in range(source.get_height()):
                for x in range(source.get_width()):
                    alpha = (
                        255
                        if opaque
                        else (x * 53 + y * 97 + x * y * 11) % 256
                    )
                    source.set_at(
                        (x, y),
                        (
        ...
        for source_index, source in enumerate(sources):
            for tint in ((0, 0, 0), (255, 255, 255), (104, 106, 118)):
                    ...
                    for variant in range(4):
                        for output_size in (
                            ...
                            with self.subTest(
                                ...
                                old_order = pygame.transform.scale(
                                    AssetSpriteLibrary._tinted_surface(
                                        source, tint, strength, variant
                                    ),
                                    output_size,
                                )
                                new_order = AssetSpriteLibrary._tinted_surface(
                                    pygame.transform.scale(source, output_size),
                                    tint,
                                    strength,
                                    variant,
                                )
                                self.assertEqual(
                                    pygame.image.tobytes(old_order, "RGBA"),
                                    pygame.image.tobytes(new_order, "RGBA"),
                                )
    ...
    def test_manifest_covers_runtime_visual_roster(self) -> None:
        ...
        named_keys = {
            key for key in manifest["items"] if key.startswith("named.")
        }
        self.assertEqual(
            set(manifest["items"]) - named_keys,
            {
                "potion",
                "mana_potion",
                "identify",
        ...
        # Every named item (base equipment and uniques) has dedicated art
        # resolvable by item name; unidentified display names fall back to
        # generic slot art so hidden gear cannot leak its identity.
        named_items = [
            *((d.name, "weapon") for d in WEAPON_DEFINITIONS),
            *((d.name, "armor") for d in ARMOR_DEFINITIONS),
            *((d.name, d.slot) for d in UNIQUE_ITEM_DEFINITIONS),
        ]
        self.assertEqual(len(named_items), len(named_keys))
        slot_frames = {
            slot: library.resolve_item(slot) for slot in ("weapon", "armor")
        }
        for name, slot in named_items:
            frame = library.resolve_item(slot, name=name)
            self.assertIsNotNone(frame, name)
            assert frame is not None
            self.assertNotEqual(frame.key, slot_frames[slot].key, name)
        masked = library.resolve_item("weapon", name="Unidentified Weapon")
        self.assertIsNotNone(masked)
        assert masked is not None
        self.assertEqual(masked.key, slot_frames["weapon"].key)
        scroll = library.resolve_item(
            "remove_curse", name="Scroll of Remove Curse"
        )
        ...
        self.assertEqual(len(required_world), 36)
        self.assertEqual(set(manifest["world"]), required_world)
        active_world_paths = {
            path
            for entry in manifest["world"].values()
            for path in (
                entry["path"],
                *entry.get("frames", ()),
                *entry.get("variants", ()),
                *entry.get("overlay_frames", ()),
            )
        }
        self.assertEqual(len(active_world_paths), 50)
        for path in sorted(active_world_paths):
            source = library._source_surface(path)
            self.assertIsNotNone(source, path)
            assert source is not None
            self.assertEqual(source.get_size(), (512, 512), path)
        for state in ("open", "closed"):
            entries = {
            ...
            self.assertTrue(
                all(
                    entry["path"]
                    in {
                        f"world/hd/door_{state}_left.png",
                        f"world/hd/door_{state}_right.png",
                    }
    ...
    def test_prewarm_actor_sources_front_loads_the_floor_roster(self) -> None:
        """4.11.0: floor entry prewarms actor sources so first-sight and
        post-eviction disk loads cannot stall combat or input frames."""

        library = AssetSpriteLibrary()
        self.assertTrue(library.available)
        warmed = library.prewarm_actor_sources(
            [
                ("Warden", ""),
                ("Warden", ""),  # duplicates resolve once
                ("Mycelial Matron", "boss"),
                ("No Such Actor", ""),  # unknown names cost nothing
            ]
        )
        self.assertGreater(warmed, 0)
        loads_after_prewarm = library.source_load_count
        # Resolving frames for the prewarmed actors must hit the source cache:
        # no further disk loads for any state or direction of those actors.
        for direction in DIRECTIONS:
            for state in ("idle", "walk", "attack"):
                library.resolve_actor("Warden", state, direction, 0.2)
                library.resolve_actor(
                    "Mycelial Matron", state, direction, 0.2, kind="boss"
                )
        self.assertEqual(library.source_load_count, loads_after_prewarm)

    def test_desktop_source_cache_holds_a_floor_working_set(self) -> None:
        # The 4.11.0 Windows hitch logs showed the previous 384-entry cap
        # thrashing (decoded pinned at the cap, loads:+36..76 per window
        # mid-fight). The byte budget guards memory; the entry cap only needs
        # to exceed a populated floor's actor working set.
        profile = AssetSpriteLibrary.CACHE_PROFILES["desktop"]
        self.assertGreaterEqual(int(profile["source_entries"] or 0), 1024)

    def test_floor_variants_and_guidance_overlay_share_the_selected_base(self) -> None:
        library = AssetSpriteLibrary()
        floor_entry = library.manifest["world"]["floor"]
        guiding_entry = library.manifest["world"]["guiding_floor"]
        expected_variants = [
            "world/hd/floor.png",
        ...
        expected_overlays = [
            f"world/hd/guiding_overlay_{index:03d}.png"
            for index in range(1, 9)
        ]
        self.assertEqual(floor_entry["variants"], expected_variants)
        self.assertEqual(guiding_entry["variants"], expected_variants)
        self.assertEqual(guiding_entry["overlay_frames"], expected_overlays)
        self.assertEqual(len(floor_entry["variants"]), DUNGEON_FLOOR_VARIANTS)
        self.assertEqual(
            library.world_animation_frame_count("guiding_floor"),
            len(expected_overlays) + 1,
        )

        fps = float(guiding_entry["fps"])
        sequence = [
            library.world_animation_frame_index(
                "guiding_floor", (step + 0.01) / fps
            )
        ...
        floor = library.resolve_world("floor", **world_kwargs)
        quiet_guidance = library.resolve_world(
            "guiding_floor", **world_kwargs, animation_frame=0
        )
        active_guidance = library.resolve_world(
            "guiding_floor", **world_kwargs, animation_frame=8
        )
        self.assertIsNotNone(floor)
        self.assertIsNotNone(quiet_guidance)
        self.assertIsNotNone(active_guidance)
        assert (
            floor is not None
            and quiet_guidance is not None
            and active_guidance is not None
        )
        self.assertEqual(
            pygame.image.tobytes(floor[0], "RGBA"),
            pygame.image.tobytes(quiet_guidance[0], "RGBA"),
        )
        self.assertNotEqual(
            pygame.image.tobytes(floor[0], "RGBA"),
            pygame.image.tobytes(active_guidance[0], "RGBA"),
        )

    def test_world_variant_and_overlay_manifest_fields_are_backward_compatible(
        self,
    ) -> None:
        library = AssetSpriteLibrary()
        library.manifest["world"]["synthetic_overlay"] = {
            "path": "base_0.png",
            "variants": [
                "base_0.png",
                "base_1.png",
        ...
        for index in range(4):
            ...
            surface.fill((40 + index * 20, 70, 90, 255))
            surfaces[f"base_{index}.png"] = surface
        surfaces["base_1.png"].set_at((0, 0), (180, 150, 120, 255))
        surfaces["base_1.png"].set_at((2, 2), (60, 70, 90, 96))
        overlay = pygame.Surface((4, 4), pygame.SRCALPHA)
        overlay.set_at((1, 1), (220, 30, 40, 255))
        overlay.set_at((2, 2), (30, 220, 40, 180))
        surfaces["overlay.png"] = overlay

        with mock.patch.object(
            library,
            "_source_surface",
            side_effect=lambda path: surfaces.get(path),
        ) as source_surface:
            kwargs = {
                "target_canvas": (20, 20),
                "target_anchor": (10, 10),
            ...
            quiet = library.resolve_world(
                "synthetic_overlay", **kwargs, animation_frame=0
            )
            active = library.resolve_world(
                "synthetic_overlay", **kwargs, animation_frame=1
            )

        self.assertIsNotNone(quiet)
        self.assertIsNotNone(active)
        assert quiet is not None and active is not None
        loaded_paths = [call.args[0] for call in source_surface.call_args_list]
        self.assertEqual(loaded_paths, ["base_1.png", "base_1.png", "overlay.png"])
        self.assertNotEqual(
            pygame.image.tobytes(quiet[0], "RGBA"),
            pygame.image.tobytes(active[0], "RGBA"),
        )
        # Authored variants replace the old implicit odd-variant mirror, so
        # the resolved surface retains the selected source's geometry.
        self.assertEqual(tuple(quiet[0].get_at((2, 2))), (177, 147, 117, 255))
        self.assertEqual(tuple(quiet[0].get_at((17, 2))), (57, 67, 87, 255))
        quiet_pixels = pygame.image.tobytes(quiet[0], "RGBA")
        active_pixels = pygame.image.tobytes(active[0], "RGBA")
        self.assertEqual(quiet_pixels[3::4], active_pixels[3::4])
        equal_pixels = sum(
            quiet_pixels[index : index + 4] == active_pixels[index : index + 4]
            for index in range(0, len(quiet_pixels), 4)
        )
        self.assertGreater(equal_pixels, 300)
    ...
    def test_stairs_manifest_animation_is_aligned_and_ping_pongs(self) -> None:
        ...
        self.assertTrue(all(surface.get_size() == (512, 512) for surface in surfaces))
        alpha_masks = [
            bytes(
        ...
        for archetype in ARCHETYPES:
            ...
            for state in ("idle", "walk"):
                ...
                for direction, frame_paths in directions.items():
                    expected_frames = 7 if state == "idle" else 8
                    with self.subTest(
    ...
    def test_arcanist_action_clips_are_complete_non_looping_and_wired(self) -> None:
        ...
        # The regenerated attack animates through distinct frames; the old
        # asset's duplicated hold frames are gone.
        self.assertEqual(len(set(north_pixels)), len(north_pixels))
        ...
        for state, expected_fps in (("attack", 12.0), ("cast", 10.0)):
            ...
            for direction in DIRECTIONS:
                with self.subTest(state=state, direction=direction):
                    ...
                    self.assertEqual(end.key[2:5], (state, direction, 7))
    ...
    def test_rogue_skills_select_authored_action_clips(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertTrue(attack.is_asset)
            self.assertEqual(attack.key[1:5], ("rogue", "attack", "north", 4))
        ...
        for state, expected_fps in (("attack", 12.0), ("cast", 10.0)):
            ...
            for direction in DIRECTIONS:
                with self.subTest(state=state, direction=direction):
                    ...
                    # The regenerated clips animate from the first authored
                    # frame; the old rotation-identity import quirk is gone.
    ...
    def test_acolyte_skills_select_authored_action_clips(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertTrue(attack.is_asset)
            self.assertEqual(attack.key[1:5], ("acolyte", "attack", "north", 4))
    ...
    def test_ranger_refresh_uses_reviewed_high_resolution_contract(self) -> None:
        ...
        self.assertEqual(tuple(ranger["source_anchor"]), (128.0, 211.0))
        self.assertEqual(ranger["reference_height"], 150)
    ...
    def test_bar_wall_sconces_render_assets_and_procedural_fallback(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            game = self.make_game(tmpdir)
            # Pin the canonical 1.0 render bucket: the HD default zoom sits on
            # the 0.8 bucket since 4.9.24, and this test's mount-point math
            # uses the unscaled TILE_W/TILE_H constants.
    ...
    def test_world_reference_width_controls_authored_tile_footprint(self) -> None:
        ...
        entry["reference_width"] = reference_width * 2
        try:
            ...
            entry["reference_width"] = reference_width
            library.clear_derived_caches()

    def test_max_zoom_floor_uses_native_master_and_bucketed_byte_cache(
        self,
    ) -> None:
        library = AssetSpriteLibrary()
        tint = (104, 106, 118)
        ...
        # WORLD_SCALE 8 (the 1.6 bucket) gives a 512 px tile plus a 32 px
        # transparent margin. The floor's 5/8 source anchor therefore lands at
        # y=352 and the authored master needs no transform at all.
        with mock.patch.object(
            pygame.transform,
            "scale",
            wraps=pygame.transform.scale,
        ) as scale:
            native = library.resolve_world(
                "floor",
                target_canvas=(576, 576),
                target_anchor=(288, 352),
                tint=tint,
                accent=accent,
                variant=0,
        ...
        self.assertEqual(native[0].get_size(), (576, 576))
        source = library._source_surface(
            library.manifest["world"]["floor"]["variants"][0]
        )
        self.assertIsNotNone(source)
        assert source is not None
        source_bounds = source.get_bounding_rect(min_alpha=1)
        self.assertEqual(
            native[0].get_bounding_rect(min_alpha=1),
            source_bounds.move(32, 32),
        )

        same = library.resolve_world(
            "floor",
            target_canvas=(576, 576),
            target_anchor=(288, 352),
            tint=tint,
        ...
        lower = library.resolve_world(
            "floor",
            target_canvas=(504, 504),
            target_anchor=(252, 308),
            tint=tint,
            accent=accent,
            variant=0,
            render_scale_bucket=1.4,
        )
        self.assertIs(native, same)
        self.assertIsNotNone(lower)
        assert lower is not None
        self.assertIsNot(native[0], lower[0])
        self.assertEqual(lower[0].get_bounding_rect(min_alpha=1).width, 448)

        stats = library.cache_stats()
        self.assertEqual(stats["world_surfaces"], 2)
        self.assertLessEqual(
            stats["world_surface_bytes"],
            library.WORLD_CACHE_BYTE_BUDGET,
        )

        # Exercise byte eviction independently of the much larger production
        # budget: two native canvases cannot remain when capped below their
        # combined storage.
        library._world_cache.maximum_weight = (
            native[0].get_width()
            * native[0].get_height()
            * native[0].get_bytesize()
            + 64
        )
        library.resolve_world(
            "floor",
            target_canvas=(576, 576),
            target_anchor=(288, 352),
            tint=tint,
        ...
        self.assertLessEqual(
            library.cache_stats()["world_surface_bytes"],
            library._world_cache.maximum_weight,
        )
    ...
    def test_directional_animation_and_resolved_frames_are_cached(self) -> None:
        ...
        self.assertEqual(attack_recovery.key[-1], 7)
    ...
    def test_player_action_ttl_drives_the_full_authored_clip(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(resolved_frames[-1].key[-1], 7)
    ...
    def test_modern_world_preserves_canonical_canvases_and_legacy_toggle(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            game = self.make_game(tmpdir)
            # Pin the canonical 1.0 render bucket: the HD default zoom sits on
            # the 0.8 bucket since 4.9.24 and would derive 0.8-scaled canvases.
            # Restart prewarm populated the tile caches at the default bucket;
            # re-prewarm at the canonical bucket so the door-cache census
            # below keeps counting the complete prewarmed family.
            game.set_view_zoom(1.0)
            game.tile_cache.clear()
            game.door_tile_cache.clear()
            game.prewarm_tile_cache()
            ...
            authored_flat_size = (TILE_W + margin * 2, TILE_W + margin * 2)
            tall_size = (
                TILE_W + margin * 2,
            ...
            self.assertEqual(stairs.get_size(), authored_flat_size)
            ...
            self.assertEqual(closed[1:], (wall_x, wall_y))
            self.assertEqual(opened[1:], (wall_x, wall_y))
            self.assertNotEqual(
    ...
    def test_graphics_option_scrolls_persists_and_stays_out_of_run_save(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(options["schema_version"], 10)
