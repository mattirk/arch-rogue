    ...
    def test_contextual_hints_visual_effects_inventory_ui_and_older_save_load(
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                game.secrets.clear()
                game.traps.append(Trap(px + 0.8, py, "Rune Trap", 13, revealed=True))
                hint = game.current_interaction_hint()
                self.assertIsNotNone(hint)
                assert hint is not None
...
class EquippedItemFocusTests(unittest.TestCase):
    def make_game(self, tmpdir: str) -> Game:
        game = Game(
            screen_size=(760, 520),
            headless=True,
            save_path=Path(tmpdir) / "run.json",
    ...
    def test_equipment_focus_cycles_and_clears_with_selection(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            game = self.make_game(tmpdir)
            game.player.equipment["weapon"] = Item(
                "Emberbrand", "weapon", power=36, rarity="Unique"
            )
            game.player.equipment["armor"] = Item(
                "Choir of Bone", "armor", defense=18, rarity="Unique"
            )
            game.player.inventory[:] = [
                Item("Iron Sword", "weapon", power=5, rarity="Common")
            ]

            self.assertIsNone(game.focused_equipped_item())
            game.cycle_equipment_focus(forward=True)
            self.assertEqual(game.inventory_equipment_focus, "weapon")
            self.assertEqual(game.focused_equipped_item().name, "Emberbrand")
            game.cycle_equipment_focus(forward=True)
            self.assertEqual(game.inventory_equipment_focus, "armor")
            game.cycle_equipment_focus(forward=True)
            self.assertIsNone(game.inventory_equipment_focus)

            # Selecting a bag row returns the detail card to the bag item.
            game.inventory_equipment_focus = "weapon"
            game.set_inventory_selection(0)
            self.assertIsNone(game.inventory_equipment_focus)

            # Empty slots are skipped when cycling and drop stale focus.
            game.player.equipment["armor"] = None
            game.cycle_equipment_focus(forward=False)
            self.assertEqual(game.inventory_equipment_focus, "weapon")
            game.inventory_equipment_focus = "armor"
            self.assertIsNone(game.focused_equipped_item())
            self.assertIsNone(game.inventory_equipment_focus)

    def test_equipped_view_summary_shows_absolute_stats(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            game = self.make_game(tmpdir)
            weapon = Item(
                "Emberbrand",
            ...
            summary = game.item_decision_summary(weapon, equipped_view=True)
            self.assertTrue(summary.startswith("36 damage"), summary)
            self.assertNotIn("vs equipped", summary)

    def test_inventory_draw_publishes_equipment_card_rects(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            # Full-size window: the compact (<112px) equipment panel is
            # text-only by design and publishes no clickable cards.
            game = Game(
            ...
            game.inventory_equipment_focus = "weapon"
            game.draw()
            rects = getattr(game, "_inventory_equipment_card_rects", None)
            self.assertIsNotNone(rects)
            slots = [slot for slot, _ in rects]
            self.assertEqual(slots, ["weapon", "armor"])
            for _, rect in rects:
