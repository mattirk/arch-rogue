...
class ShrineAndSecretTests(unittest.TestCase):
    ...
    def test_new_shrine_and_secret_effects(self) -> None:
        ...
        try:
            ...
            # 5.0: the Haste Shrine grants a real, capped move-speed blessing
            # through the channel locomotion consumes (player_move_speed),
            # instead of bumping the inert legacy ``player.speed`` rating.
            base_move = game.player_move_speed()
            ...
            self.assertGreater(game.player.shrine_move_bonus, 0.0)
            self.assertGreater(game.player_move_speed(), base_move)
