...
def _populate_boss_floor(game: Game, depth: int = 3) -> None:
    game.current_depth = depth
    game.apply_floor_plan_for_current_depth()
    game.dungeon = Dungeon(
        game.rng,
        boss_arena=game.current_floor_needs_boss_arena(),
    )
    game.enemies = []
    game.items = []
    game.traps = []
...
class StairsGateTests(unittest.TestCase):
    ...
    def test_floor_guardian_blocks_host_and_joiner_stair_interactions(
        self,
    ) -> None:
        for actor_index in (0, 1):
            with self.subTest(actor_index=actor_index):
                with tempfile.TemporaryDirectory() as tmpdir:
                    game = _make_host_game_with_partner(tmpdir)
                    _populate_boss_floor(game)
                    actor = game.players[actor_index]
                    other = game.players[1 - actor_index]
                    _move_to_stairs(game, actor)
                    start_x, start_y = game.dungeon.rooms[0].center
                    other.x, other.y = start_x + 0.5, start_y + 0.5
                    depth_before = game.current_depth

                    with game.acting_as_player(actor):
                        game.interact()

                    self.assertEqual(game.current_depth, depth_before)
                    self.assertTrue(game.stairs_guardian_alive())

                    guardian = game.boss_enemy()
...
class BossArenaPartyTeleportTests(unittest.TestCase):
    def test_either_player_engaging_moves_the_whole_party_into_arena(
        self,
    ) -> None:
        for trigger_index in (0, 1):
            with self.subTest(trigger_index=trigger_index):
                with tempfile.TemporaryDirectory() as tmpdir:
                    ...
                    boss_room = game.dungeon.room_at(boss.x, boss.y)
                    self.assertIsNotNone(boss_room)
                    assert boss_room is not None

                    trigger = game.players[trigger_index]
                    other = game.players[1 - trigger_index]
                    trigger.x, trigger.y = boss.x, boss.y
                    start_x, start_y = game.dungeon.rooms[0].center
                    other.x, other.y = start_x + 0.5, start_y + 0.5
                    ...
                    boss_gap = game.boss_arena_player_clearance(boss)
                    for player in game.players:
                        self.assertIs(
                            game.dungeon.room_at(player.x, player.y),
                            boss_room,
                        )
                        self.assertFalse(
                        ...
                        self.assertIsNone(player.net_x)
                        self.assertIsNone(player.net_y)
                    self.assertGreaterEqual(
                        math.hypot(
                            game.players[0].x - game.players[1].x,
                            game.players[0].y - game.players[1].y,
                        ),
                        PLAYER_HIT_RADIUS * 2 + 0.12,
                    )

    def test_fallen_partner_is_moved_inside_when_arena_seals(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            game = _make_host_game_with_partner(tmpdir)
            _populate_boss_floor(game)
            boss = game.boss_enemy()
            ...
            self.assertIsNotNone(boss_room)
            assert boss_room is not None
            host, partner = game.players
            host.x, host.y = boss.x, boss.y
            start_x, start_y = game.dungeon.rooms[0].center
            partner.x, partner.y = start_x + 0.5, start_y + 0.5
            partner.hp = 0
            ...
            self.assertTrue(game.boss_engaged)
            self.assertIs(
                game.dungeon.room_at(partner.x, partner.y),
                boss_room,
            )
