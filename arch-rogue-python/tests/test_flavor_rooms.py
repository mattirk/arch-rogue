...
class FlavorRoomTests(unittest.TestCase):
    ...
    def test_idle_npcs_spawn_with_expected_flavor_room_roles(self) -> None:
        ...
        try:
            ...
            # Flavor actors never become shopkeepers. Each resident dancing
            # frog, however, now exposes the Garden's shared mini-game prompt.
            for room in (bar_room, garden_room):
                self.assertFalse(
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertTrue(
                {npc.kind for npc in game.idle_npcs}.issubset(
                    {"bar", "bar_dancer", "garden", "garden_frog", "lossless_soul"}
                )
            )
    ...
    def test_bar_props_are_solid_spaced_and_door_safe(self) -> None:
        barrel_keys = PopulationMixin._BAR_BARREL_ANCHORS
        table_keys = PopulationMixin._BAR_TABLE_ANCHORS
        bars = full_sets = minimal_sets = 0
        for seed in range(1, 300):
            ...
            room = dungeon.rooms[special.room_index]
            harness = _FlavorPopulationHarness(dungeon)
            harness._populate_bar_special_room(special, room)

            barrels = [
                special.anchor(key)
                for key in barrel_keys
                if special.anchor(key) is not None
            ]
            tables = [
                special.anchor(key)
            ...
            area = (room.w - 2) * (room.h - 2)
            expected = 2 + min(2, area // 28)
            self.assertLessEqual(len(barrels), expected)
            self.assertLessEqual(len(tables), expected)
            self.assertGreaterEqual(
                len(barrels) + len(tables), 2, f"seed {seed}: bar looks empty"
            )
            if len(barrels) == 4 and len(tables) == 4:
                full_sets += 1
            if expected == 2:
                minimal_sets += 1

            placed = barrels + tables
            # Distinct tiles with breathing room between furnishings.
            ...
            for i, a in enumerate(placed):
                for b in placed[i + 1 :]:
                    self.assertGreaterEqual(
                        ...
                        2,
                        f"seed {seed}: furnishings {a} and {b} too close",
                    )

            # Solid through the shared furnishing collision machinery.
            for ax, ay in placed:
                self.assertIn((ax, ay), dungeon.solid_furnishing_tiles)
                self.assertTrue(
                    dungeon.blocked_for_radius(ax + 0.5, ay + 0.5, 0.27)
                )
                self.assertTrue(dungeon.is_floor(ax + 0.5, ay + 0.5))

            # Never on a door-front tile nor its south/east/southeast
            # neighbors (the shifted box would pinch the entrance).
            forbidden = set()
            for x in range(room.x, room.x + room.w):
                for y in range(room.y, room.y + room.h):
                    ...
                    for ox, oy in ((-1, 0), (1, 0), (0, -1), (0, 1)):
                        fx, fy = x + ox, y + oy
                        if (
                            room.x < fx < room.x + room.w - 1
                            and room.y < fy < room.y + room.h - 1
                        ):
                            forbidden.add((fx, fy))
                            forbidden.update(
                                (
            ...
            for tile in placed:
                self.assertNotIn(
                    tile, forbidden, f"seed {seed}: prop crowds a doorway"
                )

            # Re-running population keeps the persisted layout untouched.
            before = {key: special.anchor(key) for key in barrel_keys + table_keys}
            harness._populate_bar_special_room(special, room)
            self.assertEqual(
                before,
                {key: special.anchor(key) for key in barrel_keys + table_keys},
            )
        self.assertGreater(bars, 50)
        self.assertGreater(full_sets, 0, "no large bar exercised the 4+4 case")
        self.assertGreater(minimal_sets, 0, "no small bar exercised the 2+2 case")
