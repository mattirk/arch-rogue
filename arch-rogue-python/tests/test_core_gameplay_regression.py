    ...
    def test_item_interactions_and_hotkey_behaviors(self) -> None:
        ...
        try:
            ...
            self.assertEqual(
                ...
                10 + game.player.level * 2 + game.player.melee_bonus + 9,
            )
    ...
    def test_stairs_advance_until_depth_ten_then_boss_gates_victory(self) -> None:
        ...
        try:
            ...
            for expected_depth in range(2, DUNGEON_DEPTH + 1):
                ...
                if game.floor_guardian_alive():
                    guardian = game.boss_enemy()
                    self.assertIsNotNone(guardian)
                    assert guardian is not None
                    blocked_depth = game.current_depth
                    game.interact()
                    self.assertEqual(game.current_depth, blocked_depth)
            ...
            game.interact()
            # 4.9: victory now passes through the Tenth Bell — the Gate asks
            # its question at the final stairs before the run completes.
            self.assertEqual(game.state, "playing")
