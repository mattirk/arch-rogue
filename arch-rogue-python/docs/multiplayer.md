...
`drop_slot`, `choose_discipline`, `shop_open`, `shop_buy`, `shop_sell`,
`minigame`. `use_slot`/`drop_slot` carry the inventory slot index in `target`;
`choose_discipline` carries the discipline node key; shop actions carry the
validated keeper/item address; `interact` targets the nearest host-validated
interactable. `minigame` carries either the instance-scoped `instance:ready`
confirmation or `instance:revision:cell`: the host accepts both while the
dungeon simulation is story-paused. Ready confirmations are idempotent and the
preview begins only after both captured participants confirm. Cell intents
ignore stale revisions without a mistake. Consecutive ordered presses made from
one 15 Hz snapshot may continue only while each prior press produced the exact
next host revision; a host input or timer change breaks that chain. An action
...
- `snapshot.state` (fast, every tick): `players` (compact: position, facing,
  ...
  the floor baseline), `boss.engaged`, `paused` (host pause reason),
  `mini_game` (absolute active state or `null`, every tick so coalescing and
  reconnect converge), `depth`, `elapsed`, `spawns`? (full dicts for first-seen
  enemies).
- `snapshot.state.slow` (every 5th tick or on list-length change): full
  `players`, `items`, `traps`, `shrines`, `secrets`, `familiars`, `shop_met`,
  and bounded `room_games` records for completed Garden/Soul interludes and
  Soul choices so the guest's room hints converge too.
