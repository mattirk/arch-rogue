...
# 4.9.x cooperative mini-games use ``minigame`` with either the
# instance-scoped "instance:ready" target or compact
# "instance:revision:cell" target produced by ``story.minigames``. It remains
# an ordinary joiner-only intent so the relay needs no new stateful path.
# 4.10 Big Hit is the only hold-to-charge action: ``bighit`` starts the
# charge and ``bighit_release`` lets go (the host cancels the charge unless
# it already passed the commit point). Two intents instead of one because
# press and release are separate player decisions, unlike every other
