...
from .constants import (
    DUNGEON_DEPTH,
    LIGHT_BAR_TAP_COLOR,
    LIGHT_BAR_TAP_INTENSITY,
    LIGHT_BAR_TAP_RADIUS,
    LIGHT_BAR_WALL_ELEVATION,
    LIGHT_SHRINE_INTENSITY,
    LIGHT_SHRINE_RADIUS,
    LIGHT_SOUL_PROP_INTENSITY,
    LIGHT_SOUL_PROP_RADIUS,
    LIGHT_STAIRS_COLOR,
...
class PopulationMixin:
    def _populate_dungeon(self) -> None:
        ...
        for room_index, room in enumerate(self.dungeon.rooms[1:], start=1):
            ...
            # spawn multiplier (0.5) already halved the base chance; 4.2 trimmed
            # it to 0.42. 4.9.23 runs a second across-the-board rarity pass and
            # trims the per-room multiplier to 0.33, keeping exploration loot
            # present on most floors without guaranteeing it per room.
            loot_chance *= 0.33
            ...
            # 4.9.23 rarity pass: secret caches carry loot, so their spawn rate
            # drops with the rest of the board (base 0.16 -> 0.12, cap 0.48 ->
            # 0.38). Cache contents are unchanged.
            if (
                room_index > 2
                and not is_final_room
                and self.rng.random()
        ...
        # 4.9.23 rarity pass: the per-floor Cartographer's Stash follows the
        # board-wide trim (base 0.45 -> 0.36, clamp [0.10, 0.72] ->
        # [0.08, 0.60]) so roughly a third of floors carry one.
        ...
        # Special rooms placed their solid furnishings (bar barrels/tables,
        # soul-hall props) AFTER the enemy rolls above, so an enemy in a bar
        # room may be standing inside a collision box it can never leave.
        # Deterministic post-pass: nudge every trapped enemy to the nearest
        # free spot. No RNG, so the shared stream is untouched.
        self._relocate_enemies_off_furnishings()
    ...
    def _relocate_enemies_off_furnishings(self) -> None:
        """Move enemies out of solid furnishing boxes placed after they spawned."""
        if not self.dungeon.solid_furnishing_boxes:
            return
        for enemy in self.enemies:
            radius = 0.62 if enemy.size >= 2 else 0.27
            if not self.dungeon.blocked_for_radius(enemy.x, enemy.y, radius):
                continue
            free_x, free_y = self.drop_position_near(enemy.x, enemy.y)
            if self.dungeon.blocked_for_radius(free_x, free_y, radius):
                # The ring scan came up empty (dense furnishing cluster):
                # deterministic sweep over the enemy's room for any free tile.
                room = self.dungeon.room_at(enemy.x, enemy.y)
                if room is None:
                    continue
                for ty in range(room.y, room.y + room.h):
                    for tx in range(room.x, room.x + room.w):
                        candidate_x, candidate_y = tx + 0.5, ty + 0.5
                        if not self.dungeon.blocked_for_radius(
                            candidate_x, candidate_y, radius
                        ):
                            free_x, free_y = candidate_x, candidate_y
                            break
                    else:
                        continue
                    break
    ...
    def _special_room_handlers(self) -> dict[str, SpecialRoomHandler]:
        ...
        if handlers is None:
            handlers = {
                ...
                "lossless_soul": self._populate_lossless_soul_special_room,
            }
    ...
    def _populate_light_sources(self) -> None:
        ...
        for special_room in getattr(self.dungeon, "special_rooms", []):
            if special_room.room_index not in range(room_count):
                continue
            if special_room.kind not in ("bar", "garden", "lossless_soul"):
                continue
            room = self.dungeon.rooms[special_room.room_index]
            if special_room.kind == "lossless_soul":
                # Every solid furnishing casts its own faint memory-light as a
                # real static light source, so the pool fades with scene
                # lighting instead of fighting the screen-space lighting pass.
                # The brazier keeps its full flickering flame; the others hold
                # a dimmer steady aura. All carry kind="soul_prop" and are
                # rebuilt from the current anchors on every reconcile, which
                # follows props that slid aside from a doorway.
                for anchor_key, color, radius, intensity, flicker in (
                    (
                        "soul_brazier",
                        (120, 214, 205),
                        LIGHT_TORCH_RADIUS,
                    ...
                    if anchor is None and anchor_key == "soul_brazier":
                        anchor = room.center
                    if anchor is None:
                        continue
                    add(
            ...
            if special_room.kind == "bar":
                ...
                if tapped is not None:
                    _key, tap_x, tap_y = tapped
                    add(
                        LightSource(
                            x=tap_x + 0.5,
                            y=tap_y + 0.5,
                            radius=LIGHT_BAR_TAP_RADIUS,
                            color=LIGHT_BAR_TAP_COLOR,
                            intensity=LIGHT_BAR_TAP_INTENSITY,
                            ttl=None,
                            flicker=True,
                            flicker_seed=(tap_x * 131 + tap_y * 17 + 421) % 9973,
                            kind="bar_tap",
                        )
                    )
                continue
    ...
    def _reconcile_static_light_sources(self) -> None:
        ...
        soul_room: Room | None = None
        soul_special = self.dungeon.special_room_for_kind("lossless_soul")
        if soul_special is not None and 0 <= soul_special.room_index < len(
            self.dungeon.rooms
        ):
            soul_room = self.dungeon.rooms[soul_special.room_index]

        retained: list[LightSource] = []
        for source in getattr(self, "light_sources", []):
            # Bar fixtures (wall sconces + the tapped barrel's candle) rebuild
            # from the current anchors on every load.
            if source.kind in ("bar_wall_light", "bar_tap"):
                continue
            # Soul-hall furnishing lights rebuild from the current anchors so
            # they follow props that slid aside from a doorway; saves from
            # before kind="soul_prop" carried the brazier flame as a plain
            # torch inside the hall.
            if source.kind == "soul_prop":
                continue
            if (
                soul_room is not None
                and source.kind == "torch"
                and self._room_contains_world_point(
                    soul_room, source.x, source.y
                )
            ):
                continue
            if (
    ...
    def _populate_special_rooms(self) -> None:
        for special_room in list(getattr(self.dungeon, "special_rooms", [])):
            if not (0 <= special_room.room_index < len(self.dungeon.rooms)):
                continue
            self._run_special_room_handler(
                special_room, self.dungeon.rooms[special_room.room_index]
            )

    def _run_special_room_handler(
        self, special_room: SpecialRoom, room: Room
    ) -> bool:
        handler = self._special_room_handlers().get(special_room.kind)
        if handler is None:
            return False
        handler(special_room, room)
        # The definition owns the safe-refuge contract: ``spawn_policy="safe"``
        # rooms (shop, quest, soul hall) are cleared of hostiles and traps here
        # so individual handlers stay population-only.
        if special_room.spawn_policy == "safe":
            self._clear_room_hostiles_and_traps(room)
        return True

    def _reconcile_bar_dancers(self) -> None:
        """Backfill the dancer and furnishings per persisted bar, no patron rolls."""
        for special_room in list(getattr(self.dungeon, "special_rooms", [])):
            if special_room.kind != "bar":
                continue
            if not (0 <= special_room.room_index < len(self.dungeon.rooms)):
                continue
            room = self.dungeon.rooms[special_room.room_index]
            # Props first (no-op when the save already carries the anchors),
            # so a freshly backfilled dancer spawn avoids the furniture.
            self._place_bar_props(special_room, room)
    ...
    def _reserve_special_room_anchor(
        ...
        # Solid furnishings (soul-hall props) derive their collision tiles from
        # these anchors; keep the dungeon's probe set in sync on every move.
    ...
    # These rooms never clear hostiles. Their optional humanoid residents can
    # be greeted, while either of the Garden's two frogs opens its mini-game.
    # Bars and gardens may host one traveler at 50%; every bar also receives
    # one dedicated dancer. Layout-seeded local RNG keeps population
    # deterministic without advancing the shared gameplay RNG.
    _BAR_NPC_NAMES = (
        "Barkeep Ossar",
        "Drunk Mabel",
        "Quiet Fenn",
    ...
    # 4.8.6 bar furnishings: solid ale barrels along the walls and standing
    # tables in the open floor, 2-4 of each depending on room size. The
    # anchor keys are registered in ``SOLID_FURNISHING_ANCHOR_KEYS`` so the
    # shifted/inset collision boxes apply exactly as for the soul hall.
    _BAR_BARREL_ANCHORS = (
        "bar_barrel_1",
        "bar_barrel_2",
        "bar_barrel_3",
    ...
    def _place_bar_props(self, special_room: SpecialRoom, room: Room) -> None:
        """Reserve tiles for the bar's solid furnishings (barrels + tables).

        Placement rolls once per room from the layout-seeded flavor RNG and
        persists via the reserved anchors: if any bar-prop anchor already
        exists (a reloaded save), the saved layout is kept as-is. Barrels
        prefer wall-adjacent tiles, standing tables the open middle. Tiles
        just inside a doorway — and their south/east/southeast neighbors,
        which the north-west-shifted collision boxes would spill into — are
        excluded, as are reserved tiles and tiles under existing NPCs, and
        furnishings keep a Chebyshev distance of at least 2 from each other
        so every aisle stays walkable.
        """
        prop_keys = self._BAR_BARREL_ANCHORS + self._BAR_TABLE_ANCHORS
        if any(special_room.anchor(key) is not None for key in prop_keys):
            return
        rng = self._flavor_room_rng(special_room, salt=0xBA55)
        interior = [
            (x, y)
        ...
        area = len(interior)
        extra = min(2, area // 28)
        barrel_count = 2 + extra
        table_count = 2 + extra

        door_fronts = self._room_door_front_tiles(room)
        blocked = set(door_fronts)
        for fx, fy in door_fronts:
            blocked.update(((fx + 1, fy), (fx, fy + 1), (fx + 1, fy + 1)))
        blocked.update(
        ...
        for npc in self.idle_npcs:
            if self._room_contains_world_point(room, npc.x, npc.y):
                blocked.add((math.floor(npc.x), math.floor(npc.y)))

        def wall_adjacent(tile: tuple[int, int]) -> bool:
            x, y = tile
            return x in (room.x + 1, room.x + room.w - 2) or y in (
                room.y + 1,
                room.y + room.h - 2,
            )
        ...
        def pick(
            ...
            for tile in pool:
                ...
                if tile in blocked:
                    continue
                if any(
                    max(abs(tile[0] - tx), abs(tile[1] - ty)) < 2
                    for tx, ty in taken
                ):
                    continue
                taken.append(tile)
                self._reserve_special_room_anchor(
                    special_room, keys[chosen], tile[0], tile[1]
                )
                chosen += 1

        along_walls = [tile for tile in interior if wall_adjacent(tile)]
        open_middle = [tile for tile in interior if not wall_adjacent(tile)]
        pick(along_walls or interior, barrel_count, self._BAR_BARREL_ANCHORS)
        pick(open_middle or interior, table_count, self._BAR_TABLE_ANCHORS)

    def bar_tapped_barrel(
        self, special_room: SpecialRoom | None = None
    ) -> tuple[str, int, int] | None:
        """The bar's tapped ale barrel as ``(anchor_key, x, y)``, or ``None``.

        Every bar taps exactly one of its placed barrels for a free drink. The
        pick re-derives from the layout-seeded flavor RNG over the persisted
        anchors instead of storing a key, so saves, reloads, and co-op floor
        syncs agree without any schema change; only the one-drink outcome is
        recorded (``state["barrel_drunk"]``).
        """
        if special_room is None:
            special_room = self.dungeon.special_room_for_kind("bar")
        if special_room is None or special_room.kind != "bar":
            return None
        keys = [
            key
            for key in self._BAR_BARREL_ANCHORS
            if special_room.anchor(key) is not None
        ]
        if not keys:
            return None
        rng = self._flavor_room_rng(special_room, salt=0x7A9)
        key = rng.choice(keys)
        anchor = special_room.anchor(key)
        if anchor is None:
            return None
        return key, anchor[0], anchor[1]
    ...
    # --- lossless soul hall ---------------------------------------------
    # A sealed sanctum that rolls at 50% on every depth. It hosts exactly one
    # keeper NPC — the Lossless Soul — plus her memory furnishings (mirror,
    # chimes, brazier, reliquary) on deterministic, geometry-derived anchors.
    # Unlike bar/garden actors she is interactable: approaching her offers a
    # dialogue cutscene, with per-room outcome kept on ``SpecialRoom.state``.
    _LOSSLESS_SOUL_NAME = "Lossless Soul"
    _LOSSLESS_SOUL_ROLE = "Keeper of Unlost Memory"
    _LOSSLESS_SOUL_COLOR = (142, 214, 205)
    _LOSSLESS_SOUL_PROP_ANCHORS = (
        "soul_mirror",
        "soul_chimes",
        "soul_brazier",
        "soul_reliquary",
    ...
    def _populate_lossless_soul_special_room(
        self, special_room: SpecialRoom, room: Room
    ) -> None:
        self._place_lossless_soul_props(special_room, room)
        self._ensure_lossless_soul(special_room, room)

    def _place_lossless_soul_props(
        self, special_room: SpecialRoom, room: Room
    ) -> None:
        """Reserve deterministic interior tiles for the hall furnishings.

        Pure geometry, no RNG: the mirror faces the entrance from the top row
        with the chimes beside it, the brazier warms the near corner, and the
        reliquary waits along the far wall. The renderer draws props at these
        anchors; reserving them also keeps the keeper's spawn and waypoints off
        the furniture. Because the furnishings are solid, the tiles directly
        inside the hall's doorways (and a retained keeper's tile on reconcile)
        are excluded — a prop whose ideal spot conflicts slides to the nearest
        free interior tile, so halls without conflicts keep their layout.
        """
        cx, _cy = room.center
        top = room.y + 1
        door_fronts = self._room_door_front_tiles(room)
        blocked = set(door_fronts)
        for fx, fy in door_fronts:
            # The furnishing collision box shifts north-west, so a prop just
            # south/east/southeast of a doorway tile would pinch the entrance.
            blocked.update(((fx + 1, fy), (fx, fy + 1), (fx + 1, fy + 1)))
        keeper = special_room.anchor("lossless_soul")
        if keeper is not None:
            blocked.add((keeper[0], keeper[1]))
        placements = (
            ("soul_mirror", (cx, top)),
            ("soul_chimes", (max(room.x + 1, cx - 2), top)),
            ("soul_brazier", (room.x + 2, room.y + 2)),
            ("soul_reliquary", (room.x + room.w - 3, room.y + room.h // 2)),
        )
        for key, (dx, dy) in placements:
            x, y = self._nearest_free_soul_prop_tile(room, dx, dy, blocked)
            blocked.add((x, y))
            self._reserve_special_room_anchor(special_room, key, x, y)

    def _room_door_front_tiles(self, room: Room) -> set[tuple[int, int]]:
        """Interior tiles directly inside a special room's doorways.

        A solid furnishing on one of these would wall the entrance shut, so
        furnishing placement (soul hall, bar) treats them as forbidden.
        """
        fronts: set[tuple[int, int]] = set()
        tiles = self.dungeon.tiles
        for x in range(room.x, room.x + room.w):
            for y in range(room.y, room.y + room.h):
                on_ring = x in (room.x, room.x + room.w - 1) or y in (
                    room.y,
                    room.y + room.h - 1,
                )
                if not on_ring:
                    continue
                if tiles[x][y] not in (Tile.CLOSED_DOOR, Tile.OPEN_DOOR):
                    continue
                for ox, oy in ((-1, 0), (1, 0), (0, -1), (0, 1)):
                    ix, iy = x + ox, y + oy
                    if (
                        room.x + 1 <= ix < room.x + room.w - 1
                        and room.y + 1 <= iy < room.y + room.h - 1
                    ):
                        fronts.add((ix, iy))
        return fronts

    def _nearest_free_soul_prop_tile(
        self,
        room: Room,
        dx: int,
        dy: int,
        ...
        """First interior tile by distance from the ideal spot not in ``blocked``.

        The (distance, y, x) sort keeps the pick deterministic, and a clear
        ideal spot wins at distance zero. The unreachable all-blocked fallback
        returns the ideal spot itself.
        """
        candidates = sorted(
            (
                (x, y)
    ...
    def _ensure_lossless_soul(
        ...
        retained = room_souls[0] if room_souls else None
        self.idle_npcs = [
            npc
            for npc in self.idle_npcs
            if not (
        ...
        if retained is not None:
            anchor = special_room.anchor("lossless_soul")
            if anchor is None:
                anchor = (math.floor(retained.x), math.floor(retained.y))
            self._reserve_special_room_anchor(
                special_room, "lossless_soul", anchor[0], anchor[1]
            )
            return

        rng = self._flavor_room_rng(special_room, salt=0x5015)
        spawn_tiles = self._flavor_npc_spawn_tiles(special_room, room, rng, 1)
        if not spawn_tiles:
            return
        soul_x, soul_y = spawn_tiles[0]
        self._reserve_special_room_anchor(
            special_room, "lossless_soul", soul_x, soul_y
        )
        self.idle_npcs.append(
            IdleNpc(
                x=soul_x + 0.5,
                y=soul_y + 0.5,
                kind="lossless_soul",
                name=self._LOSSLESS_SOUL_NAME,
                role=self._LOSSLESS_SOUL_ROLE,
                color=self._LOSSLESS_SOUL_COLOR,
            )
        )

    def _reconcile_lossless_souls(self) -> None:
        """Backfill the keeper and her furnishings in pre-4.8.6 saves."""
        for special_room in list(getattr(self.dungeon, "special_rooms", [])):
            if special_room.kind != "lossless_soul":
                continue
            if not (0 <= special_room.room_index < len(self.dungeon.rooms)):
    ...
    def _reconcile_npc_ally_state(self) -> None:
        """Re-derive story-driven NPC aggression and reset ally state (4.8.10).

        The Story Guest fights once its dialogue resolved through a real
        choice (a forsaken "unanswered" beat never takes up arms); the
        keeper once her hall's ``soul_choice`` is set. Both derive here so
        no new story state is ever written. Older saves whose flags predate
        the ally fields roll combat stats lazily; transient combat state
        resets; and any ally standing inside a solid furnishing is clamped
        to the first open neighbor tile.
        """
        ...
        for npc in self.idle_npcs:
            if npc.kind == "lossless_soul":
                ...
                npc.aggressive = bool(
                    special is not None and special.state.get("soul_choice")
                )
        for ally in (*self.story_guests, *self.idle_npcs):
            ally.attack_timer = 0.0
            ally.nav_latch = 0.0
            ally.combat_active = False
            if not ally.aggressive:
                continue
            self._roll_npc_ally_stats(ally)
            self._clamp_actor_clear_of_furnishings(ally)

    def _clamp_actor_clear_of_furnishings(
        self, actor, radius: float = 0.27
    ) -> None:
        """Move an actor out of solid furnishings to the first safe neighbor.

        Same probe-and-offsets contract as the player backfill on load: the
        collision box is shifted and inset, so test the box probe directly
        rather than anchor-tile membership.
        """
        if not self.dungeon.furnishing_blocks_radius(actor.x, actor.y, radius):
            return
        tile = (int(actor.x), int(actor.y))
        for offset_x, offset_y in (
            ...
            if not self.dungeon.blocked_for_radius(
                candidate_x, candidate_y, radius, block_stairs=True
            ):
                actor.x, actor.y = candidate_x, candidate_y
                break
    ...
    def _make_shop_inventory(self, room: Room) -> list[Item]:
        stock: list[Item] = [
            ...
            # Always stocked: with curse-locked equipment the shop must stay a
            # reliable way out of a cursed slot.
            Item("Scroll of Remove Curse", "remove_curse", rarity="Magic"),
        ]
        stock.append(
    ...
    def _make_enemy(
        ...
        enemy = Enemy(
            ...
            ability_keys=definition.abilities,
        )
    ...
    def _make_boss(self, x: float, y: float) -> Enemy:
        ...
        # 4.9: knowing the Tyrant's true name changes his nameplate — the
        # spent memory buys the words on the health bar.
        if self.story_name_known("sorn"):
            boss_name = f"Sorn Voss, {boss_name}"
        ...
        boss = Enemy(
            ...
            ability_keys=definition.abilities if definition is not None else (),
        )
        boss = self._apply_run_modifier(boss)
        ...
        # 4.9 name economy: a spoken true name staggers the Toll-Keeper before
        # the first blow — he cannot hold the door and hear it at once.
        if self.story_name_known("sorn"):
            boss.max_hp = int(boss.max_hp * 0.88)
        if self.story_name_known("liss"):
            boss.max_hp = int(boss.max_hp * 0.94)
        boss.hp = boss.max_hp
        boss.damage += 9
    ...
    def _make_loot(self, x: float, y: float) -> Item:
        ...
        if roll < 0.455:
            return Item(
                "Scroll of Remove Curse", "remove_curse", rarity="Magic", x=x, y=y
            )
        loot_bonus = self.run_modifier.loot_bonus + self.story_effect_value(
            "loot_bonus", 0.0, 0.25
        )
        # Unique drops are deliberately scarce: a tiny base window plus a
        # dampened loot_bonus so treasure buffs nudge the odds upward without
        # flooding runs with build-defining gear. (4.10.x: Legendary is no
        # longer rolled, and the window shrank to match the 3x unique
        # power-up — a unique is now a run-defining jackpot. 5.0: halved
        # again, 0.6% -> 0.3% base, after Steam Deck playtests still saw a
        # unique most runs before floor 4.)
        if roll > 0.997 - loot_bonus * 0.25:
    ...
    @staticmethod
    def _clamp_rolled_equipment(item: Item) -> None:
        """Keep every rolled drop strictly below the weakest unique blueprint.

        Applies after affixes, cursed bargains, and relic empowerment so no
        stacked boost lets a Common/Magic/Rare (or Cursed) roll outclass the
        ...
        if item.rarity == "Unique":
            return
        if item.slot == "weapon":
            item.power = min(item.power, MAX_ROLLED_WEAPON_POWER)
        elif item.slot == "armor":
            item.defense = min(item.defense, MAX_ROLLED_ARMOR_DEFENSE)
    ...
    def _make_unique(self, x: float, y: float) -> Item:
        ...
        pool = list(UNIQUE_ITEM_DEFINITIONS)
        if int(getattr(self, "current_depth", 1)) < UNIQUE_ARMOR_MIN_DEPTH:
            # Floors 1-3 draw from the weapon uniques only: an early unique
            # armor floors most incoming hits to 1 damage (see
            # UNIQUE_ARMOR_MIN_DEPTH). Filtering the pools keeps the seeded
            # rng draw count identical to the ungated path.
            pool = [
                definition
                for definition in pool
                if definition.slot != "armor"
            ]
        archetype_pool = [
            definition
            for definition in pool
        ...
        if archetype_pool and self.rng.random() < 0.72:
            ...
            definition = self.rng.choice(pool)
