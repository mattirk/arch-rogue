...
class FriendlyNpcRuntimeMixin:
    ...
    def lossless_soul_special_room(self, npc: FriendlyNpc):
        """Return the soul hall hosting this keeper, if she stands in one."""
        dungeon = getattr(self, "dungeon", None)
        if dungeon is None:
            return None
        special = dungeon.special_room_at_point(npc.x, npc.y)
        if special is None or special.kind != "lossless_soul":
            return None
        return special

    def lossless_soul_room_resolved(self, npc: FriendlyNpc) -> bool:
        special = self.lossless_soul_special_room(npc)
        if special is None:
            return False
        return bool(special.state.get("soul_choice"))
    ...
    def update_friendly_npcs(self, dt: float) -> None:
        ...
        timing = self.friendly_npc_music_timing()
        for npc in npcs:
            if getattr(npc, "combat_active", False):
                # 4.8.10: an ally currently fighting (or walking back home)
                # is driven by ``update_npc_allies``; the beat wander resumes
                # once it stands down at its anchor.
                continue
            motion = self.friendly_npc_motion(npc)
    ...
    def _friendly_npc_home(
        ...
        anchor_key = (
            ...
            if isinstance(npc, IdleNpc) and npc.kind == "lossless_soul"
            else "npc"
        )
    ...
    def _friendly_npc_holds_for_player(
        ...
        if isinstance(npc, Shopkeeper):
            ...
            radius = 1.45
        elif isinstance(npc, IdleNpc) and npc.kind == "lossless_soul":
            # The keeper pauses her dance for an approaching player until the
            # hall's reflection has been answered (state lives on the room).
            if not self.lossless_soul_room_resolved(npc):
                radius = 1.45
        elif (
            isinstance(npc, IdleNpc)
            and npc.kind in GREETABLE_IDLE_NPC_KINDS
            and not npc.aggressive
        ):
            # 4.8.10: a greetable NPC faces an approaching player for the
            # greeting; once recruited it is a comrade and dances freely.
            radius = 1.45
        if radius <= 0.0:
            motion.holding_for_player = False
            return False
