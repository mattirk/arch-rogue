...
class MenuInventoryMixin:
    ...
    def draw_inventory_item_row(
        ...
        if self.g.legacy_graphics:
            ...
            preview = self.g.sprites.item_preview(
                item.slot,
                max(1, slot_rect.width - self.u(6)),
                item.display_name,
            )
    ...
    def draw_inventory_selected_card(self, rect: pygame.Rect) -> None:
        ...
        # The equipped panel can borrow this card: with a weapon/armor slot
        # focused (Left/Right or mouse) it shows that item's full stats.
        equipped_focus = self.g.focused_equipped_item()
        title = (
            f"Equipped {self.g.inventory_equipment_focus.capitalize()}"
            if equipped_focus is not None
            else "Selected"
        )
        self.draw_text(title, title_font, self.WARNING, title_rect)
        if equipped_focus is None and not self.g.player.inventory:
            self.draw_text(
                "No item selected",
                self.g.small_font,
                self.MUTED,
        ...
        if compact:
            ...
            self.draw_text(
                self.g.item_decision_summary(
                    item, equipped_view=equipped_focus is not None
                ),
                self.g.tiny_font,
                self.TEXT,
                pygame.Rect(
        ...
        action = (
            "Equipped"
            if equipped_focus is not None
            else self.inventory_action_label(item)
        )
        meta = f"{item.visible_rarity} {self.inventory_category_label(item)} · {action}"
        self.draw_text(
            meta,
            self.g.tiny_font,
            self.MUTED,
        ...
        if self.menu_input_hints_visible():
            extra_lines.append(
                "Enter/E back to bag · Left/Right cycle"
                if equipped_focus is not None
                else "Enter/E use · Del drop"
            )
    ...
    def draw_inventory_equipment(self, rect: pygame.Rect) -> None:
        ...
        if modern and panel_rect.height < self.u(112):
            self.g._inventory_equipment_card_rects = []
        ...
        weapon_rect = pygame.Rect(rect.x + pad, y, rect.width - pad * 2, card_h)
        self.draw_equipment_card(
            weapon_rect,
            "Weapon",
            weapon,
            "Training Sword (+0 dmg)",
        )
        card_rects.append(("weapon", weapon_rect.copy()))
        y += card_h + card_gap
        armor_rect = pygame.Rect(rect.x + pad, y, rect.width - pad * 2, card_h)
        self.draw_equipment_card(
            armor_rect,
            "Armor",
            armor,
        ...
        card_rects.append(("armor", armor_rect.copy()))
        self.g._inventory_equipment_card_rects = card_rects
    ...
    def draw_equipment_card(
        ...
        modern = bool(getattr(self, "_inventory_asset_panel", False))
        color = self.item_color(item) if item else self.MUTED
        focused = (
            item is not None
            and getattr(self.g, "inventory_equipment_focus", None)
            == label.lower()
        )
        if not modern:
        ...
        text_x = rect.x + self.u(8)
        icon_size = rect.height - self.u(8)
        if item is not None and icon_size >= self.u(18):
            preview = self.g.sprites.item_preview(
                item.slot, icon_size, item.display_name
            )
            icon_rect = pygame.Rect(text_x, rect.y, icon_size, rect.height)
            self.screen.blit(
                preview, preview.get_rect(center=icon_rect.center)
            )
            text_x = icon_rect.right + self.u(6)
        self.draw_text(
            label,
            self.g.tiny_font,
            self.MUTED,
            pygame.Rect(
                text_x,
                rect.y + self.u(4),
                max(1, rect.right - self.u(8) - text_x),
                self.g.tiny_font.get_height(),
            ),
        )
        self.draw_text(
            item.label if item else empty_text,
            self.g.tiny_font,
            color,
            pygame.Rect(
                text_x,
