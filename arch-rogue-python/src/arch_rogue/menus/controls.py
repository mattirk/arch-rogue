...
class MenuControlsMixin:
    ...
    def _gamepad_mapping_rows(self) -> list[MenuRow]:
        ...
        # Fixed rows last, with no "Map" badge so they are not selectable for
        # rebinding. Listed rather than hidden: a control nobody can find is
        # worse than one nobody can change.
        for label, binding in FIXED_GAMEPAD_BINDINGS:
            rows.append(("Fixed", label, binding))
        return rows
