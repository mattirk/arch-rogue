...
from ._utils import BOSS_FOOTPRINT_HIT_RADIUS
...
class _MobilityCombatMixin:
    def player_dash(self) -> None:
        if self.player_big_hit_charging():
            if self.player_big_hit_committed():
                # Locked in past the halfway point: nothing escapes the blow.
                return
            if not self.mp_is_joiner():
                # Dash is the Big Hit escape hatch: abort the charge at the
                # cancel price (stamina stays spent, short cooldown), then
                # dash as normal. A joiner just queues the dash below — the
                # host runs this same branch on the partner actor.
    ...
    def first_enemy_near(
        ...
        *,
        ...
        search_radius = radius + BOSS_FOOTPRINT_HIT_RADIUS - ENEMY_HIT_RADIUS
        for enemy in self.nearby_enemies(x, y, search_radius):
            if not enemy.alive or (
                excluded_ids is not None and id(enemy) in excluded_ids
            ):
                continue
