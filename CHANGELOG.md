...
- Player lantern: a warm dynamic light at `DARK_LEVEL_LIGHT_RADIUS` (dark floors) / `LIGHT_LEVEL_SIGHT_RADIUS` (light floors, adding local warmth over the ambient tint), so sight/visibility reach is identical and combat/LOS logic is untouched. Subtle low-amplitude radius/intensity flicker for a lantern feel, togglable via the `Reduce motion` accessibility option (which suppresses flicker entirely).
...
- Lit-actor shading: actors within range of a light get a Lambertian tint computed from the baked normal map on a downsampled copy, cached per (sprite, dominant-light bucket, distance bucket) so the per-pixel work only runs when the bucket changes. Skipped on the `LIGHTING_OFF` tier and when normal maps are off; hooked in `blit_sprite` (player/enemy/boss) and `draw_familiar`.
