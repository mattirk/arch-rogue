...
game has quietly kept since 4.x.

### Added

- **Chronicle of descents**: a sixth title-menu row (shortcut V) opens the
  run-history screen — the last twelve finished runs, newest first, as
  standard menu rows (outcome, archetype, depth, difficulty, kills,
  duration, date) with the selected run's full story in the parchment note:
  how it ended, the floor omen, bosses felled, notable loot. Keyboard,
  controller, mouse hover/click, and mobile taps all navigate it; the list
  windows and follows the selection on small canvases. New runs stamp an
  `ended` date into their record; older records render without one.

### Changed

- **Desktop saves moved to the SDL pref dir** (`%APPDATA%/Arch Rogue/Arch
  Rogue`, `~/.local/share/Arch Rogue/Arch Rogue`,
  `~/Library/Application Support/Arch Rogue/Arch Rogue`) under the same
  `options.json` / `run.json` names mobile already uses. The old home
  dotfiles are copied over on first launch and left behind as
  downgrade-safe backups. This is what makes Steam Auto-Cloud addressable
  on every OS (there is no Auto-Cloud root for the bare Windows home
  directory); the exact partner-site root/pattern table lives in
  docs/steam.md §6.

### Validation

- New suites: storage migration (pref-dir resolution, one-time copy,
  no-overwrite, options round-trip) and the Chronicle (title entry,
  selection clamps, scroll window, legacy records, `ended` stamping).
- `TITLE_HASH` rebaselined for the sixth title row; determinism re-verified
  across two processes.
- Runtime, Android package defaults, relay-server metadata, and download
  metadata report `5.0.1`. Save, options, and multiplayer protocol schemas
  are unchanged (the new run-record field defaults tolerantly).
- `python -m compileall src tests`
- Full non-web test suite

## 5.0.0 — Steam pre-release

The Steam pre-release milestone: the asset tree is organized by graphics
tier, standing props finally sit centered on their tiles, the Steam Deck
keeps its minimap on boss floors and gets readable discipline copy, unique
armor stops trivializing the early game, mana potions matter again, every
speed buff in the game now genuinely moves the player, and the title menu
spins its relic logo.

### Added

- **Rotating relic logo on the title menu**: the "ARCH ◆ ROGUE" lockup now
  spins its diamond relic on the main menu (desktop and web), using the same
  16 `menu.logo.diamond` frames as the loading screen and the website hero —
  composed once per spin frame and served from a new cached-surface LRU, so
  the retained-frame menu renderer repaints at just 8 Hz. Mobile keeps the
  static logo and its draw-once battery saving.
- **Discipline mana-recovery choices**: Arcane Ward (Arcanist Ward path) and
  Veil of Ash / Hex (Acolyte Veil/Curse paths) now grant real mana regen
  alongside Deep Focus, through the same `has_upgrade` seam. Committed
  casters can build back toward a sustained casting loop.
- Regression suites for prop centering (`test_prop_centering`), the 5.0 mana
  economy (`test_mana_regen`), and the real move-speed channel
  (`test_move_speed`).

### Changed

- **Assets organized by graphics tier**: HD world masters moved from the
  bare `world/` folder into `world/hd/` beside `world/modern/`; 21 orphan
  PNGs deleted (superseded guiding floors, modern-resolution duplicates, an
  unreferenced gold stack); the Rune Sentinel's authored-but-unlisted
  north-west walk frames joined the manifest; and the missing
  `items/named/*.png` package-data glob now ships all 23 named-item icons in
  wheels (previously silently excluded).
- **Unique armor waits for floor 4** (`UNIQUE_ARMOR_MIN_DEPTH`): a 15-27
  defense unique on floors 1-3 floored nearly every enemy hit to 1 damage.
  Unique weapons still drop early; every unique source (loot window, kills,
  caches, Twilight Shrine, shop stock) funnels through the same gate. The
  base unique loot window also halved (0.6% → 0.3%) — a unique is a
  run-defining jackpot, not an early-game routine.
- **Mana regen slowed for every class** (Arcanist 8→3.5/s innate, everyone
  else 5→2.5/s): idle refills now take 15-31 s instead of 8-12 s, so the
  Lesser Mana Potion is a real purchase again. Regen rates live in one
  shared helper consumed by both the local simulation and the co-op partner
  sim, so the two can never drift.
- **The speed stat is real now**: player locomotion folds the archetype
  speed rating (vs a 3.5 baseline — a Rogue genuinely outpaces an Arcanist
  by ~20%), equipment `move_speed` affixes, the eleven Rogue/Ranger
  discipline speed nodes (rescaled to ×0.25), and the Haste Shrine into one
  multiplier clamped to [-25%, +30%] — the exact window multiplayer
  prediction already budgets for, so co-op needed no new ceiling. The Haste
  Shrine's blessing is +3% move per shrine, capped at +9%, persisted and
  replicated (`shrine_move_bonus`). Character sheet, class select, and the
  co-op lobby show an honest "Move %" instead of the old inert rating, and
  the "displayed speed rating" hedge is gone from node copy.
- **Steam Deck discipline descriptions** read at the standard menu font (22
  pt, was 16 pt) in the hover hint bar — the only place node copy renders on
  the 7-inch panel.

### Fixed

- **Props sit centered on their tiles**: the 4.9.15/4.9.16 wall-contact work
  let the player press against furnishings, exposing that standing props
  rendered up to half a tile north of their floor diamond. Manifest anchors
  for the six soul-hall/bar block props, five gold stacks, shrines, and
  secret caches now mark each sprite's measured base-diamond axis, and the
  draw sites add explicit floor-contact offsets (the authored floors paint
  their diamond centers 1.5/3.0 logical px south of the projected tile
  center). The solid-furnishing collision box re-derives to the new visual
  (-0.21875 per axis, was -0.45) so feet still stop at the pedestal's
  visible base.
- **Steam Deck minimap survives boss floors**: the Deck's 1.6× card minimum
  exceeded the space under the run header, so the boss-bar give-way clamp
  suppressed the card on every boss floor — even though the centered bar
  ends ~30 px short of the card. The clamp now applies only when the boss
  cluster actually reaches under the minimap.
- **Enemies no longer orbit bar furniture**: an enemy hugging a furnishing
  box's edge could pick a clear-but-backward nav neighbor (the lookahead
  probe traded route cost for clearance) and ping-pong at the barrel
  forever; greedy pursuit could also resume straight into a box the tile
  field cannot see. The nav probe now prefers clearance only among
  cost-tied neighbors, and greedy hand-back first checks the straight
  approach against furnishing boxes.
- AGENTS.md pointed at a non-existent `src/arch_rogue/web` for the web
  build (it lives in `web/` at the repo root).

### Validation

- New coverage: prop anchor/centering invariants, Deck boss-floor minimap,
  Deck discipline hint font, unique-armor depth gate and 5.0 loot window,
  per-class regen rates and discipline regen nodes, move-speed component
  math, clamp-vs-prediction-ceiling agreement, ground-distance scaling,
  shrine cap, save/wire round-trips, and the title spin (desktop repaints,
  mobile stays static; loading-screen frame 0 identity).
- `TITLE_HASH` deliberately rebaselined for the spinning relic diamond;
  determinism re-verified across two processes. Gameplay/crowd snapshots
  unchanged.
- Runtime, Android package defaults, relay-server metadata, and download
  metadata report `5.0.0`. Save, options, and multiplayer protocol schemas
  are unchanged (the new `shrine_move_bonus` field defaults tolerantly on
  both ends).
- `python -m compileall src tests`
- Full non-web test suite

## 4.11.0 — The stutter hunt, loading screens, and Wall Facer

The last 4.x feature release before the 5.0 push. The headline is
performance on ordinary Windows machines: the packaged runtime, the Steam
build's frame-time behavior, and mid-combat sprite loading were each found
and fixed via new hitch-attribution telemetry, and the one legitimately
long pause left — floor entry — got a real loading screen under an animated
title logo.

### Added

- **Wall Facer** (`ACH_WALLFACER`): face the hidden wall 100 times across
  the whole ledger; counts mid-run via the new `lifetime_wall_touches`
  meta-progress counter and pops at the wall. Icon pair generated and
  bundled for App Admin.
- **Loading screens** (`loading.py`): new run, descent, and save-load now
  run under a repainting loading screen — the title logo with its diamond
  relic spinning (16 frames, `menu.logo.diamond.0-15`), a caption, and a
  real progress bar fed by the sprite prewarm. Input during a load is
  dropped and the window stays responsive.
- **Animated title logo**: the logo's own diamond, spun seamlessly via
  PixelLab interpolation (frame 0 is pixel-identical to the static logo).
  The website hero now uses `assets/title_logo_spin.gif`.
- **Hitch telemetry**: desktop perf monitoring (Show performance overlay)
  now attributes Steam-pump, run-save, and GC time, emits rate-limited
  `ARCH_ROGUE_HITCH` lines naming each slow frame's phases, and appends
  everything to `arch_rogue_perf.log` in the storage directory — packaged
  `--windowed` builds previously lost all telemetry.
- **Spell stat**: the character sheet shows base bolt damage (new
  `Player.spell_damage()`, shared with `player_cast_bolt` so the sheet and
  the projectile can never disagree).

### Changed

- **Packaged builds ship CPython 3.14** (was 3.11) across GitHub releases
  and Steam depots — the game is CPU-bound and the interpreter upgrade is
  a large frame-rate win on every platform.
- **Actor sprites prewarm at floor entry** and the desktop source cache
  grew from 384 entries to 2560/320 MB: a dense floor's roster no longer
  thrashes the cache, so combat frames pay zero sprite disk loads.
- **Rogue Marksman rebalance**: the path now buffs the bolt (+1/+1/+2/+2/+3
  spell power per degree, the same stat the Warden's Guard Bolt paths use)
  and grants smaller stamina reserves (22 total, was 40). Melee unchanged.
- **Character sheet class-skill cell**: every archetype shows its own
  slot-3 skill — Time Skip duration, Bell damage, Nova type and reach,
  Spirit damage, Beast damage — instead of non-Arcanists inheriting the
  "Nova" row. Nine stats lay out as a balanced 5/4 grid.
- **Music option parked** until real tracks land: the Options row and its
  M shortcut are gone and a persisted `music_enabled` is ignored on load.

### Fixed

- **Steam-build lag spikes**: the achievement funnel republished all Steam
  stats on every elite/boss kill, and each republish triggered a
  `StoreStats` client RPC. Unchanged stat values now skip the SDK entirely
  and frame-loop flushes are throttled to one per three seconds (unlock
  toasts still flush immediately).
- The legacy-graphics character sheet clipped stat rows past the second
  row instead of shrinking cells.

## 4.10.1 — Smooth fog-of-war edge on HD

On the HD desktop renderer the fog-of-war boundary is now a continuous
per-pixel gradient instead of hard tile diamonds. A **revelation field**
(`rendering/fog.py`) gives every remembered tile a visibility value from its
Euclidean distance to the nearest unexplored tile; the field lives on the
diagonal (u, v) = (x − y, x + y) lattice — where the 2:1 isometric projection
is a pure per-axis scale — and one `smoothscale` rasterizes it into the
half-resolution light buffer in place of the per-tile ambient rect fills.
The buffer's existing `BLEND_RGB_MULT` composite then feathers explored
terrain into black across the frontier. The field eases toward its targets
(~a quarter second, frame-rate independent), so an advancing frontier rolls
back smoothly instead of stepping one tile at a time, and a fresh floor's
starting disc blooms outward from the player.

Tile culling is untouched: unexplored terrain is still never drawn, so the
gradient reveals nothing the player hasn't earned — it lives inside the
outer band of already-remembered tiles. Everything derives per frame from
`revealed_tiles` behind an identity/size signature, so floor changes, save
restores (instant, no bloom above a rebuild threshold), and multiplayer
syncs need no bookkeeping; nothing new is serialized. Legacy and Modern keep
their authored instant tile-quantized reveal, dark floors keep the lantern
model, mobile keeps its cached floor layer, and the Lighting Off tier keeps
the current look (the light buffer is the fog's carrier).

### Changed

- `_stamp_ambient` routes HD light floors through the field stamp; the
  per-tile rect loop (and its elevated wall-top extension) remains the
  Modern/Legacy path. The stamp carries the field in the buffer's alpha
  channel too, preserving the "ambient coverage" convention.
- Frontier wall prisms now inherit their fog from the screen-space field, so
  the HD path bypasses the per-surface `_wall_fog_edge` gradient machinery
  (`_frame_smooth_fog`); Modern/Legacy keep it. The Frost Nova room flash is
  damped by the field so its additive rects cannot re-brighten the fog band
  with hard edges.
- Stationary frames reuse the composed fog stamp through a field-version
  cache key; movement frames recompute only the reveal delta's falloff
  neighborhood and re-scale one small lattice window.
- **Ground draw margin**: one ring of ground diamonds beyond the remembered
  frontier is drawn under field value 0 (multiplied to black — nothing
  unexplored shows), so the fog lift only uncovers geometry already on
  screen instead of a tile and its fade appearing in the same frame.
  Wall/door prisms keep the strict revealed gate and ease up from zero
  through their own lattice site — margin prisms would be hundreds of
  provably black tall blits per frame. The exception is **south-frontier
  prisms**: an occluder south of revealed ground rises up-screen into the
  lit zone, so waiting for its own reveal would pop its top in under a
  bright mask. Occluders whose prism could overlap floor-like revealed
  terrain (prism reach 4 v-units, one tile wide; rock-on-rock skipped — the
  swap is invisible there) draw early instead, surfacing into darkness
  while the overlapped frontier tile is still easing and brightening
  continuously with the field. Measured with a same-world-point temporal
  probe: the worst single-frame brightness step during continuous
  exploration fell from +95 (pop from black) to ~+34 mid-fade eastward and
  ~3 southward; worst-case walking frames cost under +1.5 ms at 2560x1440
  (stationary and dark-floor frames unchanged).
- **`visible_bounds` now centers on the smoothed camera** (desktop; mobile
  keeps its layout-pinned player center). The player-centered box left the
  trailing screen edge outside the padded radius while the camera lagged a
  walking player, so whole tile rows could pop at the screen border on every
  tier — a pre-existing artifact this feature's investigation surfaced.

### Tests

- `tests/test_smooth_fog.py`: field values, incremental-equals-full
  recompute, stamp/projection alignment, pixel-smoothness of the frontier
  row, Modern's hard rect edge, tier/mobile/dark/lighting gating, wall-fog
  bypass, floor-entry bloom, and the instant-restore threshold.
- The native-zoom fog-centering contract now runs for both Modern (exact
  rect) and HD (bilinear tent) at every zoom bucket; the ambient wall-top
  rect test pins the Modern tier it describes.

## 4.10.0 — Big Hit

Action slot 1 stops duplicating the automatic melee swing and becomes **Big
Hit**: a hold-to-charge heavy blow shared by all five archetypes. Hold slot 1
(keyboard 1, LT, a remapped pad button, or the touch button) to wind up for
0.9 s at half move speed, steering your aim the whole time; release in the
first half to cancel at a 1.5 s cooldown price, or pass the halfway commit
point and the blow locks in and fires by itself. The hit deals 3× melee
damage in the melee arc and hurls the primary target ~2 tiles backwards
through real wall collision (oversized bosses resist the throw), then sits on
a 6 s cooldown. 20 stamina, spent at press, never refunded — a whiff pays
full price. Dash stays the escape hatch: it cancels an uncommitted charge
(same 1.5 s price) and moves you out.

Each archetype adds one signature rider: **Bulwark Slam** (Warden) throws
*everyone* in the arc at full damage, **Killing Blow** (Rogue) doubles the
Precision crit chance, **Force Slam** (Arcanist) chills every hurled enemy
for 2.5 s, **Blood Reap** (Acolyte) heals a quarter of the damage dealt, and
**Pinning Strike** (Ranger) snares the primary target and inherits the
longest throw (2.2 tiles). Design doc: `build/bighit.md`.

### Added

- **Big Hit on action slot 1** with hold-to-charge input across all four
  paths: keyboard `1` (KEYUP release), the LT trigger (falling-edge release
  detection in the controller poll), remapped gamepad buttons (JOYBUTTONUP),
  and the mobile touch button (finger lift).
- **Five new 32 px HUD icons** (bulwark_slam, killing_blow, force_slam,
  blood_reap, pinning_strike); the retired melee icons left the manifest.
- **Charge telegraphs**: a growing ring underfoot that flips gold at the
  commit point (both co-op screens), a rising fill + committed border flash
  on HUD slot 1, and the Big Hit name floater on release.
- **Per-hit knockback override** (`DamageContext.knockback_speed`) so a
  single hit can hurl 2+ tiles through the existing decay/collision channel
  instead of the global ~0.16-tile nudge.
- **Chain-shove**: contact resolution ejects the mover from body overlaps,
  which dead-stopped a hurled enemy on the first packmate in its path
  (~0.06 of the 2.0 tiles, found in playtest). Above a speed threshold the
  knockback integrator now transfers velocity to bodies ahead so the pack
  plows backwards together, decaying per hop; ordinary melee nudges never
  chain, and oversized bosses stay immovable bulk.
- **Launch trail on hurled kills**: a target that dies to the throw is
  removed the same frame, which hid the flight entirely — the death now
  paints a short trail of bursts down the throw path so the hurl reads
  even when the blow kills.
- **Co-op support**: new `bighit`/`bighit_release` intents (protocol
  version 2), the Big Hit cooldown as the 6th snapshot timer element
  (backward-tolerant), instant windup pose preview for the joiner, and the
  charge status replicating through the ordinary status sync with joiner-side
  smoothing between snapshots.

### Changed

- **Slot 1 no longer casts a manual melee swing** — walking into enemies
  still auto-swings exactly as before, and the mouse click swing is
  untouched. The per-class melee names (Shield Bash, Backstab, ...) retired
  from the HUD; the auto-attack is nameless.
- **Normal melee base damage trimmed 12 → 10** so Big Hit reads as the
  payoff button; level, weapon, and bonus scaling are untouched.
- **While charging**: auto-melee is suppressed, bolt and the class skill are
  blocked (potions stay allowed), and move speed is halved on the host, the
  partner simulation, and the joiner prediction alike.
- Multiplayer protocol version 1 → 2 (older servers/clients refuse the pair
  up front instead of erroring mid-run).

## 4.9.25 — Fixed logical canvas for windowed desktop

Windowed desktop previously rendered at whatever size the window was dragged
to while the automatic UI scale guessed the host's DPI (Xft.dpi on X11,
GetDpiForWindow on Windows, CoreGraphics on macOS, toolkit env fallbacks).
Two independent inputs meant the same build laid out differently across
machines, and windowed and fullscreen disagreed on the same machine. Every
desktop mode now renders the same fixed logical canvas fullscreen already
used — 2560×1440 off-Deck, the 1280×800 panel on Deck — and SDL's `SCALED`
renderer fits it to the real window or monitor, so UI layout is authored
against one known resolution everywhere.

### Added

- **Desktop "Render resolution" options row (hotkey R): Native · 1440p,
  Balanced · 720p, Performance · 540p.** With the canvas now fixed, low-end
  hardware needs a way to pay for fewer pixels — the game software-renders
  the whole logical canvas every frame, so the 720p tier costs a quarter of
  native's pixels and 540p a seventh, while SDL still fills the window or
  monitor. The tiers reuse the mobile height-cap machinery against the 16:9
  canvas, so 720p is an exact half canvas and the canvas-derived automatic
  UI scale (2×→1×) keeps layout proportions pixel-identical; 540p matches
  the mobile Performance tier's slightly larger UI. Persisted per player,
  applies to windowed and fullscreen alike. Hidden on mobile (its own
  render-quality row already caps the physical surface) and on the Steam
  Deck (panel-native is already the cheapest configuration).

### Changed

- **Windowed desktop renders the fixed 2560×1440 canvas.** The window is
  created `RESIZABLE | SCALED`: dragging it rescales the canvas on the GPU
  with aspect ratio preserved and pygame remapping mouse coordinates, and
  the resize event no longer recreates the display mode or reflows the UI.
  Windowed rendering therefore costs the same as fullscreen regardless of
  window size — the canvas is the 4.9.24-benchmarked performance envelope.
- **Automatic UI scale derives from the canvas, not host DPI.** The same
  `min(width/1280, height/720)` rule mobile already used now covers desktop
  too: deterministically 2× on the 2560×1440 canvas, 1× on the Deck panel.
  The manual 1–4× override in Options is unchanged, and the
  `ARCH_ROGUE_DISPLAY_SCALE` environment variable remains as an explicit
  override for players who want a denser or larger UI.

### Removed

- **Host-DPI probing deleted.** The Xft.dpi/X11 resource parsing, Windows
  `GetDpiForWindow`, macOS CoreGraphics backing-scale query, and
  `GDK_SCALE`/`QT_SCALE_FACTOR` fallbacks are gone — with a fixed canvas the
  monitor's DPI can no longer influence layout, which was the "automatic
  scaling is flaky" report in the first place.

## 4.9.24 — Desktop render optimization pass

Interleaved A/B on the same host and thermal window, 53-enemy dark-floor
crowd at the 2560×1440 desktop canvas, HD tier, default view: median
update+draw fell from 36.3 ms to 24.9 ms (**−31 % frame time, +45 % FPS
ceiling**). Raw captures and methodology in
`build/performance/desktop_optimization_4924.md`. This targets the Windows
"performance is pretty bad" report: fullscreen desktop always renders the
fixed 2560×1440 canvas, so a 4K monitor pays the same CPU cost as 1440p —
the Steam Deck was fine because it renders its native 1280×800 panel.

### Changed

- **Sprite caches stopped thrashing on desktop.** The decoded-source (48)
  and normalized-frame (320) LRU caps were sized for Android and are smaller
  than a depth-10 desktop crowd's working set (~350+ resolution keys), so
  steady combat frames were re-decoding PNGs from disk and re-normalizing
  frames every frame — the kind of cost that is much worse on Windows (file
  I/O + antivirus scanning) and exactly matches "bad on Windows, fine on
  Deck". Desktop now uses byte-budgeted caps (384 sources / 192 MiB, 4096
  frames / 128 MiB, 4096 resolved keys); in the crowd profile every source
  now loads exactly once. Mobile keeps the historical limits unchanged.
- **A 0.8 world render bucket, and the HD default zoom sits exactly on it.**
  Every zoom below 1.0 previously rendered the whole world into a layer
  larger than the display (1.5× the pixels at the HD default 0.815, 2.37× at
  the widest 0.65) and then paid a full-frame composite scale. The bucket
  set is now (0.8, 1.0, 1.2, 1.4, 1.6) — WORLD_SCALE 5 maps them to integer
  4/5/6/7/8 px units — and the HD fresh-session default zoom is 0.8 instead
  of 0.815: zero residual, so the world draws straight into the display with
  no oversized layer and no composite pass, and 512 px masters land at an
  exact 2:1 texel ratio (which also removes fractional-zoom resample shimmer
  during camera motion). The view is ~2 % wider than 4.9.23's default; zoom
  behavior at every other level is unchanged except for being cheaper: the
  widest view now over-renders 1.51× instead of 2.37×.
- **Desktop lighting upscales nearest-neighbour.** The half-resolution light
  buffer was smoothscaled to the display every frame (~2.3 ms at 1440p);
  nearest costs half that, and 2× pixel duplication of an already-smooth
  radial gradient under a BLEND_RGB_MULT composite is not visually
  distinguishable (mobile has shipped nearest from quarter resolution since
  4.3.10). Same-scene A/B captures differ in 1.4 % of channel bytes,
  concentrated on halo edges.
- **Combat screen flash is two blend fills** (multiply by 255−α, add
  color·α — the exact alpha-blend arithmetic) instead of filling and
  alpha-blitting a full-screen SRCALPHA overlay every flash frame.
- **Per-frame scan overhead trimmed.** The floor and wall/door passes walk
  the whole view box (~625 tiles each) every frame and called
  `tile_visibility_alpha` → `is_current_floor_dark` per tile (~1,450
  calls/frame); both scans now hoist the visibility predicate into locals
  (`_visibility_scan_params`). The dark-floor minimap patch memoizes the
  per-tile alpha for the duration of its draw (each tile was probed several
  times via neighbor checks). HUD resource bars cache their gradient fill +
  shine composite and their label text surfaces instead of ~10 draw.rects,
  an SRCALPHA allocation, and two Font.renders per bar per frame.

### Notes

- The 60-enemy spatial-index crossover was re-validated on bare metal at 53
  enemies: the historical full scan is still faster below 60 (p50 1.86 ms vs
  1.91 ms with the index forced), so the threshold stands.
- Desktop **windowed** mode renders at the actual window size with an
  unaccelerated present: a maximized 3840×2160 window costs 2.25× the
  fullscreen canvas. When benchmarking on a 4K monitor, compare fullscreen
  (fixed 2560×1440 canvas) against windowed separately.
- Comparison captures for human review (old vs new default view, smooth vs
  nearest lighting) are in `build/performance/desktop-4924-captures/`.

## 4.9.23 — Loot rarity pass, HD default zoom

### Changed

- **Loot is rarer across the board** (second pass; the first was 4.2). The
  on-kill drop roll goes 0.36 → 0.28, the per-room exploration loot
  multiplier 0.42 → 0.33, secret caches spawn at base 0.12 (cap 0.38) instead
  of 0.16 (cap 0.48), and the per-floor Cartographer's Stash chance drops to
  base 0.36 clamped to [0.08, 0.60]. Cache and shrine *contents*, vendor
  stock, gold, and the legendary/unique rarity windows are unchanged.
- **HD graphics start two zoom notches in.** Fresh sessions on the HD tier
  default the viewport to 0.65 × 1.12² ≈ 0.82 instead of the widest 0.65;
  Modern/Legacy keep the wide default and mobile still starts at native
  scale. Zoom remains freely adjustable and is never persisted.

## 4.9.22 — Steam Deck controller fixes

### Fixed

- **Steam Deck controls were misconfigured when the game was launched through
  Steam.** The button indices a pad reports depend on how it is reached, not on
  the hardware. Launched natively a Deck reports its own evdev layout (D-pad on
  buttons 11–14, R1 on 9, quick-access on 16); launched through Steam, Steam
  Input hides that device and substitutes a virtual Xbox-style pad where 9 is
  right-stick-click and the D-pad is a hat. The game chose its mapping from
  `is_steam_deck()` — a DMI hardware probe — so through Steam it decoded an Xbox
  pad with a Deck keymap, and right-stick-click drank a potion.
  `uses_deck_native_layout()` now decides from the connected device, and the
  three Deck-only indices are suppressed on a virtual pad.
- **Trigger remapping in the Controls menu never worked, on any controller.**
  The capture code sat in `Game.update()` behind a `state == "controls"` check,
  but `update()` only runs while playing, so that branch was unreachable — and
  the menu's own axis poll cleared the queued trigger slots it would have read.
  Reported on a Stadia pad and reproduced on the Deck, which is what identified
  it as device-independent rather than a controller quirk. Capture now happens
  in the per-frame path the menu actually runs; triggers are axes, so there is
  no JOYBUTTONDOWN edge to hang it off the way button capture does.
  That per-frame path is now a method (`poll_menu_axes`) rather than inline loop
  code, so the test drives what the loop drives — a test that reimplemented the
  loop body would have passed against the unreachable version too.
- **The D-pad did nothing during gameplay.** It dispatched bare directional
  commands, which move a cursor in menus but have no meaning in the dungeon
  where the sticks carry movement and aim. Up and Down now zoom the minimap.
  Both D-pad paths — buttons 11–14 natively, a hat through Steam Input — resolve
  through one helper, so the same physical D-pad cannot behave differently
  depending on how the game was launched.

### Added

- **`ARCH_ROGUE_LOG_JOYSTICK=1`** prints the controller name, GUID,
  button/hat/axis counts and every raw button index pressed. The index layout is
  invisible from the outside and differs per launch path, which is what made the
  above hard to diagnose.

### Changed

- Minimap zoom is a `MAP_ZOOM_IN`/`MAP_ZOOM_OUT` command rather than a direct
  `adjust_minimap_zoom()` call, and is no longer gated on `is_steam_deck()` —
  that probe is wrong the moment Steam substitutes a virtual pad.

- **Boss achievements never matched the boss that was actually killed.** The
  nameplate a boss dies under is not its definition name: floor bosses spawn
  with the story antagonist prefixed ("the Toll-Keeper Mycelial Matron") — and a
  story is active on every run — while the final boss is renamed outright by
  theme ("Voidbound Gate Tyrant") or learned true name ("Sorn Voss, …"). The
  ledger recorded nameplates verbatim; the achievement triggers do exact set
  membership against definition names, so the Matron achievement was silently
  unearnable and the Tyrant's would essentially never have fired. Found on
  hardware: the kill registered, the death folded it in, nothing popped.
  `canonical_boss_name()` (content/enemies.py) now maps nameplates back to
  definition names at both fold-in sites, and `normalize_meta_progress`
  canonicalises ledgers written before the fix — so a profile carrying the
  decorated entry retroactively earns the achievement at the next startup
  evaluation. The catalogue consistency test had checked catalogue-vs-definition
  names, which was the wrong invariant; the new tests pin nameplate-vs-ledger.
- **Milestone achievements now pop when earned, not at the eventual run end.**
  Found on the first hardware test with a real SDK: reaching depth 3 and
  killing the Mycelial Matron produced nothing, because evaluation only ran at
  death/victory and at startup — depth and boss progress folded into
  meta-progression only when the run ended. `record_midrun_achievement_progress`
  now runs on every descent and on boss/miniboss/elite kills, updating
  `best_depth` and the boss ledger immediately. Outcome-dependent triggers
  cannot fire early: the mid-run outcome is empty, and the co-op and
  bar-pilgrimage flags are computed only for a finished run — evaluated mid-run
  they would be true prematurely (toasting the first bar gives 1/1 toasted while
  deeper floors may still roll more). Disk writes happen only when progress
  actually changed, so ordinary kills stay off the save path.
- **Steamworks SDK delivery hardened.** CI fetches `libsteam_api.so` from a
  private repo (`STEAM_SDK_REPO`/`STEAM_SDK_TOKEN` in the `steam` environment) —
  the library is under Valve's terms and stays out of this public repo. The
  fetch now finds `redistributable_bin/linux64/libsteam_api.so` at any depth in
  the checkout, so committing the SDK zip's `sdk/` folder wholesale works;
  half a configuration (one secret of the two) fails immediately with a plain
  message instead of three steps later inside `actions/checkout` with a git
  auth error naming neither; and the staged file's ELF magic is verified, so a
  wrong or corrupt file fails the build rather than shipping a depot whose
  achievements silently do nothing.

### Notes

- A larger Steam Input (ISteamInput action-based) layer was built and then
  removed before release. It was dormant without the bundled SDK, unproven on
  hardware, and ran as a second dispatch path beside SDL; the input layer needs
  device normalisation (SDL's GameController API) before a second source is
  worth adding. Inventory and character briefly moved to D-pad left/right in the
  same work and were reverted: those overlays switch the input context to a
  menu, where the D-pad is navigation, so the direction that opened one could
  not close it without a special case fighting the rest of the model.

## 4.9.21 — Achievements

Release 4.9.21 delivers Phase 3 of the Steam release plan and fixes a
meta-progression bug it uncovered. The game is registered on Steamworks as App
ID `5031380`, and the achievement set is now authored, wired, and covered by
tests. What remains for Phase 3 is manual App Admin work: entering the 38
achievements with matching API names, and drawing the icon pairs.

### Added

- **Achievement catalogue** (`src/arch_rogue/assets/achievements.json`): 38
  achievements mined from systems the game already tracks — depth milestones,
  first clear and ten clears, clears on Hard and Hell, a clear with each of the
  five archetypes and one for all five, each of the five floor bosses and one
  for the complete set, the three Gate answers and one for all three, every
  dungeon theme, every run modifier, lifetime kill/secret/shrine/loot counters,
  co-op, and a handful of flavour ones (dying on depth 1, clearing without
  drinking a potion, completing the bar pilgrimage, ten elites in a run, three
  challenge rooms in a run). Ids, names, descriptions and hidden flags are
  mirrored into Steamworks App Admin by hand, so the file is the contract.
- **Achievement funnel** (`src/arch_rogue/achievements.py`): gameplay code never
  unlocks anything directly. It records what happened into `meta_progress` and
  calls one run-end entry point; this module decides which achievements the
  resulting facts satisfy. Triggers in the catalogue are declarative — a fact
  name plus `at_least` / `at_most` / `contains` / `contains_all` /
  `size_at_least`, composable with `all_of` — and are evaluated against a flat
  fact table built from lifetime progress plus the run that just ended.
  Conditions that need real logic (the bar pilgrimage, co-op) are resolved in
  `build_achievement_run_facts` and handed over as flags, keeping the grammar
  small. A malformed catalogue raises at load in tests but degrades to "no
  achievements" at runtime rather than taking the game down.
- **Retroactive granting**: `Game.__init__` evaluates the catalogue once at
  startup, so a player who already has the clears and the boss ledger gets those
  achievements on their first Steam launch instead of after one more run.
- **Steam stats**: runs started, clears, best depth and lifetime kills are
  published on every evaluation, which is what progress-bar achievements and the
  store page's aggregate stats read from.
- **`tools/dump_achievements.py`**: renders the catalogue for Steamworks App
  Admin data entry, which has no bulk import. `--format table` (default) for
  reading beside the browser, `tsv` for a sheet, `vdf` for Valve's bulk-edit
  shape, `json` for tooling.
- **Steam Input support.** The game now declares *actions* and lets players bind
  them in their own Steam layout, instead of reading raw SDL button indices.
  - `src/arch_rogue/steam_input.py` — ctypes facade over ISteamInput: init with
    caller-owned `RunFrame`, connected controllers, action sets, digital and
    analog action handles and data, and `ShowBindingPanel`. Two details that
    fail silently if got wrong are handled explicitly: the action-data calls
    return structs **by value** (so `restype` must be the Structure, not the
    default `c_int` that would read the first four bytes as an integer), and
    `Init(bExplicitlyCallRunFrame=True)` puts the pump in the game loop so
    action changes land at a predictable point in the frame.
  - `src/arch_rogue/assets/steam/game_actions_5031380.vdf` — the in-game actions
    file: three action sets (Dungeon, Menu, Cutscene) with digital and analog
    actions. Shipped in the depot under `controller_config/` by the build script.
  - `src/arch_rogue/steam_input_actions.py` — maps action names to the game's
    `Command` constants and converts Steam Input's *held* state into the
    *edge-triggered* commands the game dispatches, so holding a button does not
    re-open the inventory every frame. A button held across an action-set switch
    is not counted as a fresh press.
  - Requires the bundled `libsteam_api.so`, so it is inactive on GitHub, itch,
    Android and web builds, and on Steam builds until the SDK secret is set.
    The SDL path is unchanged and unconditional.
- **Fixed: Steam Deck controls were misconfigured when launched through Steam.**
  The button indices a pad reports depend on how it is reached, not on the
  hardware. Natively a Deck reports its own evdev layout (D-pad on buttons 11–14,
  R1 on 9, quick-access on 16); through Steam, Steam Input hides that device and
  substitutes a virtual Xbox-style pad where 9 is right-stick-click and the D-pad
  is a hat. The game selected its mapping from `is_steam_deck()` — a DMI hardware
  probe — so through Steam it decoded an Xbox pad with a Deck keymap, and
  right-stick-click drank a potion. `uses_deck_native_layout()` now decides from
  the connected device's name, and the three Deck-only indices are suppressed at
  resolve time on a virtual pad rather than rewritten in the saved profile, since
  one install can move between the two launch paths.
- **`ARCH_ROGUE_LOG_JOYSTICK=1`** prints the controller name, GUID, button/hat/
  axis counts and every raw button index pressed. The index layout is invisible
  from the outside and differs per launch path, which is what made the above
  hard to see.
- **Linux Steam depot pipeline** (Phase 4, Deck-first). The Steam Deck runs the
  native Linux build rather than Proton, so the Linux depot alone is enough to
  put a build in front of a Deck through Steam.
  - Depot IDs are deployment configuration rather than source — they belong to
    the Steamworks app, not the game — so they come from the `steam` deployment
    environment (`STEAM_LINUX_DEPOT_ID`) and appear nowhere in the repository.
    The App ID is the exception and stays in `steam.py`, because the shipped
    binary needs it at runtime for `RestartAppIfNecessary` and init.
  - `tools/build_steam_linux.sh` builds the depot payload: a PyInstaller
    **one-dir** bundle (Steam's delta patching ships changed files, so one-file
    would make every update a full re-download of one opaque blob), with
    `libsteam_api.so` staged beside the executable, and renders the steamcmd
    build scripts from `tools/steam/*.vdf.template`. It runs without any Steam
    credentials, so the payload can be copied to a Deck and run directly today.
  - `.github/workflows/steam-deploy.yml` publishes to a beta branch on manual
    dispatch, with a `dry_run` toggle. It builds on **ubuntu-22.04**, not
    `ubuntu-latest`: a binary is only as portable as the glibc it links against
    and SteamOS 3 ships an older one than 24.04 provides, so a 24.04 build can
    refuse to start on the Deck. Both the script and the workflow refuse to
    target the `default` branch; promotion stays a manual App Admin click.
  - Depot safety checks: CI fails if `libsteam_api.so` is missing from the
    payload or if `steam_appid.txt` is present, the latter also excluded in the
    depot vdf. That file bypasses the restart-through-Steam check and must never
    ship.
- **Depot smoke test** (`ARCH_ROGUE_SMOKE_TEST=1`): boots the game, draws five
  real frames and exits 0. A packaged build that cannot reach its title screen —
  missing asset, bad hidden import, wrong glibc — now fails in CI rather than in
  a player's hands, and this is the only check that exercises the frozen bundle
  end to end. An environment variable rather than a flag, because the argv that
  reaches this binary belongs to the player's Steam launch options.
- **New meta-progression fields**: `coop_clears`, `lifetime_kills`,
  `lifetime_secrets`, `lifetime_shrines`, `clears_by_difficulty`,
  `clears_by_archetype`, and the locally granted `achievements` list. Difficulty
  and archetype previously survived only in the rolling 12-entry `run_history`,
  so a player's thirteenth run erased the evidence of what they had cleared.
  `normalize_meta_progress` upgrades older options files in place.

### Fixed

- **Co-op clears were invisible to the joining client's meta-progression.** The
  host owns a shared run's ending, so the joiner never reached `finalize_run` —
  `mp_record_local_result` appended a `run_history` entry and touched no
  `meta_progress` at all. A co-op clear therefore counted toward nothing on the
  joiner: not the clear count, not best depth, not the boss ledger. Both paths
  now funnel through `RunFlowMixin.record_run_meta`, which takes the archetype
  from the caller because on the joining client the local player is not
  `self.player`. This was a meta-progression bug in its own right, predating any
  Steam work.
- **Headless runs wrote to the developer's real profile.** A headless `Game`
  already refused to *read* home-directory preferences, but still defaulted
  `options_path` and `save_path` there, so every test that finalised a run
  overwrote the developer's own `meta_progress` and run save with whatever that
  test had set up. Headless now resolves storage to a throwaway per-process
  directory (`mobile.headless_storage_directory`), removed at interpreter exit;
  explicit paths still win, and persistence across instances still works.
- **`tests/test_player_wall_contact.py` was flaky** — around 40 subtest failures
  per 3 isolated runs, from three independent causes, all in the test rather
  than the renderer. It drove `draw_world_objects` without pinning the
  render-scale bucket that `_render_world_target` normally pins, so the draw
  used whichever bucket the previous render left active. It read the
  `_last_sprite_blit` scratch register after the whole frame had finished, when
  the wall-ghost pass that runs after the actor loop had already overwritten it
  with a rotated sprite positioned elsewhere — the source comment says to read
  it immediately after `draw_player`, which the test now does. And it left the
  randomly generated dungeon's revealed tiles and special-room props in the
  probe scene, so a bar barrel could land on the player and cover 84% of the
  body. Now 0 failures in 40 isolated runs and 3 consecutive full-suite runs.

### Changed

- `STEAM_APP_ID` is set to the real `5031380`. Setting it does not by itself
  enable anything — without the bundled `steam_api` library and a running client
  the facade still reports unavailable, so GitHub, itch, Android and web builds
  are unaffected. `ARCH_ROGUE_STEAM_APPID` still overrides it and
  `ARCH_ROGUE_NO_STEAM=1` still force-disables the integration.
- The hand-wired first-clear unlock in `finalize_run` is gone; that achievement
  now comes from the catalogue like every other one.

### Tests

- `tests/test_achievements.py` (46 tests): catalogue integrity, trigger
  evaluation, both run-end paths, and the meta-progress schema. Two of these
  exist specifically to catch silent drift — every trigger must name a fact the
  funnel actually builds, and every boss, archetype, theme, difficulty and
  modifier literal in the catalogue must still match the content definitions, so
  renaming a boss fails the build instead of orphaning an achievement nobody can
  earn. The co-op joiner path is covered explicitly so the fix above cannot
  regress.
- `tests/test_save_and_metadata.py` gains `HeadlessStorageIsolationTests`, which
  asserts a headless `Game` resolves neither its options nor its run save inside
  the real home directory.
- `ScriptPermissionTests` asserts every committed `*.sh` is mode `100755` in
  git's index. This repository sets `core.filemode = false`, so git ignores the
  working tree's executable bit: `chmod +x` records nothing and a script
  committed `100644` shows as no change at all. The first symptom is a CI job
  exiting 126 with "Permission denied".
- `tests/test_steam_integration.py` now neutralises an ambient
  `ARCH_ROGUE_NO_STEAM` for the whole module. That variable force-disables the
  facade, so having it set — exported in a shell, or on a CI job that assumed it
  was a safety measure — turned 13 of these tests red with "disabled by
  ARCH_ROGUE_NO_STEAM" instead of the behaviour under test. Nothing here reaches
  a real Steam client regardless, since every connected test injects a fake
  library, and the one test asserting the variable *does* disable the facade
  sets it itself.

## 4.9.20 — Steam Deck readiness

Release 4.9.20 delivers Phase 7a and the hardware-tested input/UI portion of
Phase 7b of the Steam release plan. The Steam Deck ships SteamOS 3 on a
1280x800 16:10 panel and fires the same SDL `APP_WILLENTERBACKGROUND` /
`APP_DIDENTERFOREGROUND` events as Android when the player suspends, but the
game previously treated it as a generic Linux desktop. The native Linux build
now uses the panel correctly and has first-class built-in controls, menus,
minimap behavior, and touchscreen interaction.

### Added

- **Steam Deck detection** (`arch_rogue.steam_deck`): a tiny stdlib-only
  helper that reads Valve's DMI product identifiers
  (`/sys/devices/virtual/dmi/id/board_vendor` == `Valve`, `product_name` in
  {`Jupiter`, `Galileo`}) to identify the LCD and OLED Deck revisions.
  Off-Linux the probe returns `False` so Windows, macOS, Emscripten, and
  Android stay on their existing paths. An `ARCH_ROGUE_STEAM_DECK=1/0`
  environment override wins over the DMI probe for development and CI.
  Results are cached per process.
- **Deck-aware desktop fullscreen** (`OptionsMixin.apply_display_mode`): on
  the Deck, fullscreen now requests the panel's own resolution (1280x800) at
  `FULLSCREEN | SCALED` instead of the fixed 2560x1440 canvas. This
  eliminates the wasteful up-then-down scale pass and fills the 16:10 panel
  edge-to-edge without letterbox bars. Off-Deck behaviour is unchanged.
  Invalid display info falls back to the fixed canvas.
- **Deck default graphics tier**: fresh installs on the Deck start on HD,
  matching desktop. Saved Modern or Legacy preferences remain authoritative
  across restarts.
- **Deck suspend/resume** (`Game.handle_deck_lifecycle_event`): the Deck's
  power button now saves the active run, suspends the audio mixer, and flips
  a `deck_suspended` flag that pauses the frame pump and `update()`. Resuming
  clears the flag and resumes audio. `APP_TERMINATING` performs a final save.
  This mirrors the Android lifecycle handler but skips the mobile-specific
  touch/GLES/safe-inset work the Deck does not need.
- **Deck control profile**: D-pad directions are reserved for navigation and
  cannot be rebound; Up/Down zoom the minimap during unobstructed gameplay and
  navigate normally in menus/cutscenes. Deck R1/L1 rotate Character/Inventory
  tabs, R1 drinks a mana potion in gameplay, LT uses action skill 1, RT dashes,
  and the mapped Character action toggles its HUD both open and closed. Back
  closes overlays and returns through menus without leaking into Exit.
- **Deck menu presentation**: fullscreen and UI-scale rows are hidden because
  the native panel mode is fixed; Options accepts only vertical D-pad
  navigation. The Deck minimap renders at 1.6× desktop size and remains
  independently zoomable.
- **Deck touchscreen**: touch-to-move is suppressed, single-finger taps can open
  the memory-token panel and select disciplines, and two-finger pinch adjusts
  viewport zoom. Tap activation occurs on release so the first finger of a
  pinch cannot accidentally activate UI.
- **Controller isolation**: Deck-only raw bumper/HUD aliases are layered onto
  the generic SDL mapping only on Deck, survive options persistence, and no
  longer alter Stadia/Xbox-style profiles. Hat-only external controllers retain
  D-pad navigation.

### Validation

- `tests/test_steam_deck.py` — 40 tests covering DMI detection, HD defaults
  and saved-tier precedence, native 1280x800 display mode, one-time controller
  profile migration, release-based touchscreen taps, pinch classification,
  and suspend/resume lifecycle behavior.
- `tests/test_input_and_accessibility.py` covers Deck-scoped mappings,
  persistence, D-pad button/hat deduplication, hat-only external controllers,
  menu navigation, and gameplay minimap zoom.
- `python -m compileall src tests` clean.
- `python -m unittest discover tests` — 1122 tests pass.
- `python -m unittest tests.test_mainline_regression` — 10 tests pass.
- Runtime, server, Android package, and download metadata report `4.9.20`.
  Save and multiplayer protocol schemas are unchanged; options remain schema
  10 with an additive Deck controller-profile migration marker.

### Next

The native Linux build and built-in control/UI path are validated on a
Galileo/OLED Steam Deck. Remaining Deck release checks are sustained HD-tier
performance, power-button suspend/resume, and room-code entry through the
Steam on-screen keyboard. Achievement unlocks remain inert until the numeric
App ID is filled in (`STEAM_APP_ID` in `steam.py`).

## 4.9.19 — Through the opened gate

Release 4.9.19 keeps the player visible throughout traversal of every opened
doorway by extending the existing wall shine-through effect to the doorway's
same-depth painter seam.

### Fixed

- Visible open doors now retain their tile coordinates alongside the ordinary
  occluder rectangle. When a player occupies that exact `OPEN_DOOR` tile and
  its sprite is overlapped by a door that paints later, the existing
  shine-through weight activates immediately instead of being rejected by the
  wall-edge anti-flicker depth threshold.
- Uses the persisted `OPEN_DOOR` tile state as the "opened before" gate, so the
  behavior survives saves, multiplayer floor sync, reconnects, and temporary
  boss seals without adding another state field or protocol change.
- Limits the override to player traversal. Closed doors retain their existing
  ordinary occlusion behavior; walls keep their fade/graze suppression;
  enemies and familiars remain on the generic path; and players screen-south
  of the door receive no redundant glow because they already paint above it.
- Runtime, Android package defaults, relay-server metadata, and download
  metadata now report `4.9.19`. Save, options, and multiplayer protocol schemas
  are unchanged.

### Validation

- Added full doorway traversal coverage for both render faces, both movement
  directions, successive walk frames across the covered half-tile, and Legacy,
  Modern, and HD graphics. The test opens each door through the real dungeon
  state transition and verifies the naturally visible screen-south exit is
  not ghosted.
- Retained wall approach, graze, fade, familiar/enemy, painter-order,
  collision-contact, graphics-tier, and multiplayer visibility coverage.
- `python -m compileall src tests`
- Full non-web test suite (1,026 tests)

## 4.9.18 — Native HD door silhouettes

Release 4.9.18 removes the remaining enlarged sawtooth geometry from all
closed/open, left/right HD door silhouettes.

### Fixed

- Replaced the doors' inherited 64px alpha staircase—which held each diagonal
  edge for eight rows before jumping 16 pixels—with the same native 512px
  structural contour already used by corrected HD walls.
- Preserved the 512px canvas, `(64, 56, 392, 424)` visible bounds, source
  anchor, reference width, direction mapping, open/closed artwork, tinting, and
  all interior pixels. Only the exterior wedges needed to form the smooth
  contour and 16 edge-fringe pixels across the four masters are changed.
- Reapplied both wall-style bright- and dark-edge cleanup after remasking, so
  the sawtooth repair does not restore the black fringe fixed in 4.9.17.
  Modern and Legacy assets remain byte-identical.
- Added a dedicated door profile to the deterministic silhouette-refinement
  tool. Repeat runs are no-ops.
- Runtime, Android package defaults, relay-server metadata, and download
  metadata now report `4.9.18`. Save, options, and multiplayer protocol schemas
  are unchanged.

### Validation

- Extended asset QA across all four HD door masters and all 18 manifest
  mappings: upper/lower and left/right diagonal plateaus are at most one row,
  the alpha mask exactly matches the canonical wall contour, and exterior
  near-black pixels remain zero.
- Retained regression coverage for all nine HD wall contours, Modern asset
  hashes, graphics-tier routing, door anchors, floor underlays, fog alpha, and
  player visibility behind closed and open doors.
- `python -m compileall src tests`
- Full non-web test suite (1,025 tests)

## 4.9.17 — Clean HD door contours

Release 4.9.17 removes the opaque black fringe from all closed/open,
left/right HD door silhouettes while preserving their authored shapes and
placement.

### Fixed

- Applied the same exterior-connected contour cleanup used by HD walls to the
  four active 512px HD door masters. The correction changes RGB only within
  the first eight opaque pixels of the transparent boundary; the doorway's
  intentional dark interior remains unchanged.
- Preserved every door's dimensions, binary alpha mask, bounding box, anchor,
  direction mapping, open/closed distinction, and tint behavior. Modern and
  Legacy assets are untouched.
- Extended the contour maintenance tool to cover both HD walls and doors.
  Already-clean wall files remain byte-identical on repeat runs.
- Runtime, Android package defaults, relay-server metadata, and download
  metadata now report `4.9.17`. Save, options, and multiplayer protocol schemas
  are unchanged.

### Validation

- Added asset QA for all 18 HD door manifest mappings and their four unique
  masters, including zero near-black exterior pixels and frozen alpha/geometry.
- Added runtime guards for unchanged door anchors and player ghost visibility
  behind both closed and open foreground doors.
- `python -m compileall src tests`
- Full non-web test suite (1,025 tests)

## 4.9.16 — Clear footing at the wall edge

Release 4.9.16 completes the north-wall contact correction by keeping the
player visible while moving along exposed isometric wall faces and corners.

### Fixed

- Painter ordering now moves the player only just past the exact wall cells
  touched by the shallow north-wall relief strip. Wall edges therefore paint
  before the player instead of swallowing the body at wall-tile seams.
- Reused the collision system's exact exposed-wall geometry for the render
  decision and applies no blanket feet-depth bias. Open ground, opposite wall
  faces, doors, other actors, ordinary objects, and genuine foreground-wall
  occlusion retain their prior ordering wherever no correction is required.
- Runtime, Android package defaults, relay-server metadata, and download
  metadata now report `4.9.16`. Save, options, and multiplayer protocol schemas
  are unchanged.

### Validation

- Added opaque-pixel visibility coverage for both north-wall orientations,
  their neighboring tile seams, the shared diagonal corner, multiple movement
  frames, all graphics tiers, and representative viewport zooms.
- Retained focused coverage for eight-direction collision contact, wall-ghost
  fade/graze behavior, dark-floor hiding, painter buckets, fog-wall opacity,
  and deterministic gameplay rendering.
- `python -m compileall src tests`
- Full non-web test suite (1,023 tests)

## 4.9.15 — Boots against the northern stone

Release 4.9.15 aligns player movement with the visible isometric ground
contact. Approaching walls toward screen north—most noticeably northeast and
northwest—no longer stops across an unnecessary strip of floor.

### Fixed

- Derived the actor ground-depth offset from the shared feet anchor and
  isometric tile height instead of adding a tier- or viewport-specific visual
  adjustment.
- Player movement may now use the shallow exposed strip on a wall's `+x`,
  `+y`, and shared northern corner faces. Relief is capped by the existing
  movement radius, so the logical player point reaches but never enters a wall
  tile.
- Kept the correction specific to player contact with `Tile.WALL`. Closed
  doors, stairs, authored furnishing boxes, map bounds, enemies, screen-south
  clearance, and generic collision probes retain their established behavior.
- Runtime, Android package defaults, relay-server metadata, and download
  metadata now report `4.9.15`. Save, options, and multiplayer protocol schemas
  are unchanged.

### Validation

- Added eight-direction contact coverage for north-facing approach, unchanged
  opposite-side clearance, closed doors, diagonal room corners, logical
  wall-impermeability, and one-tile corridors.
- Verified identical contact endpoints across Legacy, Modern, and HD at
  representative viewport zooms.
- `python -m compileall src tests`
- Full non-web test suite (1,020 tests)

## 4.9.14 — Three ages of the dungeon

Release 4.9.14 makes the new high-resolution world merge-ready without
discarding either established renderer. Graphics now have three explicit,
independent tiers: procedural Legacy, the original lower-resolution authored
world as Modern, and the new upscaled world as HD.

### Changed

- Added a persisted `graphics_tier` option with `Legacy`, `Modern`, and `HD`
  choices. Fresh installs target HD; existing option files migrate their old
  procedural setting to Legacy and their old authored setting to Modern so an
  upgrade does not silently change visual fidelity or performance.
- Preserved `legacy_graphics` and `set_legacy_graphics()` as compatibility
  mirrors. `Ctrl+Alt+L` still toggles procedural rendering, returning to the
  last selected authored tier, while the Options row cycles all three tiers in
  either direction.
- Restored the exact master-era 64×64 floor/stair and 68×68 wall/door resources
  in a validated Modern companion manifest. Modern and HD share current actor,
  prop, menu, and HUD assets; only their complete world sprite sets differ.
- Limited the native `1.0 / 1.2 / 1.4 / 1.6` world raster buckets to HD.
  Modern and Legacy retain the established single-resolution render path.
- Made graphics-sensitive world, paused-scene, mobile-floor, GPU-shell, fog,
  normal-map, and scaled-sprite caches tier-aware or explicitly invalidated at
  the one graphics-mode transition seam.
- Added `--graphics-tier legacy|modern|hd` to the deterministic gameplay
  profiler so future HD work can be compared directly with both compatibility
  paths.
- Runtime, Android package defaults, relay-server metadata, and download
  metadata now report `4.9.14`. Options advance to schema `10`; run-save and
  multiplayer protocol schemas are unchanged.

### Validation

- Modern world manifest metadata and all 35 active source PNGs are checked
  byte-for-byte against the pre-HD baseline.
- Added three-tier regression coverage for persistence/migration, forward and
  reverse menu cycling, compatibility toggling, source selection, render
  buckets, cache retirement, and desktop/mobile rendering.
- Resolved Modern floors, walls, special floors, stairs, and doors match the
  pre-HD renderer's aggregate pixel/anchor baseline at the compatibility
  bucket.
- The built wheel contains the Modern companion manifest, all 38 preserved
  lower-resolution world files, and the HD world set.
- `python -m compileall src tests tools/build_world_asset_derivatives.py`
- Full non-web test suite (1,024 tests)

## 4.9.13 — The rear rim meets the floor

Release 4.9.13 redraws the stairwell's north, northwest, and northeast
exterior rim against the floor tiles that actually sit behind those edges.
The rear masonry now uses the surrounding flagstone's midtone palette instead
of a continuous near-black contour, while the approved descent remains intact.

### Changed

- A PixelLab precise-object edit is confined to a deterministic 19,017-pixel
  rear-rim band derived from the projected north, northwest, and northeast
  floor coverage. The representative repo floor trio is layered in the same
  painter order as the runtime before generation.
- The selected redraw is normalized identically into all eight existing stair
  frames. The exact 512×512 binary alpha silhouette, `[256, 336]` source
  anchor, rim-block geometry, upper landing, treads, shaft, southern/front
  rim, and violet pulse are unchanged.
- The world-asset derivative tool can now build the repo-only PixelLab context
  and masked rear-rim inputs, then mechanically restore all frozen pixels and
  animation deltas around a generated edit.
- The manifest, renderer, painter order, desktop and mobile tile caches,
  animation timing, and runtime memory footprint are unchanged.
- Runtime, Android package defaults, relay-server metadata, and download
  metadata now report `4.9.13`. Save, multiplayer protocol, and schema versions
  are unchanged.

### Validation

- Native 512px review covered all four uniform floor variants, mixed
  north/NW/NE combinations, and quiet/peak-glow frames.
- Near-black pixels along the rear exterior boundary fell from 346 to 11.
  Mean RGB distance to the nearest covering floor palette fell from `52.11`
  to `11.24`; strongly contrasting boundary pixels fell from 402 to 17.
- Added regression coverage for edit-mask extent, frozen pixels, identical
  rim bytes across all frames, the canonical stair alpha, and the unchanged
  animation-delta signature.
- `python -m compileall src tests tools/build_world_asset_derivatives.py`
- Focused stair asset, renderer, cache, native-zoom, and metadata suites
  (88 tests)
- Full non-web test suite (1,012 tests)

## 4.9.12 — The descent begins at the threshold

Release 4.9.12 corrects the native stairwell's entry geometry. The old
high-resolution master left a tall uninterrupted wall above its first tread,
making the staircase appear to begin halfway down the shaft.

### Changed

- A floor-level landing now meets the rear inner rim, followed by four
  contiguous stone treads that descend clockwise into the existing spiral.
- The repair is confined to the upper stairwell interior. All eight frames
  retain the exact 512×512 canvas, floor-matched alpha silhouette, bounding
  box, and `[256, 336]` source anchor.
- The existing violet pulse is transferred as low-frequency illumination over
  the corrected masonry, preventing old wall joints from ghosting onto the new
  treads and keeping geometry fixed across the full ping-pong animation.
- Runtime, Android package defaults, relay-server metadata, and download
  metadata now report `4.9.12`. Save, multiplayer protocol, and schema versions
  are unchanged.

### Validation

- Added a semantic asset regression that requires a substantial tread component
  at the upper rim instead of accepting the former halfway-down start.
- Preserved the canonical stair alpha hash and existing native-resolution
  animation, manifest, cache, underlay, and zoom coverage.
- `python -m compileall src tests`
- Focused stair asset, animation, tile, native-zoom, and metadata suites
  (81 tests)
- Full non-web test suite (1,011 tests)

## 4.9.11 — The dungeon holds at full scale

Release 4.9.11 completes the full native high-resolution world pass. Wall
silhouettes no longer expose their enlarged 64px staircase, ordinary floors
use one canonical isometric shell, and maximum zoom now resolves the 512px
terrain masters at their final render density instead of shrinking and
re-enlarging them.

### Changed

- The generic and eight special-room wall masters now share a precise native
  2:1 isometric contour. Their former eight-row edge plateaus advance at
  single-source-pixel resolution, while locally sampled stone replaces both
  exposed wedges and near-black boundary fringe.
- All four ordinary floor variants use a full-width canonical top diamond and
  shallow 30px masonry side. Their runtime widths are now exactly
  320/384/448/512px across the four render buckets, eliminating the persistent
  join gap caused by height-constrained scaling.
- Wall and door cells now receive a canonical floor underlay before their
  depth-sorted prism is painted. The tapered wall-contact corners therefore
  reveal continuous masonry instead of small clear-color wedges on desktop or
  in incremental mobile floor-cache patches.
- World rendering uses bounded 1.0/1.2/1.4/1.6 detail buckets, corresponding
  to integer 5/6/7/8 pixel drawing units. Continuous camera zoom applies only
  a final identity or downscale residual; maximum 1.6 zoom draws directly to
  the display with the generic 512px floor alpha unchanged.
- Tiles, directional doors, actors, props, telegraphs, lighting, fog, visible
  bounds, painter ordering, and display-to-world picking now consume the same
  active render bucket. Legacy graphics retain their established 1.0 path.
- Derived world surfaces are cached by render bucket with a 192-entry and
  64 MiB LRU budget. Scaled actor sprites have a separate 48 MiB bound,
  game-level tile caches retain only the active bucket, and Android low-memory
  cleanup releases every native-zoom surface family and render layer.
- Generic floor canvases retain their compact rectangular allocation; only
  stairs and deeper special-room floor masters use a full-height square
  canvas. This preserves native detail while avoiding nearly twice the common
  floor memory cost on mobile.
- Guiding-floor overlays are clipped and blended only as visible floor
  material; invisible RGB below transparent pixels no longer counts as route
  coverage.
- Approved door and guest-room floor artwork is unchanged.
- Runtime, Android package defaults, relay-server metadata, and download
  metadata now report `4.9.11`. Save, multiplayer protocol, and schema versions
  are unchanged.

### Validation

- Added an eight-test native zoom matrix covering bucket selection, exact
  512px maximum-detail alpha, bounded reuse, painter order, actor scale and
  ground anchors, fog and light projection, desktop/mobile picking, target
  allocation, wall/door floor underlays, memory ceilings, and Android
  low-memory release.
- Extended world-asset QA with canonical floor bounds, wall alpha hashes,
  one-row diagonal plateaus, and zero near-black exterior boundary pixels.
- Runtime contact sheets at 0.65/1.0/1.2/1.4/1.6 zoom measured zero
  clear-color seam pixels after the wall underlay, exact floor widths at every
  bucket, and aligned stair/fog/light geometry.
- Headless Android-layout/native-resolution benchmarking at 1280×720
  (30 steady-state full draws per zoom) measured 5.865–9.882 ms medians,
  a 15.363 ms worst p95, and 2.307–4.710 MiB of live derived world surfaces.
- `python -m compileall src tests`
- Focused zoom, sprite-cache, tile, mobile, lighting, and asset-quality suites
- Full non-web test suite (1,004 tests)

## 4.9.10 — The dungeon settles into place

Release 4.9.10 completes the selected Plan 1 refinement pass: texture variety
is separated from structural geometry, wall silhouettes lose their remaining
black fringe, and the stairwell is rebuilt to belong to the surrounding floor.

### Changed

- The ordinary floor now has four native 512×512 coarse-stone masters with the
  same canonical footprint. Broad irregular slabs, restrained joints, and
  distinct wear patterns reduce both visual noise and obvious repetition.
- Coordinate variant selection now uses an avalanche hash instead of exposing
  low-bit modulo patterns, removing the exact four-tile repetition that was
  visible along dungeon axes.
- Guiding-floor art is stored as eight sparse transparent rune overlays and
  composed over whichever floor variant occupies the tile. The floor therefore
  keeps its local texture when guidance activates instead of swapping to a
  repeated full-tile frame.
- The generic and eight special-room wall sprites retain their approved
  geometry and interior motifs while removing contour-connected near-black
  pixels from the transparent boundary. The floor contact now reads as a clean
  masonry edge rather than a black seam.
- The stairwell and all seven pulse frames are rebuilt at native 512×512
  detail with broad floor-matched rim stones, readable treads, and a
  geometry-locked violet glow confined to the shaft and inner steps.
- Approved door and guest-room floor artwork is unchanged.
- Runtime, Android package defaults, relay-server metadata, and download
  metadata now report `4.9.10`. Save, multiplayer protocol, and schema versions
  are unchanged.

### Validation

- Added automated world-asset checks for canonical floor masks and genuinely
  distinct variants, sparse guidance overlays, fringe-free wall boundaries,
  and aligned native-resolution stair animation.
- Added regression coverage that rejects exact four-tile variant repetition
  along either dungeon axis.
- `python -m compileall src tests`
- Focused sprite, tile-variant, and world-animation suites (65 tests)
- `python -m unittest discover tests` (1001 tests)

## 4.9.9 — The stonework falls into line

Release 4.9.9 refines the native-resolution dungeon masonry after the first
human review pass, concentrating on modular joins, floor contact, and door
readability.

### Changed

- Generic wall cap and mortar rails now repeat exactly across the runtime
  isometric neighbor vectors, removing the accumulated row skew between
  adjacent wall blocks.
- All eight special-room wall variants inherit the corrected top, opposite
  face, contact geometry, and periodic masonry bands while retaining their
  dedicated bar, garden, Lossless Soul, and quest-room motifs.
- The ordinary floor uses narrower, smoother slab joints and its canonical
  tile footprint again, reducing distracting gaps and producing a cleaner
  wall-to-floor intersection. All eight guiding-floor frames were rebuilt from
  that corrected base without animation-geometry drift.
- Closed and open directional doors now share one corrected masonry shell.
  Both open variants read as fully open apertures; in particular,
  `door_open_right.png` no longer leaves a broad half-open leaf across the
  doorway.
- Runtime, Android package defaults, relay-server metadata, and download
  metadata now report `4.9.9`. Save, multiplayer protocol, and schema versions
  are unchanged.

### Validation

- Verified the 512×512 masters retain binary alpha, canonical anchors, and
  stable guiding-floor geometry.
- Verified generic and special walls in repeated runtime-scale overlap grids,
  including exact `(±160, 80)` mortar continuity and clean floor contact.
- Verified closed/open state pairs for both directional door families.
- `python -m compileall src tests`
- Focused sprite, world-rendering, room, lighting, metadata, and regression
  suites (146 tests)
- `python -m unittest discover tests` (995 tests)

## 4.9.8 — The stonework comes into focus

Release 4.9.8 replaces the world sprites that still read as enlarged low-
resolution art after the first pass, while retaining the ten assets already
approved in review.

### Changed

- Twenty-five floor, guiding-floor, special-room wall, and door masters were
  regenerated as native-detail 512×512 pixel art from repository references.
- The approved `wall_401.png`, guest-room floor, and complete stair animation
  remain byte-identical.
- Ordinary and special-room floors now use native-resolution contours,
  masonry, material wear, and side-face detail instead of reapplied 8× source
  masks. The guiding-floor sequence shares one stable native alpha silhouette
  and ramps toward a single detailed illuminated endpoint.
- Special wall and door motifs were regenerated only inside controlled face
  regions based on the accepted wall structure. Runtime paths, anchors,
  directional aliases, animation timing, and tint behavior are unchanged.
- Runtime, Android package defaults, relay-server metadata, and download
  metadata now report `4.9.8`. Save, multiplayer protocol, and schema versions
  are unchanged.

### Validation

- Verified all 25 replacement masters at 512×512 with binary alpha and genuine
  sub-8px detail; guiding animation geometry is pixel-stable across all frames.
- Verified the ten accepted masters remain byte-identical.
- `python -m compileall src tests`
- `python -m unittest tests.test_sprite_assets tests.test_mainline_regression`
  (49 tests)
- Focused story-room, soul-room, lighting, and dark-level regressions
  (77 tests)
- `python -m unittest discover tests` (995 tests)

## 4.9.7 — The depths in sharper relief

Release 4.9.7 upscales the active dungeon world art while preserving the
silhouettes, placement, and animation behavior of the original designs.

### Changed

- All 35 active floor, special-floor, guiding, stair, wall, and door PNG
  masters were rebuilt from the repository-original 64×64 designs at
  PixelLab's maximum sensible 512×512 size.
- The active 68×68 wall and door sources had only their transparent border
  cropped, restoring the original 64×64 design bounds before the 8× upscale.
- Source alpha masks and anchors are preserved at 8× resolution, and animation
  geometry is stabilized across frames. Three unused legacy PNGs remain
  unchanged.
- Runtime, Android package defaults, relay-server metadata, and download
  metadata now report `4.9.7`. Save, multiplayer protocol, and schema versions
  are unchanged.

### Performance

- Applying tint after scaling was 1.93× faster in the focused microbenchmark
  and processed 60.9% fewer tint pixels.

### Validation

- Verified all 35 active masters at 512×512 with exact 8× source alpha,
  stable guiding/stair frame geometry, and byte-identical unused legacy files.
- `python -m compileall src tests`
- `python -m unittest tests.test_sprite_assets` (39 tests)
- Focused world, story-room, lighting, and gameplay regressions (100 tests)
- `python -m unittest tests.test_mainline_regression` (10 tests)
- `python -m unittest discover tests` (995 tests)

## 4.9.6 — The frame holds

Release 4.9.6 delivers the first performance-optimization pass from the
project-wide profile, removing the largest save, music, and menu stalls while
protecting deterministic combat behavior.

### Added

- A repeatable before/after benchmark harness covers 45- and 90-enemy updates,
  default-zoom crowd rendering, paused menus, About, durable saves, routine
  kills, and procedural music latency.
- A deterministic spatial broad phase activates for crowds of 60 or more
  enemies and preserves source-list ordering for contacts, targeting, and
  chained effects.
- Raw benchmark results and the implementation comparison are available under
  `build/performance/`.

### Changed

- Routine kill saves are coalesced behind a short checkpoint window, while
  bosses, lifecycle events, exits, floor transitions, and other critical save
  boundaries remain immediate. Save JSON is now compact UTF-8.
- Procedural music PCM is generated by one cancellable background worker;
  Pygame mixer objects and channel operations remain on the game thread.
- Opaque and paused screens avoid unnecessary world rendering. Static menus,
  About/license layout, Discipline layout, text measurement, and rendered menu
  text now use bounded retained caches with explicit invalidation.
- The story intro skips hidden world/HUD work and uses a direct opaque fill.
  Zoomed pixel-art composition now uses nearest-neighbor scaling.
- Discipline path names and Degree labels now use the same panel-edge padding
  as the `Chosen` / `Available` footer legend.
- Runtime, Android package defaults, relay-server metadata, and download
  metadata now report `4.9.6`. Save schema and multiplayer wire protocol
  versions are unchanged.

### Performance

- Routine kill work fell from 16.58 ms to 0.075 ms at the median.
- Run/menu music request latency fell from roughly 225-645 ms to about 0.27 ms;
  synthesis completes in the background.
- Paused Inventory and Disciplines rendering improved by roughly 90% and 86%
  respectively at 1280×720.
- Default-zoom 1920×1080 crowd rendering improved by 22.3% in the paired
  benchmark. Direct output-resolution rendering remains the next larger
  renderer optimization.

### Validation

- `python -m compileall src tests` — OK.
- Full desktop/non-web suite — 987 tests, all passing.
- See `build/performance/best_first_pass_comparison.md` for methodology,
  p50/p95 results, raw artifact names, tradeoffs, and remaining Android/device
  validation.

## 4.9.5 — The path remembered

Release 4.9.5 makes unspent memory tokens visible during play and turns that
notice into a direct route to the Disciplines screen.

### Added

- A compact bottom-left HUD panel appears whenever the player has one or more
  unspent memory tokens. It shows the available count beside the existing
  ouroboros memory glyph and remains hidden when there is nothing to spend.
- Clicking the panel on desktop, or tapping it on mobile, opens the Character
  screen directly on the Disciplines tab. The prompt is a dedicated input
  target, so activating it cannot fall through into a melee attack or world
  interaction.
- The Disciplines footer now repeats the exact available memory-token count
  below its state legend. The row stays reserved when the count reaches zero,
  preventing the discipline grid from shifting as tokens are spent.

### Changed

- The `Chosen`, `Available`, `Locked`, and `Sealed` legend is inset from the
  left panel edge. The matching `x paths ready` text receives equal padding
  from the right edge.
- Story and interaction panels account for the new HUD prompt, and the mobile
  layout keeps it inside the gameplay area clear of the joystick and action
  rails.
- Runtime, Android package defaults, relay-server metadata, and download
  metadata now report `4.9.5`. Save schema and multiplayer wire protocol
  versions are unchanged.

### Validation

- `python -m compileall src tests` — OK.
- Focused HUD layout, desktop mouse, mobile touch, Discipline footer, and
  release-path coverage — 82 tests, all passing.
- The HUD prompt and Disciplines footer were visually reviewed at 1280×720 and
  the compact 640×360 layout; all prompt/footer copy fits without overlap.
- Full desktop/non-web suite — 952 tests, all passing.

## 4.9.4 — What the depths remember

Release 4.9.4 renames the Discipline currency from **mastery tokens** to
**memory tokens**. The new name frames each choice as recalling hard-won
knowledge: “By consuming a memory token, you will remember a discipline you
have once acquired.”

### Renamed

- `Player.mastery_tokens` is now `Player.memory_tokens`, and
  `grant_mastery_token` is now `grant_memory_token`.
- New run saves write `"memory_tokens"` instead of `"mastery_tokens"`.
- Level-up and reward floaters, War Shrine feedback, the Character subtitle,
  Discipline availability details, controls documentation, and the
  human-review Discipline specification now consistently say “memory token.”
- The default Disciplines footer uses the new explanatory copy supplied for
  the currency.

### Compatibility

- Save schema version 5 is unchanged. Restore precedence is
  `"memory_tokens"` → legacy `"mastery_tokens"` → older `"skill_points"` →
  zero, preserving banked currency across both prior renames.
- Multiplayer fast snapshots retain their neutral `"tokens"` field. Full
  player payloads prefer `"memory_tokens"` while temporarily mirroring
  `"mastery_tokens"` for warning-only mixed-version sessions.
- The unrelated end-of-run **Mastery** statistic and historical changelog
  wording remain unchanged.
- Runtime, Android package defaults, relay-server metadata, and download
  metadata now report `4.9.4`.

### Validation

- `python -m compileall src tests` — OK.
- Focused currency, Discipline-menu, multiplayer-sync, Discipline-path,
  familiar, save, and metadata coverage — 82 tests, all passing.
- The supplied memory-token explanation fits the Disciplines footer at the
  tested 640×360, 960×600, and 1280×720 layouts.
- Full desktop/non-web suite — 947 tests, all passing.

## 4.9.3 — The storm finds its conduit

Release 4.9.3 makes the Arcanist's Storm discipline path a complete,
action-specific Arc Bolt route. Storm was introduced as the chaining
companion to Bolt's fan, pierce, and homing progression, but only Chain
Lightning ever had a combat hook: Static Charge, Tempest, Stormcaller, and
World Storm described charge or autonomous-storm mechanics that did not
exist. Every Storm Degree now changes Arc Bolt directly while preserving the
Storm discipline names, save keys, prerequisites, path identity, and authored
glyphs.

### Changed

- **Five-Degree Storm ladder**:
  - **Static Charge** lowers Arc Bolt's mana cost by 1.
  - **Chain Lightning** arcs from the first target to one secondary target
    within 2.6 tiles.
  - **Tempest** continues through two secondary targets with 2.8-tile hops.
  - **Stormcaller** reaches three secondary targets at 3.2 tiles per hop and
    seeks elites, minibosses, and bosses before ordinary foes.
  - **World Storm** reaches four secondary targets at the path's maximum
    3.6-tile hop range.
- Every projectile in an Arc Bolt cast shares exactly one Storm charge.
  Whichever shard hits first spends it, even if that bolt survives by piercing.
  This prevents a missed preselected shard from discarding Storm without
  allowing Bolt's fan to multiply the full effect across every shard and
  pierced target. Every chain target takes 55% of the triggering bolt's damage,
  can be struck only once by that chain, and still requires line of sight from
  the previous target. The bounded four-hop maximum keeps the capstone
  predictable in dense rooms and avoids an unbounded per-projectile scan on
  mobile.
- **Complementary Bolt + Storm identities**: Bolt owns projectile count,
  lifetime, fan shape, piercing, pierce retention, and homing. Storm owns the
  mana discount and a single bounded cast charge. In a combined build every
  bolt retains Bolt's pierce and homing effects, and the first one to connect
  triggers Storm. Homing bolts now ignore dead and already-hit targets, and
  piercing collision skips already-hit bodies instead of letting them mask a
  fresh target. Misleading Bolt copy about impact splitting, elite ricochets,
  and mana-on-kill has been replaced with truthful qualitative behavior.
- **Runtime-truth copy for every Discipline**: all player-facing descriptions
  are qualitative, avoid raw numeric values, and name only effects that
  currently execute. Warden Vow, advanced Riposte, Rogue Marksman, Arcanist
  Ward, advanced Acolyte Veil, Acolyte Curse, Ranger Survival, and other
  stats-only nodes now state their actual stat categories instead of promising
  unwired executes, reflections, summons, curses, stealth, trap immunity, or
  similar mechanics. Exact tuning remains in the human-review specification
  and regression tests.
- Storm's flat bonuses are rebalanced from `+20` spell / `+56` maximum mana to
  `+16` spell / `+52` maximum mana: equal to Bolt's mana budget and Nova's
  spell budget, with reliable multi-target damage supplying the path's power.
- The separate Bolt-path **Storm Sigil** is now **Conduit Sigil**, removing its
  naming collision with the Storm path. It still lets piercing Arc Bolts
  retain 82% damage after each foe instead of the normal 70%. Its stable
  `arcanist_storm` save key and authored glyph remain unchanged.
- Loading a pre-4.9.3 active Storm run reconciles the old aggregate bonuses to
  the new values once, clamps current mana to the adjusted maximum, and stamps
  the current release on its next save so the delta cannot be applied twice.
- Runtime, Android package defaults, relay-server metadata, and download
  metadata now report `4.9.3`. The Storm path's names, all discipline save and
  glyph keys, the run-save schema, and wire protocol are unchanged.

### Validation

- `python -m compileall src tests` — OK.
- `python -m unittest tests.test_skill_paths` — 22 tests, all passing,
  including the exact Storm content contract, every chain cap/reach, damage,
  elite priority, shared-charge rule, missed-shard handling, post-chain homing,
  old-save migration, mana cost, Conduit Sigil pierce retention, qualitative
  copy, and exact stats-only description contracts.
- Responsive Discipline copy fits every archetype at the tested UI breakpoints.
- `python -m unittest discover tests` — 950 tests, all passing.

## 4.9.2 — Signs in the stone

Release 4.9.2 gives every discipline a face. The character sheet's old generic
rounded cards are replaced by five archetype-specific, socketed node plates,
and the full set of 100 authored discipline glyphs now ships with the game.
Warden cathedral iron and aged gold, Rogue obsidian and bruised violet,
Arcanist moon silver and electric blue, Acolyte ossuary ivory and dried
crimson, and Ranger dark yew and antler bone remain visually distinct without
competing with the node names or state colors.

### Added

- **Complete discipline glyph library**: all 20 glyphs for each of the five
  archetypes are copied losslessly from `build/discipline_glyphs`, renamed to
  their stable discipline save keys, registered in the UI manifest, and
  packaged for desktop wheels, web builds, and Android.
- **Five selected PixelLab Variant B socket plates**: the more ornate,
  border-confined designs use equal-axis 600×384 sources and unscaled
  `y=96..287` center crops for circular 600×192 runtime panels. Source IDs and
  seeds are Warden `ff0adaab-47dc-44ee-bcb8-ef7b02f73cf8` / `492202`,
  Rogue `7cef642a-3074-4ca3-8f09-24541b7022c1` / `492212`, Arcanist
  `5b711ab5-902a-4be5-a9df-609e0f1f6af2` / `492222`, Acolyte
  `2b5ae344-78f0-4685-bf87-09f09268f8eb` / `492232`, and Ranger
  `8fd139c5-39bb-4374-86eb-3c45c1fe17de` / `492244`. Warden's selected rail
  repair is PixelLab job `09df8968-7e21-42bc-9b9f-0b4a957e20a2`, seed
  `492252`.
- **Rogue socket refinement**: a custom-mask PixelLab inpaint removes Variant
  B's smallest concentric socket circle while retaining its two larger bezel
  rings. The selected repair is job
  `b220994d-1388-42d1-9441-486b18879884`, seed `492263`; pixels outside the
  authored `(104, 96)` disk mask are unchanged.

### Changed

- Discipline cells now use archetype-specific horizontal three-slice geometry,
  encoded through the existing nine-slice renderer with square, full-height
  192×192 protected socket endcaps and per-panel safe text insets. This keeps
  X/Y scaling identical at every target height. All five Variant B sockets
  share the authored `(104, 96)` center. Glyphs use a larger 9–40 pixel target
  range with nearest-neighbor sampling, and symmetric size bounds keep every
  glyph exactly centered instead of allowing edge clamping to shift it out of
  its socket.
- Runtime state stays separate from the source art: node-name treatments, the
  thin selection/hover outline, and the legend communicate chosen, available,
  locked, and sealed states. The one-pixel outline follows each rendered
  plate's visible outer bounds instead of the larger transparent asset canvas.
  Sealed node titles now share the muted-red seal treatment instead of the
  generic locked-text color. No runtime circle is drawn inside the authored
  glyph sockets.
- Node names retain their existing typography and fit as complete text, using
  a smaller font only when needed or up to three lines inside the authored rail
  instead of silently ellipsizing into the border. Inline descriptions use a
  dedicated smaller font, allowing every complete wrapped description to fit
  at default UI scale at 1280×720 and larger. Tighter or more heavily scaled
  layouts retain readable name-only cards, and the selected or hovered node
  always exposes its complete description in the wide detail footer. Tall,
  narrow windows preserve the plate's aspect inside a larger input target so
  decorative endcaps cannot consume the text rail.
- The empty “Committed x/x paths · pick…” instruction no longer occupies the
  top of the tree. Its space returns to the discipline grid; completed-path
  bonus details appear only after a path is finished and omit the redundant
  commitment counter.
- Sealed path headers retain their clean path names without a `[lock]` prefix;
  the red sealed-node titles and legend provide the state cue.
- Missing panel or glyph resources remain contained by the optional UI asset
  loader, and legacy graphics preserves the procedural discipline cards.
- Runtime, Android package defaults, relay-server metadata, and download
  metadata now report `4.9.2`. The wire protocol is unchanged.

### Validation

- Bytecode compilation succeeds across `src` and `tests`.
- The focused discipline/UI/input/mobile/metadata pass completes 135 tests, and
  the full supported non-web test collection completes 931 tests.
- Automated layout checks cover all five archetypes at 1920×1080, 1280×720,
  960×540, 640×360, 640×600, and 640×960, including responsive
  name/description fitting, content bounds, legacy graphics, and missing-glyph
  fallback.
- The built wheel contains all five panel sources and all 100 discipline
  glyphs.

## 4.9.1 — Hands in the margins

Release 4.9.1 gives both descenders something to do inside the story's pauses.
Three short, fail-forward interludes now turn the multiplayer code-seal runes
into shared play: **Bind the Page** asks the party to repeat Rue's sequence at
the depth-5, depth-8, and depth-9 turning points; **Wake the Moonbloom** begins
by talking to either of the Garden's dancing frogs, then becomes a quick target
hunt; and **Mirror the Unlost** opens the Lossless Soul's audience with paired
memory seals. A loss never blocks a relic path or dialogue answer. Wins grant
small permanent run boons alongside a fitting recovery, and every refuge-room
reward is once-only.

### Added

- **Deterministic mini-game models** (`story/minigames.py`): Pygame-free Story,
  Garden, and Soul state machines with untimed ready guides and short
  preview/play/result phases, depth-scaled goals, bounded absolute
  serialization, contribution tracking, and compact ready/cell intent codecs.
- **Three story attach points** (`story/runtime.py`): Bind the Page runs before
  the chosen relic path is committed on depths 5/8/9; talking to either dancing
  frog opens the Garden's once-per-room interlude; and Mirror the Unlost runs
  once before Preserve / Release / Refuse. All outcomes continue the authored
  flow. Bind fully restores HP and adds +1 damage; Garden adds +5 maximum HP
  and mana; Soul adds +5 maximum mana and +1 spell damage.
- **Full-screen rune board** (`rendering/minigames.py`): responsive 3-column
  Story/Garden grids and adaptive Soul grids, timer and progress feedback,
  result cards, co-op contribution copy, large direct hit targets, and
  procedural glyph fallbacks. The board reuses the multiplayer lobby's
  `menu.glyph.sigil.*` vocabulary and assets, seated in three PixelLab-authored
  sockets: bound iron and amethyst for Story, rooted moon-stone for Garden,
  and mirrored silver-obsidian for Soul. Gameplay highlights remain dynamic
  overlays, with the original procedural cells retained as per-asset fallbacks.
- **Unified controls**: each concise guide has one large authored **READY**
  button, confirmed with Interact / `E`, its remapped joypad equivalent, click,
  or touch. Once both co-op players are ready, direct mouse click and Android
  tap, number keys, arrow-key navigation, and controller D-pad/A route through
  the same modal cell action. Misses, swipes, Back, and stale gameplay inputs
  are consumed so nothing leaks into the paused dungeon.
- **Co-op from the first version**: the host owns simulation and sends an
  absolute mini-game state in every fast snapshot; the guest sends immediate
  revision-stamped presses, can keep contributing after falling, and may
  initiate the Garden or Soul interlude. Ordered Story/Soul bursts from one
  snapshot remain responsive while any independent host/timer change safely
  breaks the chain. Completed Garden/Soul state and Soul choices converge in a
  compact, bounded slow-snapshot payload so guest interaction hints stay true.
- **Save support and regression coverage**: active interludes are an additive
  run-save field, including the pending story continuation. New engine,
  runtime, rendering/input, save round-trip, hostile-state, relay, and
  multiplayer synchronization tests cover completion, failure, once-only
  rewards, malformed data, fallen-guest participation, and input ordering.

### Changed

- Depth 1 remains direct and onboarding-focused; the main-story interlude is
  reserved for later authored turning points instead of becoming a floor-by-
  floor toll.
- Mini-game play windows are restored to their original cadence: Bind the Page
  allows 7.5 seconds, Wake the Moonbloom 9 seconds, and Mirror the Unlost
  12 seconds. The timer does not begin until the solo player—or both players
  in co-op—has read the on-screen guide and explicitly confirmed readiness.
- Mini-games own the opaque frame and skip dungeon/HUD rendering underneath,
  preserving the existing cutscene optimization on Android.
- Runtime, Android package defaults, relay-server metadata, and download
  metadata now report `4.9.1`. The additive `minigame` intent remains on wire
  protocol v1, so deployed relays should be updated with this shared codec.

### Fixed

- Named floor guardians now seal their stairs on every depth, for both solo
  and host-validated guest interactions; descent unlocks only after the
  guardian dies.
- Either co-op player can now engage a boss arena. When its doors seal, every
  player—including a fallen partner—is moved to a distinct safe position
  inside instead of leaving the guest trapped beyond the doors.
- Malformed save/snapshot combinations such as a result with no outcome can no
  longer hold the full-screen modal open; non-finite and oversized timers are
  bounded during ingestion.
- A delayed or simultaneous guest press is ignored without becoming a mistake,
  while valid ordered presses no longer have to wait for a fresh 15 Hz snapshot
  between every Story rune or pair of Soul cards.
- A remote player can now initiate the Lossless Soul interaction instead of
  being silently excluded from the host-owned modal flow.

### Validation

- `python -m compileall src tests` — OK.
- `python -m unittest discover tests` — 933 tests, all passing.
- Headless desktop/mobile visual review of all three boards; the warmed
  full-screen Soul board measured ~0.97 ms/frame at 960×540 and ~0.42 ms/frame
  at 720×360 in the dummy SDL renderer.

## 4.9.0 — Story that makes some sense

Release 4.9.0 is the narrative rewrite. The dungeon is now one place with one myth: the sunken city **Kasar Voss**, stacked on the **Last Gate** — the door where unfinished things go to end. The Dread Gate Tyrant is **Sorn Voss**, the Toll-Keeper who refused to let anything end; the narrator is **Nim Rue**, a Scriptorium scribe writing the run as a rationed ledger entry — which is the diegetic reason every line of story text is now short. The old ~1,950-character wall-of-text omen (26 wrapped lines against a 2-line window) is gone: text budgets measured from the renderer are enforced at the corpus source and in CI. Each archetype's three rolled backstories became chapters of one canonical life with a fixed wound, oath, and secret; each archetype gains a recurring **tether NPC** (Ser Caldus, Coin-Eye Pell, Nim Rue, the Blue-Lipped Child, Sable of the Moon-Hunt) who appears at depths 1/5/9 — meet, reveal, crisis — plus a boss mirror among **the Stalled**, the four floor bosses reframed as officials of fallen Voss frozen mid-office, each dying with a last word. Depth 8 is always the Gate's Confession: the Toll-Keeper's per-archetype offer to un-happen the wound. Aid/Bargain/Defy mechanics are wired exactly as before, but the run now reads them back as a **Road** (Witness/Debtor/Defiant/Forsaken) that shades omens; true names are a discoverable **name economy** (learning *Sorn Voss* costs one story-log memory, changes the final boss's nameplate, and staggers his HP; the Lossless Soul is canonically **Liss Voss**, his daughter); everyone you Aided gathers at the depth-10 door; and victory now passes through **the Tenth Bell** — the Gate asks the last question and one of fifteen authored endings (archetype × verb, shaded by road, crisis, and names) plays before the summary. Nim Rue closes every run on the death and victory screens, and remembers you across runs. The full story bible lives in `build/story/`.

### Added

- **Story bible** (`build/story/`, gitignored working canon): premise, progression, characters, five archetype throughlines, choice/road/ending design, measured text budgets, and code attach points.
- **`content/story_corpus.py` rewrite**: canonical `STORY_ARCS` (wound ≤90 / oath ≤80 / secret ≤90, three chapter titles each), `STORY_TETHERS` with authored meet/reveal/crisis beats and per-verb crisis text, `TYRANT_OFFERS`, budgeted dilemmas (three pinned: The Door That Remembers at D5, The Gate's Confession at D8, The Last Guest's Mask at D9), guest `speeches` parallel to motives (guests now talk instead of being narrated), `STORY_ENDINGS` (15), crisis echoes, road codas, `BOSS_OMEN_LINES` + archetype mirror variants, `BOSS_LAST_WORDS`, `RUE_DEATH_LINES`, `STORY_GATE_CHOICES`, and Guild `SHOP_PATTER_LINES`.
- **`StoryEngine` 4.9 composition** (`story/engine.py`): beat summary = scene line + stake line (≤220 total); dialogue = the guest's spoken line (≤185); tether override on depths 1/5/9; pinned dilemmas; faction-orbit weighting for petitioners; `story_road()` derived purely from beats at act breaks (D3/6/9) — no new save state.
- **Cutscene assets** (`assets/quest_cutscenes.json`): budgeted floor omens (`{omen_body}`, assembled with boss-office foreshadow on boss floors and road clauses in later acts) and `story_epilogue` — gate question → ending → the tenth bell (`complete_victory` action). The narrator speaker is now Nim Rue; the quest-dialogue node speaks as the guest. Every floor — including depth 1 — opens straight on the relic choice: there is no separate prologue screen (playtesting judged it too much text before play); Rue's cross-run greeting rides as the depth-1 omen's first line, the wound shows on the archetype select screen, and the objective lives in the story panel.
- **The Tenth Bell**: `finish_final_descent` routes the depth-10 stairs through the epilogue; `choose_story_gate_verb` records `gate:{verb}`; endings persist to a new `meta_progress["story"]` slot so Rue's depth-1 greeting remembers pages and clears.
- **Name economy**: `learn_story_name` (flag + one log line blanked to "———" as the visible memory cost); Sorn's name via Defy at the D8 confession (or the Acolyte's refusal), Liss's name from an act-III Hall visit; nameplate, engage floaters, and spawn-HP effects on the final boss.
- **Depth-10 gathering**: every Aid-resolved guest stands at the door as a silent witness (idle NPCs on the story-guest art); the Forsaken road gets the empty room and one line.
- **Hall of Unlost Echoes guarantee**: `Dungeon(force_soul_hall=…)` forces the hall by depth 7 if the 50% roll never landed (flavor RNG draw still consumed, streams stay byte-stable), tracked via a `:hall` story flag.
- **Surface flavor**: archetype-select preview shows the canonical wound line; shopkeepers (canonically Voss Mortuary Guild factors under Coin-Eye Pell) speak rotating Guild patter; boss deaths float their last words; death/victory overlays carry Rue's closing line.
- **`tests/test_story_budgets.py`** (12 tests): corpus source budgets, engine composition across seeds × archetypes, tether/pinned structure, road derivation, ending coverage, live cutscene nodes vs the 2-line window, omen assembly per depth, prompt clamp, the full epilogue flow to victory, name costs + Tyrant stagger, the hall guarantee, and the gathering.
- **Page medallion** (`menus/story_choice_page.png` + `cutscene.choice.icon.page`): a 32×32 parchment-page coin in the existing icon set's style (authored from the aid medallion's coin base) for continue-style choices — "Turn the page" and the epilogue's silent beat carry `choice_key: "page"`, with a matching procedural dog-eared-page glyph in legacy graphics.

### Changed

- **Cutscene text is bigger and letter-spaced**: the narration body moved from `small_font` (16 px) to the main UI font (22 px); choice labels sit a step below on a new dedicated 19 px `choice_font` with details in small, and both authored and legacy choice rows now center their text block vertically (the old centering clamped to the top when the block ran taller than the panel content box); the story narration additionally renders with one extra pixel of letter tracking (UI-scaled) via a cached glyph-by-glyph renderer whose width measure drives the word wrap, so wrapping always matches the blit. The narrator card guarantees four lines (usually five) at the larger size — budgets sit at ~345 chars (~70 tracked chars/line), the floor omen drops its redundant "Depth N" prefix to keep the reveal sentence, and completed narration tail-follows so the reveal stays visible even on the smallest windows.
- **Cutscene texts run one sentence longer and read more revealing**: every dilemma carries a required plain-spoken `truth` sentence (≤110 — the myth stated, not riddled) that rides in floor omens and the story panel (new `StoryBeat.truth` field, save-compatible default); guest speeches grew to two sentences (≤160, the second reveals who they are or what's really wanted); tether beats to ≤180 (Caldus names the signet, Pell admits brokering the sale, the Child lived in the Acolyte's tower); the Tyrant's depth-8 offers now speak their cost aloud (≤180); the depth-1 prologue gains the fixed myth line ("Below lies Kasar Voss, the drowned city…"). Budgets in `build/story/50-cutscene-style.md` updated to the new law and enforced in CI.
- Interaction-prompt story hint shortened to a ≤48-char key legend (the mobile prompt shows two 24-char lines and silently dropped ~45% of the old text).
- `story_intro_lines` fallback overlay and the story panel follow the same recap ban: backstory + objective render on depth 1 only.
- Final-boss naming: themed Gate Tyrant titles no longer prefix the faction epithet; knowing the true name prepends "Sorn Voss," instead.
- Runtime/package release version is `4.9.0`.

### Fixed

- **Enemies no longer get stuck in bar props, doorways, or behind each other.** Four cooperating fixes: (1) enemies spawned before special rooms placed their solid furnishings could stand inside a barrel/table collision box forever — a deterministic post-populate sweep (`_relocate_enemies_off_furnishings`) nudges them to the nearest free spot; (2) the nav distance field now seeds from furnishing-tile origins, treats an actor's own uncosted tile as infinite cost (routing off the walkable inset strip instead of giving up), and probe-filters descending neighbors so field descent never steers into a furnishing collision corner (with a strict descent constraint so the filter can't ping-pong across a tile boundary); (3) the separation fan-out only applies to greedy open-floor movement — routed (field-descent) movement stays pure so pairs single-file through doorways instead of wedging against the jambs; (4) stalled advances slide perpendicular past the obstruction, and a net-displacement stuck probe (~0.9 s window, speed-scaled threshold) catches oscillating wedges the per-frame checks can't see, arming a sustained chokepoint yield: the enemy farther from the target clears away along the contact normal so the closer one takes the gap — which also ends "only the closest enemy attacks" pile-ups, since rear enemies now orbit into flanks. New transient AI fields (`yield_timer`, stuck-probe reference) are session-only like `nav_latch`. Regression suite: `tests/test_enemy_unstuck.py` (furnishing relocation, strip routing, doorway funnel simulation, packmate flanking simulation).
- Cutscene choice-row width mismatch: row heights were measured at a fixed-inset ~776 px estimate while the authored nine-slice panel drew at ~720 px, letting long labels wrap after the height was fixed and bleed into the next row. Width now derives from the real panel content box and iterates height↔width to a fixpoint.
- Authored choice panels no longer require a per-verb icon: keyless continue-style choices ("Turn the page", "Ring the tenth bell") keep the authored panel with the number badge filling the icon socket.
- Selected-choice badge highlight: the old circular glow was drawn with `pygame.draw.circle` whose alpha is ignored on opaque surfaces, so it rendered near-solid and bled over the panel frame and corners. It is now a translucent rectangular box in the choice color, sized from the panel's own nine-slice geometry (frame inset to the socket divider), so it always sits inside the artwork.

### Validation

- `python -m unittest discover tests` — 889 tests, all passing.
- Desktop render snapshots (gameplay/crowd) intentionally rebaselined for the new theater text; determinism re-verified across two processes before rebaselining.
- Headless screenshot review of prologue, omen choice, tether dialogue, gate question, ending, bell, and both end-screens (`build/story/shots/`).

## 4.8.12 — Let there be light

Release 4.8.12 makes the Arcanist's Nova discipline path the dedicated Frost Nova widening route. Every acquired Nova-path node now adds one radius step (+0.55 tiles on the 2.45-tile base), so each pick visibly reaches farther — the cast ring expands to the true blast edge — and the fully mastered path more than doubles the nova's reach. The old deepest-node-only radius ladder is gone; equipment class-skill affixes stack on top unchanged. Mastering the Nova path **plus a second full path** — the run's entire two-path commitment — unlocks the max effect: Frost Nova engulfs the whole room the Arcanist stands in, catching every foe in it regardless of distance, and the chamber flashes in light. The flash is a wave that originates at the caster: it bursts outward at 26 tiles/s along a flood-filled region bounded by walls and doors (open or closed — door tiles catch the light but never pass it), holds the whole area bright, then fades out together. Because the region is a flood fill rather than a room rectangle, the wave fills rooms, corridor stretches, and both sides of a doorway threshold alike. Travel is bounded in two tiers: inside the caster's room the wave always reaches the far corner (the engulf damage does, so the light must too), while corridor casts and spill through doorless room openings stop after eight steps — enough to cover the maxed 5.2-tile blast radius along the worst L-shaped corridor path, without the light crawling down distant hallways. On light floors the flash honors fog-of-war memory; on dark floors it deliberately illuminates the entire chamber — the payoff of the max effect. Foes outside the room (or in corridors, which have no room to engulf) still need the numeric radius.

### Added

- **`nova_radius()` / `nova_engulfs_room()`** (`combat/attacks.py`): the blast radius derived from `path_progress` over the Nova path plus equipment affixes, and the max-effect predicate (`"Nova"` among `completed_paths` and at least `MAX_COMMITTED_PATHS` paths complete). `player_cast_nova` hits room-mates beyond the radius via `room_at` identity while keeping the line-of-sight gate. The cast ring stays at true blast size even at the max effect: room-scale impact overlays rasterized multi-thousand-pixel surfaces every frame and planted their perspective ground ellipse (offset by `radius // 3`) tiles below the caster — the room-filling spectacle belongs to the light wave.
- **`Dungeon.room_region_distances()`**: BFS step-distances over the contiguous chamber around a point — walls block, door tiles join the region but never expand (starting on a door floods both adjoining spaces), capped by the caller. The flash caps at the room's corner-to-corner walk inside the caster's room and trims everything else to `LIGHT_ROOM_FLASH_SPILL_DISTANCE` (8 steps).
- **Room flash wave** (`trigger_room_flash` in `game.py`, stamped by `rendering/lighting.py`): the flood-filled region (tile → wavefront distance), caster anchor, fade clock, and tint; decays with the other visual effects and clears with `reset_transient_visuals` (floor transitions, cutscenes, loads). `_stamp_room_flash` additively lights each region tile once the wavefront reaches it (per-tile attack ramp, global end-of-life fade) using the same per-tile rect approximation as the ambient wash; the wave joins the accelerated-mobile retained-frame signature at ~30 Hz quantization so GLES frames rebuild while it expands and fades. New `LIGHT_ROOM_FLASH_*` constants in `constants.py`.
- **Co-op replication**: new `"rf"` fx event carrying only the anchor point, tint, and total fade clock; the joiner re-runs the same flood fill on its dungeon copy in `_apply_fx_events`, exactly once per sequence number, and the wire ttl keeps both sides fading in lockstep.
- **Character sheet**: the Arcanist's class-skill stat line now shows the nova's reach — the radius in tiles, or `Room` at the max effect.

### Changed

- Nova-path discipline descriptions (Permafrost Sigils, Glacial Tide, Blizzard Heart, Absolute Zero) advertise the per-node widening, and Absolute Zero documents the second-path room-engulf finisher.
- Runtime/package release version is `4.8.12`.

### Validation

- `pytest tests/test_skill_paths.py` — 4 new tests: cumulative per-node radius growth, a Nova pick extending a real cast past a fixed foe, the room engulf unlocking only with two full paths, and the engulf not reaching foes outside the room.
- `pytest tests/test_lighting.py` — 9 new tests: max-cast flash trigger (pre-max no-op, corridors flashing too), doorway regions flooding both sides while doors never pass light, full room coverage beyond the spill cap with corridor spill trimmed to it, decay + transient reset, pixel-level stamp containment (door tiles lit; walls, beyond-room tiles, and never-revealed fog tiles dark), the wave visibly originating at the caster before reaching far tiles, dark-floor full illumination, and a draw-pipeline smoke pass with an active flash.
- `pytest tests/test_mp_action_visuals.py` — the `"rf"` event records on the host, resolves against the joiner's dungeon exactly once, and single-player records nothing.
- `python -m compileall src tests` — OK.
- Full suite: 852 tests + 569 subtests, all passing.

## 4.8.11 — The way ahead

Release 4.8.11 ships the desktop minimap: a player-centered, isometric-projected map card in the top-right HUD corner, toggled with Ctrl+M and persisted across sessions. The projection uses the same 2:1 iso axes as the world, so up-right on screen is up-right on the map. Terrain honors the existing visibility contracts exactly — light floors draw remembered fog-of-war tiles from an incrementally-updated cached surface (bar and garden interiors tinted in their identity colors, doors and stairs picked out in gold), while dark floors keep their zero-memory lantern model and render only the live visibility ring each frame. The Wayfarer's Bar, the Overgrown Garden, and the stairs earn glyph markers (tankard, sprout, descending steps) once discovered, edge-clamped with a direction arrow when they scroll off the card — so a wounded run can always find its way back to a refuge. When the story relic's guiding light is active, the minimap traces its full BFS route as a pulsing gold thread and marks the relic with a radiant beacon, even into unexplored dark. Co-op partners appear as blue diamonds beside the player's parchment one. Everything on the card is procedurally drawn in both graphics modes; only the panel chrome swaps to the authored `hud.panel` art on modern graphics. The mobile minimap is deliberately deferred — the mobile HUD path never reaches the card.

### Added

- **`rendering/minimap.py` (`RenderingMinimapMixin`)**: the minimap card, composed into `RenderingMixin` and drawn from `_draw_ui_fitted` after the run header / story panel / boss bar (post-lighting, desktop-only by construction). Publishes `self._minimap_rect` for HUD-avoidance like the other cards. Layout gives way gracefully: the card shrinks under the boss plaque row during boss fights (`boss_bar_top()` clamp) and suppresses itself entirely on windows too narrow to fit beside the run header (`MINIMAP_MIN_W`) rather than shrinking into noise.
- **Terrain mirror**: one long-lived `self._minimap_cache` dict (registered in the `_invalidate_render_caches()` seam) holds a full-floor iso terrain surface mirrored incrementally from `revealed_tiles` — only the delta since the last frame is rasterized, and a fog wipe (dark toggle, MP joiner rebuild) restarts the mirror and its discovery flags. Floor identity, tile scale, and theme accent key the cache; new run / descent rebuilds from scratch.
- **Markers** (`minimap_markers()`): discovery-gated `(kind, x, y)` list for `bar` / `garden` / `stairs`. Light floors remember a discovery once any interior tile is revealed; dark floors only report markers inside the current lantern ring, preserving the no-memory contract.
- **Guiding light** (`minimap_guidance()`): reuses the exact renderer activation predicate (`story_relic_target_position()` + `story_relic_guidance_enabled` + not `story_intro_pending`) and the memoized `story_relic_guidance_route()` BFS — no new pathfinding. The route thread draws on its own alpha layer so its translucent glow blends over terrain instead of punching through it.
- **Ctrl+M** in `handle_events`, placed with the other global chords (above the state branches, below confirm-exit/Escape): flips `minimap_visible` via the new `toggle_minimap()` helper, which saves options immediately. The options-menu bare-M music toggle now requires no Ctrl so the chord stays inert there. Documented on the controls screen (`KEYBOARD_ROWS`), in the desktop Run Guide, and in the HUD control hint line.

### Changed

- **Options schema v9**: `minimap_visible` (default on) joins the options file; pre-v9 files migrate to visible. Run-save schema untouched.
- Runtime/package release version is `4.8.11`.

### Validation

- `python -m unittest tests.test_minimap` — 14 new tests: Ctrl+M toggle + options-file round trip, bare-M/title/options-menu inertness (including the music-toggle guard), rect publication and top-right anchoring, hidden/mobile/narrow-window suppression, boss-bar shrink and give-way, incremental mirror growth + floor reset, cache invalidation seam, legacy-graphics draw, bar/garden/stairs discovery gating with per-floor memory, marker positions, dark-floor lantern gating with fog memory untouched, and guidance predicate + route.
- Schema-version assertions in `test_input_and_accessibility`, `test_frame_pacing`, `test_archetypes_options_and_difficulty`, `test_sprite_assets`, and `test_mobile_layout` updated 8 → 9.
- `python -m compileall src tests` — OK.
- `python -m unittest discover tests` — 849 tests, all passing.

## 4.8.10 — Glory to the machine, Part B

Release 4.8.10 ships Part B of the machine-intelligence milestone: friendly NPCs join the fight. Greet the bar wayfarer, the garden wanderer, or the bar dancer (a new "Greet" prompt just below the Lossless Soul's slot) and they take up arms beside you for the rest of the floor — the sturdy barkeep-blooded patron as a bruiser, the gardener as quick light melee, the dancer as a fast flurry of low-damage hits. The Story Guest joins on its own once you resolve its dialogue, and the Lossless Soul raises ranged ward-bolts once her reflection is answered — both derived from the story flags that already persist, so no new story state is written. Allies fight on a 9-tile leash around their home anchor with the same distance-field cornering enemies learned in 4.8.9, route their kill XP, gold, and credit to the player who greeted them, walk home and resume their beat-wander when the room falls quiet, and can genuinely die: enemies target the nearest of players and aggressive NPCs with the full 4.8.9 tactic/ability machinery, enemy bolts strike allies in the same pass that guards familiars, and a fallen friend is simply gone. Garden frogs and shopkeepers remain pacifist, permanently; un-greeted NPCs keep their old untouchable wander, byte for byte.

### Added

- **Combat model on friendly NPCs** (`IdleNpc`/`StoryGuest`): appended defaulted fields `max_hp`, `hp`, `damage`, `attack_range`, `attack_cooldown`, `combat_speed`, `aggressive`, `greeter_id`, `home_x`, `home_y` plus transient `attack_timer`/`nav_latch`/`combat_active` (`_TRANSIENT_NPC_FIELDS`, reset on load). `max_hp == 0` means "stats not rolled yet"; stats roll lazily on recruitment from the authored per-kind table in `combat/npc_allies.py`, scaled by depth (+12%/depth) and difficulty (enemy multipliers at half weight).
- **Greet interaction** (`nearby_greetable_npc` / `greet_idle_npc`): kinds `bar`, `garden`, `bar_dancer` at radius 1.25, hint and interact priority just below the Lossless Soul; flips `aggressive`, records the greeting player for kill credit, and speaks one authored line as a floater — no cutscene. Greetable NPCs now hold and face an approaching player like the guest and keeper do; once recruited they dance freely again.
- **Ally AI** (`combat/npc_allies.py`, `update_npc_allies` next to `update_familiars` in the update order): aggressive NPCs leave the music-beat wander, pick the nearest living enemy with line of sight inside the 9-tile home leash, chase it through the shared distance-field/greedy movement seam (with the 4.8.9 wall-stall latch), attack on their own timer through `DamageContext`/`damage_enemy` inside `acting_as_player(player_for_credit(greeter_id))`, and walk home to resume the wander when no enemy remains in leash. The Soul's ward-bolt rides the existing player-projectile pass (`owner="player"` + the greeter's `owner_id`), so damage, credit, and delayed kills reuse proven code.
- **Enemies aggro back**: `enemy_target_player` generalized to `enemy_target` — nearest of living players ∪ aggressive living NPCs, players winning equal-distance ties (pre-4.8.10 co-op behavior verbatim). Aggro, tactics, abilities, boss rotations, and the distance field all operate on the chosen target; per-NPC/per-enemy fields build lazily through the generalized `_nav_field_for_target` (same tile-crossing + stale-time throttle, entries dropped on death and pruned across floors). Melee/cast executors branch to the new `take_npc_damage` sink for NPC victims, novas catch allies in radius, enemy projectiles intercept aggressive NPCs in the same pass that checks familiars, and enemies body-block against allies. Familiars stay non-targeted (retaliation only), and pacifist kinds are never targeted.
- **Multiplayer replication**: `idle_npcs` and `story_guests` enter the slow snapshot block (combat fields, positions, and guest resolution flags), the NPC rosters and their aggressive counts join `world_list_lengths` so a greeting, story resolution, or ally death forces a prompt slow payload, and NPC positions ease through `lerp_networked_actors`. Identity-stable rosters update in place (no motion-cache churn); a death rebuilds the joiner list once. Greeting rides the existing `interact` intent — **no wire change, no `MP_PROTOCOL_VERSION` bump**.

### Changed

- `resolve_story_choice` arms the resolved guest and `resolve_lossless_soul_choice` arms the hall's keeper (greeter = the resolving player). On load, `_reconcile_npc_ally_state` re-derives guest aggression from `resolved` (a forsaken "unanswered" beat never fights) and keeper aggression from the hall's `soul_choice`, rolls stats where older flags predate the ally fields, resets transient ally state, and clamps allies out of solid furnishings.
- Run-save schema stays 5: every new field is additive-with-default (`familiar_to_dict` precedent), so pre-4.8.10 saves load with pacifist NPCs and new saves persist recruited allies with their wounds.
- Runtime/package release version is `4.8.10`. Options schema unchanged.

### Validation

- `python -m unittest tests.test_npc_allies` — 21 new tests: pacifist old-save load, transient-field exclusion, greeted-ally save/load round trip, depth/difficulty stat scaling, greet flow (hint, flag, credit, home anchor), frog never greetable, ally kill XP/gold/credit to greeter, leash + walk-home stand-down, chase closes distance, wander skips fighting allies, soul ward-bolt ownership, ally death removal, projectile interception (aggressive yes / pacifist no), enemy retarget to ally and back on its death, full-update pursuit of the nearer ally, nova splash, soul-choice and guest-resolution derivations, world-length change signals, and joiner snapshot round trip incl. roster shrink.
- `python -m unittest tests.test_friendly_npcs tests.test_bar_toast tests.test_enemy_tactics_and_abilities tests.test_enemy_los_walls tests.test_enemy_knockback_and_attack_windup` — wander, toast (helper now greets a bar regular before drinking when one stands in reach), and 4.8.9 AI contracts all hold.
- `python -m compileall src tests` — OK.
- `python -m unittest discover tests` — 835 tests, all passing.

## 4.8.9 — Glory to the machine

Release 4.8.9 ships Part A of the machine-intelligence milestone: enemies now fight by role and ability instead of one shared chase-and-attack loop. Skirmishers orbit, flankers curve onto your side, marksmen fight from a long firing line and sidestep for a shot, sentinels plant themselves at the first spot they can already fire from, and the five named bosses weave their authored signature attacks — ember cleave and nova, spore volleys, frost fans, arcane lances, alternating gate strikes and shadow volleys — into the familiar pressure pattern, recovering them faster below half health. Every doctrine is aggression-biased: a hold is only ever allowed where the enemy can actually attack from, so no tactic can leave an enemy standing idle. Enemies path around walls instead of grinding along them (with a collision-feedback latch that catches the near-corner cases geometry can't), spread out instead of funneling into one shoving column, wake their packmates when they spot you, chase your last known position for a few seconds instead of freezing at the aggro edge, and always turn on whoever hurt them — sniping from beyond aggro range is no longer free. Part B (friendly NPCs joining the fight) follows in the next release.

### Added

- **Ability framework** (`EnemyAbility` in `content/definitions.py`, authored table + `ABILITY_INDEX` in `content/enemies.py`, resolution in `combat/abilities.py`): four effect kinds — `strike` (single-target melee hitscan), `bolt`, `fan` (n bolts across a spread, generalizing the old hard-coded boss 3-bolt fan), and `nova` (radial LOS-checked burst) — dispatched through the existing windup/commit seam. `windup_attack` now carries an ability key; the reserved keys `"melee"`/`"cast"` name the defaults synthesized from an enemy's stat columns, so enemies without an authored list behave exactly as before, and existing windup telegraphs, snapshots, and tests carry over unchanged. Authored abilities may override the per-kind windup and run their own rotation cooldowns (`Enemy.ability_timers`, transient) on top of the global `attack_timer` recovery gate, and spend the rotation cooldown at fire time so a stun that cancels the windup does not burn the ability.
- **Boss ability rotations**: each `BossDefinition` carries authored ability keys making its telegraph flavor mechanical — Ash Gallows Knight (ember cleave strike + ember nova), Mycelial Matron (poisoning spore volley), Rime Chanter (five-bolt chilling frost fan), Voidbound Rune Sentinel (slow heavy arcane lance), Dread Gate Tyrant (gate strike / shadow volley alternation via "prefer a different ability than last"). With every authored ability on cooldown or out of band, bosses fall back to the legacy cast-band/melee pressure, so they are never disarmed. Below half HP, authored cooldowns recover at ×0.8.
- **Per-role tactics** (`Tactic` in `content/definitions.py`, `TACTICS` table in `content/enemies.py`): the previously cosmetic `Enemy.role` now drives movement doctrine. Bruisers/maulers/guards close directly (unchanged from the legacy chase); flankers curve sideways onto the target's flank while closing; skirmishers hold a mobile 2.0–3.0 band and orbit while their attack recovers; casters/marksmen/artillery hold role-specific kite bands (3.4–4.6 preferred) and sidestep for a firing line when attack-eligible but wall-blocked (bounded two-candidate probe); sentinels plant at the first spot the target is already inside attack range instead of closing further. `hold_anchor` is only ever honored where the enemy can actually fire from its post — an early stand-your-post guard/sentinel doctrine could leave damaged enemies statue-still just outside melee reach and was cut for direct aggression. The old inline literals (ranged 2.5–3.5 band, boss 2–6 cast band, 0.28 fan spread, 6.0 bolt speed) all moved into these tables; unknown roles and directly-constructed test enemies fall back to tactics carrying the exact legacy values.
- **Distance-field pathfinding** (`combat/pathing.py`): a classic Dijkstra map — one radius-limited 8-connected BFS from each living player's tile, shared by all enemies, recomputed only when that player crosses a tile boundary (plus a 0.5 s stale-time refresh that picks up opened doors and boss-room seals). Enemies whose route detours around walls descend the field tile by tile and finally corner properly; enemies with a straight route keep the legacy greedy step (in open rooms the BFS cost equals the Chebyshev estimate exactly, so open-room behavior is byte-identical). Around a *near* corner that equality hides the wall, so a **wall-stall latch** (`Enemy.nav_latch`, transient) uses collision feedback instead of geometry: an advance step that barely moves arms ~0.8 s of preferring field descent, steering the enemy around the corner (or around the packmate blocking a doorway). The field feeds directions into the existing `_move_enemy_locomotion` → `move_actor` seam — collision, wall-slide, and animation scaling untouched — and 2×2 bosses keep the greedy step. Consumes no RNG and never traces LOS.
- **Separation steering**: an advancing enemy overlapping a packmate within 0.9× combined hit radii blends a small away bias into its step so packs fan out; `resolve_actor_contacts` remains the hard backstop.
- **Perception** (`Enemy.ai_state` idle/engaged + target memory, all transient): entering the engaged state alerts idle packmates within 4 tiles (one hop, no chain); taking damage always engages and pack-alerts via `alert_enemy` in `damage_enemy`; and when the target leaves aggro range an engaged enemy drifts to the last noticed position for up to 4 s before standing down, instead of freezing mid-stride at the radius edge. Radius aggro, the movement-not-LOS-gated pursuit contract, and the trace-LOS-only-when-attack-eligible perf contract are all preserved.

### Changed

- `EnemyDefinition`/`BossDefinition` gained appended defaulted `tactic`/`abilities` fields (positional call sites untouched); `Enemy` gained persisted `tactic`/`ability_keys` identity fields and transient `ability_timers`/`ai_state`/`alert_timer`/`nav_latch`/`memory_x`/`memory_y`/`strafe_sign`/`last_ability` state, all defaulted.
- `_move_enemy_locomotion` now returns the distance moved (used to flip strafe/reposition direction at walls); `enemy_melee`/`enemy_cast` accept an optional `ability` and a new `enemy_nova` executor joins them.
- Strafe/flank side and boss rotation are deterministic (position-derived sign, tuple-order rotation): the new AI consumes no extra `self.rng` draws, so floor-generation and combat RNG stream order is unchanged.
- Runtime/package release version is `4.8.9`. Options and run-save schemas are unchanged.

### Compatibility and resilience

- No save-schema bump: the new `Enemy` fields are additive-with-default (old saves load as pacifist-identical enemies with synthesized default abilities; new saves persist only the identity fields — all AI state is in `_TRANSIENT_ENEMY_FIELDS`). `ability_keys` normalizes list→tuple on restore beside the existing color normalization.
- No multiplayer wire change: the joiner never simulates AI, windup telegraphs already ride `enemy_compact_dict`, authored-ability windup keys are just strings in the existing field, and fan/nova visuals ride the existing projectile/fx replication.
- Pre-4.8.9 behavior is pinned where it was load-bearing: default-role enemies keep the legacy bands and cadence, the boss legacy fan remains the fallback (test updated to park the authored rotation on cooldown), and the lure, windup, knockback, stun, and Time-Skip paths are verbatim.

### Validation

- `python -m unittest tests.test_enemy_tactics_and_abilities` — 22 new tests: transient-field registration, authored windup override, independent ability cooldowns, boss rotation alternation and legacy fallback band, phase cooldown scaling, fan projectile count + status riders, nova radius/LOS/wall behavior, marksman/sentinel/guard/flanker/skirmisher tactic behaviors (including the damaged-guard-must-close and sentinel-plants-at-firing-range regressions), legacy band for default roles, LOS reposition sidestep, distance-field cornering around a wall, the near-corner wall-press latch regression, damage aggro beyond aggro range, one-hop pack alerts, and target-memory expiry.
- `python -m unittest tests.test_enemy_los_walls tests.test_enemy_knockback_and_attack_windup tests.test_boss_encounters tests.test_time_skip` — all legacy AI contracts hold.
- Headless scenario replay of the reported stall (two damaged guard-role Hollow Knights ~3.5 tiles from a stationary player): both close to melee contact and land hits within 5 simulated seconds.
- `python -m compileall src tests` — OK.
- `python -m unittest discover tests` — 814 tests, all passing.

## 4.8.8 — The cursor bows to the hand

Release 4.8.8 brings full desktop mouse support to every menu: hovering a row moves the selection highlight and a left click activates it, across the title screen, options, controls, archetype select, exit confirmation, multiplayer screens, story choices, and the in-game inventory, shop, and character overlays. Keyboard, controller, and mobile touch input are untouched.

### Added

- **Desktop mouse menus** (`handle_menu_mouse_motion` / `handle_menu_mouse_click` on `InputMixin`): two pointer handlers wired into the event loop that reuse the render-published hitboxes mobile taps already consume (`_title_row_rects`, `_menu_row_rects`, `_controls_gamepad_row_rects`, `_mp_row_rects`, choice rects, inventory/shop rows and chips). Hovering moves the active menu's cursor to the row under the mouse — the existing selected-row rendering does the highlighting — and keyboard/controller navigation simply continues from wherever the mouse last pointed.
- **Click activation per menu**: title rows (disabled Resume stays inert), options rows (toggle/cycle exactly like Enter), controls rows (click begins a rebind capture), exit-confirmation rows, cutscene and story-intro choices (clicks also page incomplete narration, mirroring taps), multiplayer consent/setup/lobby rows (the setup and lobby clicks route through the existing tap handlers, which also carry the archetype carousel arrows — the old dedicated desktop arrows branch is folded in), character sheet tabs, inventory sort chips, and shop BUY/SELL chips. Clicks on the about, help, and death/victory screens continue past them like Enter.
- **Double-click confirm on selection-preview lists**: archetype select, inventory rows, and shop rows treat a single click as select/preview and require a second click on the same row within 0.45s to commit (start the descent, use/equip the item, transact) — a stray click can never overwrite an existing run save or spend gold. A live text-entry session blocks menu clicks so typing can't trigger rows underneath (except multiplayer setup, where clicking outside the field confirms like Enter).

### Changed

- The lobby archetype carousel's ‹ › handles now route through the shared lobby click handler instead of a dedicated `mp_lobby`-only mouse branch; behavior is identical.
- Runtime/package release version is `4.8.8`. Options and run-save schemas are unchanged.

### Compatibility and resilience

- Both handlers are desktop-only (`not mobile_mode`, non-touch events): mobile overlay hitboxes live in safe-area coordinates that raw display positions would miss, and the touch path already covers every target. Synthesized touch-mouse events keep their `touch` flag guard.
- Gameplay clicks are untouched: unconsumed clicks fall through to the existing aim/melee and discipline-cell branches, and mouse motion still drives aiming in every non-menu context.

### Validation

- `python -m unittest tests.test_mouse_menus` — 12 new tests covering title hover/click with the disabled Resume row, options hover plus row activation, archetype preview-then-double-click descent, cutscene click-to-advance and choice clicks, inventory hover/double-click-confirm and sort chips, shop row selection and mode chips, character tabs, exit confirmation, multiplayer consent rows, gameplay/overlay fall-through, text-entry blocking, and end-to-end event-loop routing of real MOUSEMOTION/MOUSEBUTTONDOWN events.
- `python -m compileall src tests` — OK.
- `python -m unittest discover tests` — 792 tests, all passing.

## 4.8.7 — A toast in every hall

Release 4.8.7 gives the Wayfarer's Bars a reason beyond shelter: every bar now taps exactly one of its ale barrels for a single free drink, and a pilgrim who toasts every bar the run generates is joined on the final depth by the immortal Bar Dancer — a familiar companion who dances at rest, follows on the road, and cannot be killed at the gate tyrant's side.

### Added

- **Tapped ale barrel** (one per bar): each bar room designates a single drinkable barrel among its placed `bar_barrel_*` furnishings. The pick re-derives deterministically from the layout-seeded flavor RNG over the persisted anchors — no save-schema change, and saves, reloads, and co-op floor syncs always agree; only the one-drink outcome is stored (`SpecialRoom.state["barrel_drunk"]`). While the drink is unclaimed the barrel is findable at a glance: warm ale motes rise off the bung and a candle stub burns beside it as a real static light source (`kind="bar_tap"`, dark-floor-safe like the soul-prop auras, rebuilt from anchors on every load).
- **Drink interaction** (`E — Drink from the tapped barrel`): standing beside the tapped barrel offers a one-time drink per bar — a small swig of health and stamina, a toast floater, and a new low `drink` tone. The prompt's detail line tracks the pilgrimage (`this toast makes it 2/2 bars this run`), and once the tally is even the room's dancer marks your toast. The interact priority slots after shrines; a spent barrel never prompts again.
- **Toast pilgrimage tally** (`RunStats.bars_visited` / `bars_toasted`): bars are counted once when their floor is generated (restart/descend — never on load, so reloading cannot double-count), toasts on each drink. The pair persists through the existing `run_stats` serialization, replicates to the co-op joiner through the slow snapshot (`bar_toast`), and surfaces as a "Bars toasted" row in the death/victory statistics table's Exploration panel.
- **The immortal Bar Dancer companion**: arriving on the final depth — or completing the tally in a bar that rolled on the final floor itself — with every generated bar toasted summons the Bar Dancer as a familiar (`kind="bar_dancer"`, 46 HP / 7 damage / 3.5 speed). She is `unkillable` under the Eternal-Owls contract (damage floors at 1 HP, regenerates between fights), fights with the shared familiar follow/attack AI dealing physical damage, belongs to the summoning player in co-op, and persists through save/load via the normal familiar serialization. On screen she reuses the bar NPC's authored actor art wholesale — dancing in place while idle, walking when following — grounded on the shared feet line with a warm amber HP bar. The pact is strict: a run that rolls zero bars, or misses a single toast, earns nothing.

### Changed

- The run statistics table grew from 24 to 25 rows: Exploration carries the new "Bars toasted" row (7 rows), and the Legacy panel's index map was corrected so the Mastery row survives the insertion.
- Runtime/package release version is `4.8.7`. Options and run-save schemas are unchanged.

### Compatibility and resilience

- No save-schema change: the tapped pick is derived, the drunk flag rides the existing `SpecialRoom.state` bag, the tally rides `run_stats`, and the dancer rides the familiar list. Pre-4.8.7 saves load with zero-initialized tallies and their current floor's bar immediately drinkable; bars from floors created before the update simply weren't counted, so the pilgrimage starts from the next generated floor.
- Multiplayer: the host resolves drinks like every world interaction; the joiner's tapped-barrel hint, motes, and tally stay honest through the additive `bar_toast` slow-snapshot entry (unknown to old clients, ignored gracefully), and the summoned dancer replicates through the existing familiar sync.

### Validation

- Rendered in-game verification of the tapped barrel (motes + candle beside the wall sconce), the after-drink state, and the dancer dancing beside the player in the bar.
- `python -m unittest tests.test_bar_toast` — 9 new tests covering the deterministic tapped pick across seeds, one-drink enforcement with counters and heal, save/load round trips of pick/flag/tally, the candle fixture, the full toast-every-bar run earning the immortal dancer (damage floor, regen, no duplication, save/load), the missed-bar and zero-bar denials, the final-floor-bar on-the-spot summon, and the familiar's use of the Bar Dancer art.
- `python -m compileall src tests` — OK.
- `python -m unittest discover tests` — 780 tests, all passing.

## 4.8.6 — The hall keeps every memory

Release 4.8.6 introduces the Lossless Soul: a new friendly NPC with her own sealed special room — the Hall of Unlost Echoes — rolling at 50% on every depth. She dances a slow ritual sway among her memory furnishings, pauses to face an approaching player, and offers a story-conditioned reflection cutscene whose answer marks the run. Underneath, special-room placement and theming moved onto declarative registries so future rooms plug in as data.

### Added

- **Lossless Soul keeper NPC** (`IdleNpc` kind `lossless_soul`, "Keeper of Unlost Memory"): one guaranteed keeper per hall with full-resolution `244×244` PixelLab rotations plus `walk` (6f) and `dance` (8f) clips, beat-locked to the floor music like every friendly performer. She holds and faces the player within reach until the hall's reflection has been answered, and a procedural pale-veiled fallback covers legacy graphics.
- **Hall of Unlost Echoes special room** (`lossless_soul`, sealed, `spawn_policy="safe"`): rolls at 50% on every depth from the layout-seeded flavor stream — bar/garden placements for a given seed are byte-identical to 4.8.5, with the hall's roll appended. The interior gets seal-engraved floor art, chained memory-seal wall faces on interior sides only, and four furnishings on deterministic geometry-derived anchors: the cracked memory mirror, a memory-chime stand, a teal-flamed soul brazier (with a matching flickering light), and the chained reliquary. The furnishings are dedicated tile-fitting isometric block props in the shrine/world-prop idiom — the flat portrait art is reserved for the cutscene stage, which uses its own prop set. Each prop is physically solid — its pedestal blocks every movement probe (players, enemies, NPCs, familiars, spawn scans) through an inset collision box in the stairs-shaft idiom that is also shifted north-west (`SOLID_FURNISHING_COLLISION_OFFSET_*`, ~-0.45/axis) to cancel the two screen-space offsets between drawn pixels and world coordinates (the art's footprint-centered anchor and the actors' grounded feet line): approaching from the south the player's feet now stop at the pedestal's visible base instead of half a tile of bare floor, and from the north they can no longer slip visually inside the top face. The box may spill into a furnishing's north/west neighbor tiles, so placement also keeps props off the tiles south/east/southeast of a doorway tile; LOS and projectiles are unaffected. The props render at furniture scale with the shared pedestal base centered on the tile and spanning roughly 3/5 of it, and each casts its own faint static memory-light (kind `soul_prop`: teal glass, moon-silver chimes, violet ward, plus the brazier's brighter flickering flame) as a real light source, so the glow participates in the lighting pass instead of being crushed by it wherever the lantern doesn't reach. Because the furnishings are solid, placement excludes the tiles just inside each doorway — a prop whose geometry-derived spot would wall off the entrance slides to the nearest free interior tile (~13% of halls), and reconciliation applies the same rule (and rebuilds the furnishing lights from the final anchors) on every load.
- **Bar furnishings** (`bar_barrel_1..4` / `bar_table_1..4` anchors): every bar now hosts 2-4 solid ale barrels along the walls and 2-4 standing tables in the open floor, count scaling with room size. Placement rolls once per room from the layout-seeded flavor stream and persists via the reserved anchors; it avoids doorway tiles and their south/east/southeast neighbors (which the shifted collision boxes would spill into), reserved tiles and existing NPCs, and keeps furnishings a Chebyshev distance of at least 2 apart so every aisle stays walkable. The props share the soul-hall furnishing contract exactly — registered in `SOLID_FURNISHING_ANCHOR_KEYS`, blocking through the same shifted/inset boxes, transparent to LOS and projectiles — and `_reconcile_bar_dancers` backfills them into existing saves on load (before the dancer, so a backfilled dancer spawn avoids the furniture).
- **Authored choice glyphs for the reflection**: the three answers carry dedicated 32×32 badge icons (`cutscene.choice.icon.soul_{preserve,release,refuse}` — a chain-bound teal mirror, an opened link releasing a wisp, and a barred dark mirror) with matching per-choice row tints, mirroring the aid/bargain/defy treatment.
- **Reflection cutscene** (`lossless_soul_reflection` · "Lossless Soul · Hall of Unlost Echoes"): a full authored stage — new Hall backdrop, the four furnishings as stage props (`mirror`, `chimes`, `brazier`, `reliquary` prop kinds), teal spot/wash lighting, drifting motes — where the keeper recalls the player's most recent *finished* story turning by title and outcome (or admits none has been made). On stage she faces the audience and lifts into her ritual sway in measured spells (a few bars of dance, then stillness) rather than dancing throughout. Three answers with lasting marks: **Preserve it exactly** (+`healing_echo`), **Release its hold** (−`enemy_pressure`), or **Refuse the mirror** (no boon, no copy). The choice writes a story flag and log line, persists on the room, and later visits go straight to a personalized "settled" audience.

### Changed

- **Declarative special-room placement**: `SpecialRoomDefinition` gained a `SpecialRoomPlacement` (chance, RNG stream, gating flag), and `Dungeon._plan_special_rooms` became one data-driven loop over the definition table. The shop/quest/flavor RNG streams and draw order are preserved exactly — verified against the 4.8.5 generator across 800 layouts with zero divergence.
- **`spawn_policy` is now enforced centrally**: `"safe"` rooms (shop, quest, soul hall) are cleared of hostiles and traps by the shared handler runner instead of ad-hoc calls inside individual populate functions.
- **Registry-driven interior theming**: the renderer's special floor/wall art selection, wall-face detection, and prewarm pass all iterate `_SPECIAL_FLOOR_ASSET_KEYS` / `_SPECIAL_WALL_ASSET_PREFIXES`; `tile_surface` takes a canonical `special_floor_kind` (legacy boolean kwargs still map onto it) and its cache key shrank accordingly.
- **Actors stand on their contact shadows**: every grounded actor (player, enemies, bosses, shopkeepers, story guests, idle NPCs, the spirit beast) now anchors its feet on the shared `ACTOR_FEET_Y_OFFSET` ground line — the same line `draw_shadow` centers the contact ellipse on. Sprites previously anchored 4–5 source px above it, so the detached shadow made actors read as hovering. Verified against the projection: the floor art's top-face diamond is centered exactly on the projected tile center, so this was purely a sprite/shadow offset mismatch, not a grid misalignment. Feet-tracking overlays (hit flash, garden heal glow, enemy windup ring, aim-cone apex, shopkeeper/guest floor rings) follow the same line; the wisp familiar intentionally keeps hovering above its shadow.
- Runtime/package release version is `4.8.6`. Options and run-save schemas are unchanged.

### Asset provenance

- PixelLab character `Lossless Soul` (high-resolution): `676b330f-d1e7-4fdb-8adb-c6d852e1d064`, `244×244`, 8 directions. The earlier `92×92` prototype `95104733-ef46-4762-a048-53bbe5d5c0c4` is retained but tagged `superseded, low-res-old` per the no-delete convention.
- `walk` group `68f5b7e7-6b65-476f-b1ed-e2bbb584f639`: template `walking-6-frames`, all eight directions; four directions were subsequently hand-corrected in PixelLab and the packaged frames refreshed from the corrected export.
- `dance` group `d3741d0d-287c-4f18-a479-2e985cd68292`: v3 custom, "performing a slow ethereal ritual dance, swaying gracefully in place with flowing arm movements, robes and long hair drifting as if underwater", eight directions; the static reference frame is trimmed at packaging so the clip loops continuously through 8 frames.
- Cutscene-stage furnishing objects (tag `lossless-soul`, `128×128`, flat portrait view): mirror `dcfed80d-0ff0-4218-9f94-fe64dfc13e28`, brazier `184e1532-c6cc-40c1-90ba-ebe28fc67927`, chimes `9933edad-09fe-48b2-8f53-61b352c5d115`, reliquary `c13bec83-5482-44b7-af82-4ea94ad5403c` — packaged as `props/lossless_memory_{mirror,chimes,reliquary}.png` and `props/lossless_soul_brazier.png`.
- In-room furnishing objects (tag `lossless-soul-room`, `128×128`, tile-fitting isometric block props styled on the shrine): picks packaged as `props/lossless_room_{mirror,brazier,reliquary,chimes}.png` from kept objects `a169b5b0-2248-4d00-af73-3b9c45a1b14b` (mirror), `1e938345-c94e-46ac-8e7c-11eb1e290d44` (brazier), `a82677d5-4c6d-4968-b842-f64a09d17d36` (reliquary), `4a174743-7b65-4e8d-8532-3cb3dab9b4c4` (chime stand); one runner-up per prop is retained under the same tag for easy swapping. The full 16-candidate review sheet is archived at `build/lossless-soul-486/room-prop-candidates.png`.
- Bar furnishing objects (tag `bar-props`, `128×128`, styled on the `lossless_room_{brazier,reliquary}` props so the shared pedestal template's base geometry carries over exactly): ale barrel pick `54ad7184-97fa-4b97-9bda-0dc517814dc4` (runner-up with spigot `a3c12d61-beb8-4109-b927-cb257708c566`), standing table pick `b27c6fed-eb96-426a-a7cc-961e53afeb22` (warm-toned runner-up `5af4f83d-f376-4d2e-8050-1b7e56503b9b`) — packaged as `props/bar_{barrel,table}.png`. A ready-made save landing the player inside a full 4+4 furnished bar is archived at `build/bar-props-save.json`.
- Choice glyphs (tag `lossless-soul-ui`, `32×32`, styled on the aid/bargain/defy badges): preserve `a291040f-f6b6-431b-a2f2-b3cd74f604b4`, release `48245981-945f-470b-812d-eb7969d0deda`, refuse `c1424757-14f1-46aa-acfc-5e9c728f429a` — packaged as `menus/story_choice_soul_{preserve,release,refuse}.png`, with one retained runner-up each.
- World art: `lossless_soul_floor` selected from a 16-candidate `64×64` run styled on the stock floor tile (kept as object `3e5415ba-6279-40cd-a0e7-98f3c05704f1`); `wall_lossless_soul_left`/`right` selected from per-side 16-candidate `68×68` runs styled on the garden wall blocks (kept as `f248d72b-e294-431f-94bf-06833ea0609c` / `7fdfd7ec-8a31-46e2-8e79-ea662ff3cdcc`), each verified pixel-for-pixel against the stock wall silhouette before packaging. An earlier full-block run that ignored the palette was dismissed; the freestanding wall-segment object from the reverted first attempt (thin-slab geometry, see `build/lossless_soul_scuffed_walls.png`) was not reused.
- Stage backdrop `stage.backdrop.lossless_soul` from UI asset `ae0a1383-77f4-493b-9b97-b6e3ef9e08d0` ("Lossless Soul — Hall of Unlost Echoes", `688×384`), cropped to the painted interior because the generated rounded-panel margins (checkerboard fill) leaked onto the stage under cover-scaling; corner checker remnants were inpainted from neighboring scene pixels.

### Compatibility and resilience

- Dungeon layouts for existing seeds are unchanged: shop, quest, bar, and garden rooms land in the same rooms as 4.8.5; only the appended soul-hall roll (and its own door seal) is new.
- Pre-4.8.6 saves with a persisted hall backfill exactly one keeper plus her furnishing anchors on load via `_reconcile_lossless_souls`, without touching gameplay RNG; repeated restores are idempotent. Saves without the hall load unchanged. Because the furnishings are now solid, a save written while their tiles were still walkable may leave a player standing inside one — such players are moved to the first safe adjacent tile on load, mirroring the stairs-shaft backfill.
- The hall's dialogue state rides the existing `SpecialRoom.state` bag and `idle_npcs` serialization — no save-schema change. Multiplayer floor sync carries the room, keeper, and state through the existing run-state serializers; her cutscene stays host-local like story-guest dialogue, and a remote actor's interact never opens the host's modal.
- Missing assets degrade gracefully: the keeper falls back to a bespoke procedural sprite, furnishings and themed floor/walls simply revert to stock art in legacy graphics, and the stage falls back to the procedural backdrop.

### Validation

- Rendered in-game verification of the hall (seal floor, interior-only wall faces, all four furnishings, keeper dance, talk prompt) and of the cutscene (backdrop, staged props, both performers, tailored narration, choices, settled revisit).
- Dialogue flow exercised end-to-end headlessly: hint → cutscene → tailored memory line → choice → room state + story flag + effect → settled node → save/load round trip → resolved-hall hint and direct settled revisit.
- `python -m unittest tests.test_lossless_soul` — 9 new tests covering definition, 50% placement without collisions, shared-RNG neutrality, idempotent population, central safe-clearing, sprite completeness and beat addressing, cutscene schema, save backfill, and render smoke.
- `python -m compileall src tests` — OK.
- `python -m unittest discover tests` — 766 tests, all passing.
...
- **The bar pours**: the bar flavor room is now a working refuge — standing inside heals at half the garden's strength (amber "Bar +N" floater) while quickly sapping the drinker's stamina to zero (faster than regen can fight; a full bar empties in about a second). Garden and bar share one per-player refuge-effect tick, so both work identically for either co-op player.

### Changed

- **Refuge healing is slower**: garden (and bar) heal ticks now land once per five seconds of lingering instead of once per second, so refuge rooms read as a breather rather than a heal station worth camping.

### Fixed

- **The joiner earned no XP or gold from delayed kills**: kill rewards went to whichever actor `self.player` pointed at when the enemy died, which during the host's update loop is always the host. The joiner was credited only for instant melee kills — bolt kills (the projectile resolves frames later), poison-tick kills, and familiar kills all paid the host, so a ranged or DoT-focused joiner could effectively never level (noticed after a Raise, when the missing level-up refill hurts most). Projectiles now remember their shooter, enemies remember their last attacker, and familiars bite as their summoner, so damage, kill XP/gold, Blood-path leech, chain lightning, and discipline bonuses all resolve as the player who earned them. A shooter who has since fallen defers the credit to the host — a level-up's full-heal must never revive a corpse mid-death-animation.
- **The joiner never healed in refuge rooms**: garden healing ran only inside the host's own `update_player`, and the joiner never simulates — so a partner standing in a garden (typically at half HP right after being Raised) got nothing. The refuge tick now runs for the remote actor in the host's remote-player update, with the heal accumulator moved from the Game object onto each player so the partners' progress can't reset each other. The greenish garden aura stays a local-screen effect; the partner sees the replicated floater and HP rise.
...
- **Connection consent screen** ("A Word Before the Gate"): shown before the first socket to a server each session — hobby-project/use-at-own-risk notice, the exact endpoint with its encryption state (TLS-verified caveat or a plaintext warning), and a pointer to the project repository. Agree proceeds (remembered per endpoint until the game closes or the endpoint changes); Exit returns without connecting. Keyboard, gamepad, and touch supported; body text auto-fits so every paragraph stays readable on the smallest windows.
