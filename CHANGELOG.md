...
- Player lantern: a warm dynamic light at `DARK_LEVEL_LIGHT_RADIUS` (dark floors) / `LIGHT_LEVEL_SIGHT_RADIUS` (light floors, adding local warmth over the ambient tint), so sight/visibility reach is identical and combat/LOS logic is untouched. Slow (~0.5 Hz) low-amplitude radius/intensity pulsate for a lantern feel, togglable via the `Reduce motion` accessibility option (which suppresses flicker entirely).
...
- Lit-actor shading: the player and bosses within range of a light get a Lambertian tint computed from the baked normal map: a low-res tint map is smoothscaled up and `BLEND_RGB_MULT`ed onto a full-res copy of the sprite so the sprite stays pixel-crisp (it is never scaled), cached per (sprite, dominant-light bucket, distance bucket) so the per-pixel work only runs when the bucket changes. Regular enemies and familiars rely on the light-buffer multiply. Skipped on the `LIGHTING_OFF` tier and when normal maps are off; hooked in `blit_sprite` (player/enemy/boss) and `draw_familiar`.
