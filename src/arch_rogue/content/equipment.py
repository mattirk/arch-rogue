...
WEAPON_DEFINITIONS = (
    ...
    # Base values stay below MAX_ROLLED_WEAPON_POWER / MAX_ROLLED_ARMOR_DEFENSE
    # so a printed base stat is actually reachable on a Common drop.
    EquipmentDefinition("Ash Pike", "weapon", 7),
...
ARMOR_DEFINITIONS = (
    ...
    EquipmentDefinition("Pilgrim's Plate", "armor", 4),
)


# Icons must stay within pygame's bundled default font coverage (ASCII +
# Latin-1 + a few typographic marks like † ‡); dingbat/geometric glyphs
# (✦ ◆ ✹ ✷ ★ …) render as tofu boxes in every label that shows them.
...
# Legendary is no longer produced by the rolled-equipment generator (4.10.x);
# its entries remain for old saves and for future named legendary blueprints.
...
UNIQUE_ITEM_DEFINITIONS: tuple[UniqueItemDefinition, ...] = (
    ...
    UniqueItemDefinition(
        ...
        skill_bonus="Ambush Bell potency",
        proc_effect="smoke",
...
# Rolled (Common/Magic/Rare, incl. cursed bargains and relic empowerment)
# equipment is hard-capped so every unique is always an upgrade over any
# rolled drop. Fixed values, deliberately NOT derived from the unique
# blueprints: the 3x unique power-up (4.10.x) opened a wide gap between the
# best roll and the weakest unique (7 vs 24 power, 4 vs 15 defense) and the
# caps intentionally stay at the old rolled ceiling.
MAX_ROLLED_WEAPON_POWER: int = 7
MAX_ROLLED_ARMOR_DEFENSE: int = 4

# 5.0 early-game gate: a 15-27 defense unique armor on floors 1-3 floors
# nearly every enemy hit at the 1-damage minimum (base enemy damage is 6-17
# with no depth scaling until floor 5), trivializing the early game. Unique
# ARMOR therefore waits until the dungeon can push back; unique weapons stay
# available early since high power accelerates clears without blanking
# incoming damage. Consumed by PopulationMixin._make_unique, which covers
# every unique source (loot rolls, kills, caches, shrines, shop stock).
UNIQUE_ARMOR_MIN_DEPTH: int = 4
