...
# 5.0 speed rework: locomotion reads the archetype rating by class name (the
# co-op partner sim only knows the remote's class_name string, and reading
# the rating from content instead of the replicated ``player.speed`` field
# keeps old saves' accumulated Haste Shrine stacks from leaking in).
ARCHETYPE_SPEED_BY_NAME: dict[str, float] = {
    archetype.name: archetype.speed for archetype in ARCHETYPES
}
