...
class InventoryMixin:
    def item_decision_summary(self, item: Item, *, equipped_view: bool = False) -> str:
        ...
        if item.slot == "remove_curse":
            cursed = sum(
                ...
                if entry is not None and entry.cursed
            )
            return "Lifts one curse · frees the item" + (
                f" · {cursed} cursed" if cursed else " · none cursed"
            )
        ...
        if item.slot in ("weapon", "armor"):
            ...
            if equipped_view:
                return f"{incoming} {stat}{tradeoff}{detail}"
    ...
    def focused_equipped_item(self) -> Item | None:
        """Return the equipped item highlighted by the equipment-panel focus."""

        slot = getattr(self, "inventory_equipment_focus", None)
    ...
    def cycle_equipment_focus(self, forward: bool) -> None:
        """Left/Right in the inventory: bag → weapon → armor → bag."""

        order = (None, "weapon", "armor")
        current = getattr(self, "inventory_equipment_focus", None)
        index = order.index(current if current in order else None)
        step = 1 if forward else -1
        for _ in range(len(order)):
            index = (index + step) % len(order)
            slot = order[index]
            if slot is None or self.player.equipment.get(slot) is not None:
    ...
    def cycle_inventory_sort_mode(self, forward: bool = True) -> None:
        ...
        step = 1 if forward else -1
        self.inventory_sort_mode = modes[(current + step) % len(modes)]
        self.sort_inventory()
    ...
    def use_inventory_slot(self, index: int) -> None:
        ...
        if item.slot == "identify":
            ...
            return
        if item.slot == "remove_curse":
            if not self.lift_curse_with_scroll():
                # No curse anywhere — keep the scarce scroll instead of
                # consuming it on nothing (unlike identify's cheap scrolls).
                self.player.inventory.insert(index, item)
            self.clamp_inventory_selection()
            self.save_run()
            return
        if item.slot in ("weapon", "armor"):
            equipped = self.player.equipment.get(item.slot)
            if equipped is not None and equipped.cursed:
                self.player.inventory.insert(index, item)
                self.floaters.append(
                    FloatingText(
                        f"{equipped.display_name} is cursed — it will not come off",
                        self.player.x,
                        self.player.y - 0.4,
                        (214, 92, 150),
                        ttl=1.4,
    ...
    def lift_curse_with_scroll(self) -> bool:
        """Cleanse one cursed item; equipped curses take priority. False if none.

        Lifting the curse frees the slot and drops the bargain's handling
        maluses, but the raw power stays — the reward for spending a scarce
        scroll. The cleansed item reads as Rare so its color/icon shift is
        visible immediately.
        """
        ...
        target = next(
            (
                ...
                if (equipped := self.player.equipment.get(slot)) is not None
                and equipped.cursed
            ),
            None,
        )
        if target is None:
            cursed_in_bag = [
                entry for entry in self.player.inventory if entry.cursed
            ]
            if cursed_in_bag:
                target = max(
                    cursed_in_bag,
        ...
        if target is None:
            ...
            return False
        target.cursed = False
        if target.rarity == "Cursed":
            target.rarity = "Unique" if target.unique_effect else "Rare"
        if "Tempting Curse" in target.affixes:
            target.affixes.remove("Tempting Curse")
        target.affix_tags = [
            tag for tag in target.affix_tags if tag not in ("curse", "risk")
        ]
        # Undo only the bargain's handling penalties (_apply_cursed_bargain).
        if target.slot == "weapon":
            target.move_speed += 0.03
        else:
            target.cast_speed += 0.03
            target.move_speed += 0.04
        self.floaters.append(
            FloatingText(
                f"The curse on {target.display_name} is lifted",
                self.player.x,
                self.player.y - 0.4,
                (170, 230, 170),
                ttl=1.4,
