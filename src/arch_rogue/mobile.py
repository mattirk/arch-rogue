...
MOBILE_PERF_PHASES = (
    ...
    "flip",
    "audio",
    # Steam callback pump (desktop Steam builds only; zero elsewhere). Timed
    # so a stalling Steam client shows up attributed instead of as "other".
    "steam",
)
MOBILE_PERF_DETAIL_PHASES = (
    ...
    "ui_upload",
    "gpu_present",
    # Durable run-save writes (serialize + fsync) — the classic frame-stall
    # suspect on machines with slow or busy disks.
    "save",
    # Cyclic-GC pauses, captured via gc.callbacks; lands inside whichever
    # phase was running, so this detail is what separates "update was slow"
    # from "the collector ran mid-update".
    "gc",
)
# A frame slower than this emits one immediate attributed ARCH_ROGUE_HITCH
# line (rate-limited), so intermittent spikes can be diagnosed from the log
# after the fact instead of eyeballing the rolling averages.
MOBILE_PERF_HITCH_FRAME_SECONDS = 0.045
MOBILE_PERF_HITCH_ENV = "ARCH_ROGUE_HITCH_MS"
MOBILE_PERF_HITCH_MIN_GAP_SECONDS = 0.25
...
def application_storage_directory(mobile: bool) -> Path:
    ...
    5.0.1: desktop uses SDL's per-user pref dir too (mobile always has) —
    ``%APPDATA%/Arch Rogue/Arch Rogue`` on Windows,
    ``~/.local/share/Arch Rogue/Arch Rogue`` on Linux/Deck, and
    ``~/Library/Application Support/Arch Rogue/Arch Rogue`` on macOS —
    instead of dotfiles in the bare home directory. Steam Auto-Cloud has
    per-OS roots for exactly these locations but none for the Windows home
    folder, so this is what makes cross-device cloud saves addressable.
    ``Game.__init__`` migrates the legacy home dotfiles on first launch.
    """
...
_HEADLESS_STORAGE_DIR: Path | None = None


def headless_storage_directory() -> Path:
    """A throwaway per-process storage directory for headless runs.

    Headless means tests, benchmarks and tooling, none of which may touch the
    developer's real profile. ``Game`` already refuses to *read* home-directory
    preferences when headless; without this it still wrote them, because
    ``save_options`` and ``save_run`` resolve against the same default storage
    directory. A test that finalised a run therefore overwrote the developer's
    own ``meta_progress`` with whatever that test had set up.

    One directory per process, reused by every headless ``Game``, removed at
    interpreter exit. Callers that need persistence across instances still get
    it; callers that need isolation from each other pass explicit paths.
    """

    global _HEADLESS_STORAGE_DIR
    if _HEADLESS_STORAGE_DIR is None:
        path = Path(tempfile.mkdtemp(prefix="arch-rogue-headless-"))
        atexit.register(shutil.rmtree, path, ignore_errors=True)
        _HEADLESS_STORAGE_DIR = path
    return _HEADLESS_STORAGE_DIR
...
class MobilePerformanceMonitor:
    ...
    def __init__(
        ...
        *,
        ...
        try:
            self._hitch_threshold = (
                float(os.environ.get(MOBILE_PERF_HITCH_ENV, "")) / 1000.0
            )
        except ValueError:
            self._hitch_threshold = MOBILE_PERF_HITCH_FRAME_SECONDS
        if self._hitch_threshold <= 0.0:
            self._hitch_threshold = MOBILE_PERF_HITCH_FRAME_SECONDS
        self._last_hitch_emitted = float("-inf")
        self._gc_probe_installed = False
        self._gc_pause_started: float | None = None
        self.last_report = ""
    ...
    def install_gc_probe(self) -> None:
        """Attribute cyclic-GC pauses to the ``gc`` detail phase."""

        if self._gc_probe_installed:
            return
        gc.callbacks.append(self._on_gc_event)
        self._gc_probe_installed = True

    def uninstall_gc_probe(self) -> None:
        """Detach from gc.callbacks so a replaced monitor is collectable."""

        if not self._gc_probe_installed:
            return
        try:
    ...
    def _on_gc_event(self, phase: str, _info: dict) -> None:
        if phase == "start":
            self._gc_pause_started = self._clock()
        elif phase == "stop" and self._gc_pause_started is not None:
            self.record_detail_phase("gc", self._clock() - self._gc_pause_started)
    ...
    def finish_frame(self, game: Any) -> str | None:
        ...
        # Immediate per-hitch attribution: the interval report only describes
        # the single worst frame per window, which is useless when a machine
        # hitches several times per window for different reasons. Rate-limited
        # so a sustained slow patch cannot flood the log.
        if (
            frame_seconds >= self._hitch_threshold
            and now - self._last_hitch_emitted >= MOBILE_PERF_HITCH_MIN_GAP_SECONDS
        ):
            self._last_hitch_emitted = now
            phase_label = ",".join(
                f"{phase}:{seconds * 1000.0:.1f}"
                for phase, seconds in self._current_phase_seconds.items()
                if seconds >= 0.001
            )
            detail_label = ",".join(
                f"{phase}:{seconds * 1000.0:.1f}"
                for phase, seconds in self._current_detail_phase_seconds.items()
                if seconds >= 0.0005
            )
            unattributed_ms = max(
                0.0,
                (frame_seconds - sum(self._current_phase_seconds.values()))
                * 1000.0,
            )
            self._emit(
                f"ARCH_ROGUE_HITCH frame={frame_seconds * 1000.0:.1f}ms "
                f"state={getattr(game, 'state', 'unknown')} "
                f"phases={phase_label or 'none'},other:{unattributed_ms:.1f} "
                f"details={detail_label or 'none'} "
                f"loads:+{load_delta} builds:+{build_delta} "
                f"reveals:+{reveal_delta} "
                f"enemies={self._safe_len(getattr(game, 'enemies', ()))}"
            )
...
class MobileMixin:
    ...
    def init_mobile_runtime(
        ...
        self._set_performance_monitor(self._make_performance_monitor())
    ...
    def handle_mobile_finger_event(self, event: pygame.event.Event) -> bool:
        ...
        if contact.role == f"target:{Command.ABILITY_1}":
            # Lifting the finger off the slot-1 button releases the Big Hit
            # charge (cancels it if the hold ended before the commit point).
    ...
    def _handle_mobile_swipe(
        ...
        if context == "mini_game":
            # The cells are direct touch targets. A drag across the board is
            # intentionally inert instead of becoming navigation or world aim.
            return
        if abs(dx) > abs(dy):
    ...
    def handle_mobile_tap(self, point: tuple[int, int]) -> bool:
        ...
        if context == "mini_game":
            ...
            if getattr(state, "phase", "") == "ready":
                ...
                if (
                    isinstance(ready_rect, pygame.Rect)
                    and ready_rect.collidepoint(local)
                ):
                    self.confirm_active_mini_game_ready()
                return True
            index = self._rect_index(
                getattr(self, "_mini_game_cell_rects", ()), local
            )
            if index is not None:
                self.mini_game_cursor = index
                if getattr(state, "phase", "") == "play":
                    self.activate_mini_game_cell(index)
            # The modal owns the full display, including gaps and letterbox
            # space, so an imprecise tap cannot leak into gameplay.
            return True
        if context == "title":
    ...
    def _make_performance_monitor(self) -> Any:
        ...
        #
        # Desktop monitors also append to <storage>/arch_rogue_perf.log:
        # packaged Windows builds run --windowed with no console, so printed
        # telemetry (including the per-hitch attribution lines) is otherwise
        # unrecoverable from a player's machine.
        emit = (
            None
            if getattr(self, "mobile_mode", False)
            else self._perf_log_emit
        )
        if mobile_performance_telemetry_enabled():
            return MobilePerformanceMonitor(emit=emit)
        if bool(getattr(self, "show_perf_overlay", False)):
            return MobilePerformanceMonitor(emit=emit)
        return None

    def _set_performance_monitor(self, monitor: Any) -> None:
        """Swap the live monitor, moving the process-global GC probe with it."""

        previous = getattr(self, "_mobile_performance_monitor", None)
        if previous is not None:
            previous.uninstall_gc_probe()
        self._mobile_performance_monitor = monitor
        if monitor is not None:
            monitor.install_gc_probe()
    ...
    def _perf_log_emit(self, line: str) -> None:
        """Print telemetry and append it to the storage-directory perf log.

        Line-buffered append, no fsync: the log must never become the stall it
        exists to diagnose. A failed open is remembered as ``False`` so a
        read-only profile costs one attempt, not one per line.
        """

        print(line, flush=True)
        handle = getattr(self, "_perf_log_handle", None)
        if handle is None:
            try:
                path = Path(self.options_path).parent / "arch_rogue_perf.log"
    ...
    def clear_mobile_memory_caches(self) -> None:
        ...
        for name in (
            ...
            "_soft_shadow_template_cache",
            "_scaled_soft_shadow_cache",
            "_impact_overlay_cache",
            "_projectile_trail_cache",
            "_story_relic_sprite_cache",
            ...
            # Native zoom buckets can retain large 512px tile/door canvases in
            # these front caches even after the sprite-library LRU is cleared.
            "tile_cache",
            "door_tile_cache",
