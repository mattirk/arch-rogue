...
class RenderingStoryOverlayMixin:
    def draw_story_panel(self) -> None:
        ...
        for prompt_name in (
            "_interaction_prompt_rect",
            "_memory_token_prompt_rect",
        ):
            prompt_rect = getattr(self, prompt_name, None)
            if (
    ...
    # 4.9.x readability: cutscene text is the game's central storytelling
    # surface, so the narration body and choice labels read in the main UI
    # font (22 px) instead of small_font (16 px); choice details step up from
    # tiny to small. The narrator card guarantees three lines at the bigger
    # size, which still fits the 208-char omen budget without scrolling.
    @property
    def cutscene_narration_font(self) -> pygame.font.Font:
        return self.font

    @property
    def cutscene_choice_label_font(self) -> pygame.font.Font:
        # A step below the narration font: the story text is the star, the
        # choice rows answer it.
        return getattr(self, "choice_font", None) or self.small_font

    @property
    def cutscene_choice_detail_font(self) -> pygame.font.Font:
        return self.small_font

    # --- letter-spaced narration (4.9.x) ----------------------------------
    # Pygame fonts have no tracking, so the story text renders glyph by
    # glyph with an extra pixel of air between letters (scaled with the UI).
    # Measuring and rendering share the same per-char advance cache so the
    # wrap always matches the blit.

    def cutscene_letter_spacing(self) -> int:
        return max(1, self.ui(1))

    def _tracked_advances(self, font: pygame.font.Font) -> dict[str, int]:
        cache = getattr(self, "_tracked_advance_cache", None)
        if cache is None:
            cache = {}
            self._tracked_advance_cache = cache
        return cache.setdefault(id(font), {})

    def tracked_text_width(
        self, font: pygame.font.Font, text: str, tracking: int
    ) -> int:
        if not text:
            return 0
        advances = self._tracked_advances(font)
        width = 0
        for char in text:
            advance = advances.get(char)
            if advance is None:
    ...
    def wrap_tracked_text(
        self, text: str, font: pygame.font.Font, max_width: int, tracking: int
    ) -> list[str]:
        lines: list[str] = []
        for paragraph in text.splitlines() or [""]:
            ...
            for word in words[1:]:
                ...
                if self.tracked_text_width(font, candidate, tracking) <= max_width:
                    current = candidate
                else:
    ...
    def render_tracked_text(
        ...
        cache = getattr(self, "_tracked_text_cache", None)
        if cache is None:
            cache = {}
            self._tracked_text_cache = cache
        key = (text, tuple(color), tracking, id(font))
        cached = cache.get(key)
        if cached is not None:
            return cached
        width = max(1, self.tracked_text_width(font, text, tracking))
        rendered = pygame.Surface((width, font.get_height()), pygame.SRCALPHA)
        advances = self._tracked_advances(font)
        x = 0
        for char in text:
            if not char.isspace():
                rendered.blit(font.render(char, True, color), (x, 0))
            advance = advances.get(char)
            if advance is None:
                advance = font.size(char)[0]
                advances[char] = advance
    ...
    def cutscene_response_text_width(
        self, choice_width: int, row_height: int | None = None
    ) -> int:
        # 4.9.4: when the authored choice panel is active, measure the real
        # nine-slice content box (its insets scale with row height) instead of
        # the legacy fixed-inset estimate — the old 776 px estimate against a
        # ~720 px drawn box let long labels wrap after the row height was
        # fixed and bleed into the next row.
        probe = pygame.Rect(
            0, 0, choice_width, row_height if row_height else self.ui(44)
        )
        content = self.ui_asset_content_rect("cutscene.choice.panel", probe)
        if content is not None:
            return max(1, content.inflate(-self.ui(8), -self.ui(4)).width)
        key_size = self.ui(36)
    ...
    def cutscene_response_lines(
        ...
        label_lines = (
            self.wrap_ui_text(label, self.cutscene_choice_label_font, text_width)
            or [""]
        )
        detail_lines = (
            self.wrap_ui_text(detail, self.cutscene_choice_detail_font, text_width)
            if detail
            else []
        )
    ...
    def cutscene_response_height(
        ...
        # Row height and wrap width depend on each other (authored insets
        # scale with height); iterate to the fixpoint so the measured lines
        # match what the draw pass will actually wrap at that height.
        height = self.ui(44)
        for _ in range(3):
            text_width = self.cutscene_response_text_width(choice_width, height)
            label_lines, detail_lines = self.cutscene_response_lines(
                label, detail, text_width
            )
            text_h = len(label_lines) * self.cutscene_choice_label_font.get_height()
            if detail_lines:
                text_h += self.ui(3) + len(
                    detail_lines
                ) * self.cutscene_choice_detail_font.get_height()
            new_height = max(self.ui(44), text_h + self.ui(16))
    ...
    def draw_cutscene_response_text(
        ...
        *,
        ...
        label_height = self.cutscene_choice_label_font.get_height()
        detail_height = self.cutscene_choice_detail_font.get_height()
        ...
        if center_vertically:
            # True centering, even when the block runs a little taller than
            # the content box — symmetric overflow reads centered; clamping
            # to the top does not.
            cursor_y += (text_rect.height - text_height) // 2
        for line in label_lines:
            self.draw_ui_text(
                surface,
                line,
                self.cutscene_choice_label_font,
                (246, 235, 210),
                pygame.Rect(
        ...
        for line in detail_lines:
            self.draw_ui_text(
                surface,
                line,
                self.cutscene_choice_detail_font,
                (184, 178, 168),
                pygame.Rect(
    ...
    def _draw_quest_cutscene_overlay_fitted(self) -> None:
        ...
        line_h = max(
            self.cutscene_narration_font.get_height() + self.ui(3), self.ui(18)
        )
        # Four guaranteed lines at the bigger narration size — the 302-char
        # omen budget wraps to <=4 lines, so choice-time text never scrolls.
        ...
        tracking = self.cutscene_letter_spacing()

        def wrap_narration(width: int) -> list[str]:
            wrapped: list[str] = []
            for paragraph in visible_text.splitlines() or [""]:
                wrapped.extend(
                    self.wrap_tracked_text(
                        paragraph, self.cutscene_narration_font, width, tracking
                    )
                )
            return wrapped
        ...
        for index, text_line in enumerate(visible_lines):
            ...
            surface.blit(
                self.render_tracked_text(
                    self.cutscene_narration_font, text_line, color, tracking
                ),
                (narrator_rect.x, y),
            )
            y += line_h
    ...
    STAGE_FULL_SET_ASSETS = {
        ...
        "lossless_soul": ("stage.backdrop.lossless_soul", 0.82),
    }
    ...
    def draw_cutscene_choice_option(
        ...
        *,
        ...
        icon = (
            self.ui_asset_surface(
                f"cutscene.choice.icon.{choice_key}", key_rect.size
            )
            if choice_key
            else None
        )
        # 4.9: keyless continue-style choices ("Turn the page", the tenth
        # bell) have no per-verb icon; the authored panel still applies and
        # the number badge fills the icon socket instead.
        authored = panel is not None
        panel_content = (
        ...
        if authored:
            ...
            if is_selected:
                # Selected badge: a rectangular box in the choice color,
                # cut to the panel's own frame geometry. (The old circular
                # glow was drawn with pygame.draw.circle whose alpha is
                # ignored on opaque surfaces, so it rendered near-solid and
                # bled over the panel frame and corners.)
                if panel_content is not None:
                    frame_pad = max(self.ui(2), panel_content.y - choice_rect.y)
                    # Slightly inset from the frame on the left, running up
                    # to just short of the socket divider on the right.
                    box_left = choice_rect.x + frame_pad + self.ui(3)
                    box_right = (
                        socket_right - max(1, divider_gap // 2) - self.ui(1)
                    )
                    socket_box = pygame.Rect(
                        box_left,
                        choice_rect.y + frame_pad,
                        max(1, box_right - box_left),
                        max(1, choice_rect.height - frame_pad * 2),
                    )
                else:
                    socket_box = key_rect.inflate(self.ui(6), self.ui(6))
                highlight = pygame.Surface(socket_box.size, pygame.SRCALPHA)
                pygame.draw.rect(
                    highlight,
                    (*choice_color, 58),
                    highlight.get_rect(),
                ...
                surface.blit(highlight, socket_box)
            if icon is not None:
            ...
            if not getattr(self, "mobile_mode", False):
                ...
                if icon is None:
                    # No verb icon: the number badge takes the whole socket.
                    badge_size = max(badge_size, min(key_size, self.ui(18)))
                    badge_rect = pygame.Rect(0, 0, badge_size, badge_size)
                    badge_rect.center = key_rect.center
                pygame.draw.circle(
                    surface,
    ...
    def draw_cutscene_choice_glyph(
        ...
        if choice_key == "aid":
        ...
        elif choice_key == "page":
            # A ledger page with a dog-eared corner and ink lines — the
            # procedural twin of the authored story_choice_page medallion.
            page_w = max(3, int(radius * 1.2))
            page_h = max(4, int(radius * 1.7))
            page = pygame.Rect(0, 0, page_w, page_h)
            page.center = (cx, cy)
            fold = max(2, page_w // 3)
            outline = [
                page.topleft,
            ...
            pygame.draw.polygon(surface, (*color, alpha), outline, self.ui(1))
            pygame.draw.line(
                surface,
                (*color, alpha),
                (page.right - fold, page.top),
            ...
            ink_y = page.top + fold + self.ui(3)
            while ink_y < page.bottom - self.ui(3):
                pygame.draw.line(
                    surface,
                    (*color, max(40, alpha - 60)),
                    (page.left + self.ui(2), ink_y),
                    (page.right - self.ui(2), ink_y),
                    1,
                )
                ink_y += self.ui(3)
        else:
    ...
    def cutscene_choice_color(self, choice_key: str, accent: Color) -> Color:
        ...
        if choice_key == "defy":
            return (232, 83, 74)
        if choice_key == "soul_preserve":
            return (142, 214, 205)
        if choice_key == "soul_release":
            return (150, 168, 226)
        if choice_key == "soul_refuse":
            return (168, 150, 178)
        if choice_key == "page":
            # Ledger parchment — the continue verb of the Rue frame.
            return (226, 205, 160)
        return accent
    ...
    def cutscene_actor_visual(
        ...
        if actor.sprite != "relic" and self.sprites.modern_graphics_active:
            ...
            if actor.sprite == "story_guest":
                ...
                return frame.surface, frame.anchor
            if actor.sprite == "lossless_soul":
                # The keeper faces the audience and lifts into her ritual sway
                # in measured spells — a few bars of dance, then stillness —
                # rather than dancing through the whole audience.
                cycle = clock % 14.0
                dancing = not moving and cycle < 8.0
                frame = self.sprites.lossless_soul_visual(
                    cycle if dancing else clock,
                    direction="south",
                    moving=moving,
                    dancing=dancing,
                )
    ...
    def cutscene_actor_surface(
        ...
        if actor.sprite == "lossless_soul":
            return self.sprites.lossless_soul_sprite.copy()
        if actor.sprite == "enemy":
    ...
    def _draw_story_intro_overlay_fitted(self) -> None:
        ...
        if not lines:
            return
        width, height = self.screen.get_size()
        # This backdrop is fully opaque. A direct target fill avoids allocating
        # and alpha-blitting a display-sized temporary surface every frame.
        self.screen.fill((0, 0, 0))
