...
class RenderingEffectsMixin:
    ...
    def draw_impact(self, effect: ImpactEffect) -> None:
        ...
        radius = max(
            2, int((effect.radius + progress * effect.radius * 1.4) * 28 * self.world_pixel_scale)
        )
        ...
        if cached is not None:
            ...
            self.screen.blit(
                cached, cached.get_rect(center=(sx, sy - 12 * self.world_pixel_scale))
            )
            return
        ...
        if effect.kind == "blood":
            ...
            for index in range(4):
                ...
                y_offset = int((index - 1.5) * 3 * self.world_pixel_scale)
                pygame.draw.line(
                    overlay,
            ...
            for index in range(8):
                ...
                pygame.draw.line(overlay, (*bright, alpha), start, tip, max(1, self.world_pixel_scale))
        ...
        self.screen.blit(overlay, overlay.get_rect(center=(sx, sy - 12 * self.world_pixel_scale)))
        ...
        if archetype == "Warden":
            ...
            for index in range(6):
                ...
                pygame.draw.rect(
                    ...
                    (
                        rune[0] - self.world_pixel_scale,
                        rune[1] - self.world_pixel_scale,
                        self.world_pixel_scale * 2,
                        self.world_pixel_scale * 2,
                    ),
                )
            pygame.draw.circle(
    ...
    def _draw_cast_warden(
        ...
        pygame.draw.circle(
            ...
            center,
            ring_radius,
            max(2, self.world_pixel_scale * 2),
        )
        pygame.draw.circle(
            overlay,
    ...
    def draw_shadow(
        ...
        scaled_w = max(1, round((width + squash * 3 + lift) * self.world_pixel_scale))
        scaled_h = max(1, round((height - squash - lift * 0.32) * self.world_pixel_scale))
        ...
        self.screen.blit(
            shadow,
            shadow.get_rect(
                center=(sx, sy + int(ACTOR_FEET_Y_OFFSET * self.world_pixel_scale))
            ),
        )
    ...
    def draw_item(self, item: Item) -> None:
        ...
        bob = int(math.sin(self.elapsed * 3.2 + item.x * 0.7) * 2 * self.world_pixel_scale)
        # Soft contact shadow grounds the floating loot bob on the floor
        # diamond; lift fades/shrinks the patch as the item rises.
        self.draw_shadow(item.x, item.y, 22, 9, lift=bob / self.world_pixel_scale)
        ...
        rare_scale = (
            1.25
            if item.visible_rarity in ("Rare", "Unique", "Legendary")
            else 1.0
        )
        glow_size = (
            int(42 * rare_scale) * self.world_pixel_scale,
            int(20 * rare_scale) * self.world_pixel_scale,
        )
        ...
        self.screen.blit(glow, glow.get_rect(center=(sx, sy + self.world_pixel_scale)))
        if item.visible_rarity in (
            "Magic",
            "Rare",
            "Unique",
            "Legendary",
            ...
            for index in range(
                3 if item.visible_rarity in ("Unique", "Legendary") else 2
            ):
                angle = (
                    self.elapsed * (2.4 + index * 0.7) + item.x + index * math.tau / 3
                )
                sparkle_x = sx + int(math.cos(angle) * (11 + index * 4) * self.world_pixel_scale)
                sparkle_y = (
                    sy - int((8 + math.sin(angle * 1.7) * 5) * self.world_pixel_scale) - bob
                )
                sparkle_alpha = int(95 + 95 * (0.5 + 0.5 * math.sin(angle * 2.0)))
                pygame.draw.line(
                    self.screen,
                    (*rarity_color, sparkle_alpha),
                    (sparkle_x - 2 * self.world_pixel_scale, sparkle_y),
                    (sparkle_x + 2 * self.world_pixel_scale, sparkle_y),
                    max(1, self.world_pixel_scale),
                )
                pygame.draw.line(
                    self.screen,
                    (*rarity_color, sparkle_alpha),
                    (sparkle_x, sparkle_y - 2 * self.world_pixel_scale),
                    (sparkle_x, sparkle_y + 2 * self.world_pixel_scale),
                    max(1, self.world_pixel_scale),
                )
        item_sprite = sprite
        ...
        item_sprite = self._world_scaled_sprite(item_sprite)
        rect = item_sprite.get_rect(midbottom=(sx, sy + 4 * self.world_pixel_scale - bob))
        ...
        if math.hypot(item.x - self.player.x, item.y - self.player.y) < 1.0:
            ...
            self.screen.blit(
                label, label.get_rect(center=(sx, rect.top - 10 * self.world_pixel_scale))
            )
    ...
    def draw_shop_sign_item(self, item: Item) -> None:
        ...
        glow = self._cached_ellipse_overlay(
            "shop_sign_glow",
            (42 * self.world_pixel_scale, 20 * self.world_pixel_scale),
            gold,
            (int(40 + 35 * pulse) // 8) * 8,
        )
        self.screen.blit(glow, glow.get_rect(center=(sx, sy + self.world_pixel_scale)))
        ...
        if frame.is_asset:
            rect = self.blit_resolved_sprite(
                ...
                item.x,
                item.y,
                y_offset=4.0 - bob / self.world_pixel_scale,
            )
        else:
            sign = self._world_scaled_sprite(sign)
            rect = sign.get_rect(midbottom=(sx, sy + 4 * self.world_pixel_scale - bob))
    ...
    def draw_story_relic_guidance(self) -> None:
        ...
        for index, (wx, wy) in enumerate(crack_world):
            ...
            if index == 0 or index == len(raw_screen) - 1 or not open_area:
                ...
                offset = (jag + sign) * self.world_pixel_scale
        ...
        for branch_index in (1, len(crack_points) - 2):
            ...
            stub_len = 5 * self.world_pixel_scale
        ...
        if tile_lit(tx, ty):
            ...
            ring_cy = float(target_sy - int(4 * self.world_pixel_scale))
            ring_radius = 7.0 * self.world_pixel_scale
        ...
        padding = max(4, int(3 * self.world_pixel_scale))
        ...
        if ring_center is not None:
            pygame.draw.circle(
                ...
                max(1, int(2 * self.world_pixel_scale)),
            )
    ...
    def draw_story_relic(self, item: Item) -> None:
        ...
        glow = pygame.Surface((58 * self.world_pixel_scale, 34 * self.world_pixel_scale), pygame.SRCALPHA)
        ...
        bob = int(math.sin(self.elapsed * 3.5 + item.y) * 3 * self.world_pixel_scale)
        self.draw_shadow(item.x, item.y, 26, 11, lift=bob / self.world_pixel_scale)
        ...
        sprite = self._world_scaled_sprite(
            self._story_relic_sprite(visual, accent, tilt_bucket)
        )
        self.screen.blit(
            sprite, sprite.get_rect(midbottom=(sx, sy + 4 * self.world_pixel_scale - bob))
        )
        ...
        for index in range(4):
            ...
            mote = (
                sx + int(math.cos(angle) * 20 * self.world_pixel_scale),
                sy - int((10 + math.sin(angle) * 8) * self.world_pixel_scale) + bob,
            )
            pygame.draw.circle(
                self.screen, self.shade(accent, 55), mote, max(1, self.world_pixel_scale)
            )
        if math.hypot(item.x - self.player.x, item.y - self.player.y) < 1.0:
            label = self.small_font.render(f"E: {item.display_name}", True, accent)
            self.screen.blit(label, label.get_rect(center=(sx, sy - 36 * self.world_pixel_scale)))
    ...
    def draw_trap(self, trap: Trap) -> None:
        if not trap.active:
            return
        fade = max(0.0, min(1.0, trap.reveal_progress))
        if fade <= 0.0:
            return
        sx, sy = self.world_to_screen(trap.x, trap.y)
        # The trap plate is flat floor art: it centers on the actor ground
        # line (the same line feet and contact shadows use), and casts no
        # drop shadow of its own — a shadow under a flat plate is exactly
        # what made traps read as hovering half a tile above the floor.
        gy = sy + int(ACTOR_FEET_Y_OFFSET * self.world_pixel_scale)
        color = {
        ...
        wobble = math.sin(self.elapsed * 3.7 + trap.y) * 2 * self.world_pixel_scale
        warning = pygame.Surface((42 * self.world_pixel_scale, 22 * self.world_pixel_scale), pygame.SRCALPHA)
        pygame.draw.ellipse(
            warning, (*color, int((22 + pulse * 28) * fade)), warning.get_rect()
        )
        self.screen.blit(warning, warning.get_rect(center=(sx, gy)))
        if fade >= 0.5:
            points = [
                (sx, gy - int((10 + pulse * 2) * self.world_pixel_scale)),
                (sx + 16 * self.world_pixel_scale + int(wobble), gy),
                (sx, gy + int((10 + pulse * 2) * self.world_pixel_scale)),
                (sx - 16 * self.world_pixel_scale + int(wobble), gy),
            ]
            pygame.draw.lines(self.screen, color, True, points, max(1, self.world_pixel_scale))
            pygame.draw.lines(
                self.screen, self.shade(color, 45), True, points, max(1, self.world_pixel_scale)
            )
        frame = self.sprites.trap_visual(trap.kind, self.elapsed + trap.x * 0.5)
        alpha = int(255 * fade)
        if frame.is_asset:
            # The authored anchor rides the sprite's bottom edge (standing-prop
            # convention); recenter the plate art on the floor contact point.
            y_offset = (
                ACTOR_FEET_Y_OFFSET
                + (
                    frame.anchor[1] - frame.surface.get_height() / 2
                )
                * self.world_render_scale
                / self.world_pixel_scale
            )
            rect = self.blit_resolved_sprite(
                frame, trap.x, trap.y, y_offset=y_offset, alpha=alpha
            )
        else:
            ...
            rect = sprite.get_rect(center=(sx, gy))
            self.screen.blit(sprite, rect)
        if fade >= 0.5 and (
            math.hypot(trap.x - self.player.x, trap.y - self.player.y)
            < TRAP_REVEAL_RADIUS
        ):
            label = self.small_font.render(f"! {trap.kind}", True, color)
            self.screen.blit(
                label, label.get_rect(center=(sx, rect.top - 8 * self.world_pixel_scale))
            )
    ...
    def draw_ambush_bell(self, bell: AmbushBell) -> None:
        ...
        ring_w = int((42 + pulse * (14 if armed else 7)) * self.world_pixel_scale)
        ring_h = int((20 + pulse * (7 if armed else 4)) * self.world_pixel_scale)
        ring = pygame.Surface((max(4, ring_w), max(4, ring_h)), pygame.SRCALPHA)
        ring_alpha = int(44 + pulse * (64 if armed else 34))
        pygame.draw.ellipse(ring, (*color, ring_alpha), ring.get_rect(), max(1, self.world_pixel_scale))
        if armed:
            inner = ring.get_rect().inflate(-ring.get_width() // 3, -ring.get_height() // 3)
            pygame.draw.ellipse(ring, (*self.shade(color, 48), 36), inner, max(1, self.world_pixel_scale))
        # The bell sits on the actor ground line like every other grounded
        # prop; ring/arc/mote offsets are expressed relative to that line so
        # the whole composition stays glued to the contact shadow.
        gy = sy + int(ACTOR_FEET_Y_OFFSET * self.world_pixel_scale)
        self.screen.blit(ring, ring.get_rect(center=(sx, gy)))

        if not armed and bell.max_arm_timer > 0.001:
            progress = 1.0 - max(0.0, min(1.0, bell.arm_timer / bell.max_arm_timer))
            arc_rect = pygame.Rect(0, 0, 30 * self.world_pixel_scale, 17 * self.world_pixel_scale)
            arc_rect.center = (sx, gy - 4 * self.world_pixel_scale)
            pygame.draw.arc(
                self.screen,
                self.shade(color, 50),
                arc_rect,
            ...
            for index in range(3):
                ...
                mote = (
                    sx + int(math.cos(angle) * 15 * self.world_pixel_scale),
                    gy - int((8 + math.sin(angle) * 4) * self.world_pixel_scale),
                )
                pygame.draw.circle(
                    self.screen,
                    self.shade(color, 60),
        ...
        if bell_frame is not None:
            self.blit_resolved_sprite(
                bell_frame, bell.x, bell.y, y_offset=ACTOR_FEET_Y_OFFSET - 1.0
            )
        else:
            body_w = max(6, 8 * self.world_pixel_scale)
            body_h = max(5, 7 * self.world_pixel_scale)
            body = pygame.Rect(0, 0, body_w, body_h)
            body.midbottom = (sx, gy - 1 * self.world_pixel_scale)
            pygame.draw.ellipse(
                self.screen, (24, 20, 28), body.inflate(2 * self.world_pixel_scale, 0)
            )
            pygame.draw.ellipse(self.screen, self.shade(color, -48), body)
            pygame.draw.arc(
                self.screen,
                self.shade(color, 42),
                body.inflate(2 * self.world_pixel_scale, 2 * self.world_pixel_scale),
                math.pi,
                math.tau,
                max(1, self.world_pixel_scale),
            )
            crack_x = sx + int(math.sin(self.elapsed * 8.0) * self.world_pixel_scale)
            pygame.draw.line(
                self.screen,
                self.shade(color, 80),
                (crack_x, body.top + self.world_pixel_scale),
                (crack_x - 2 * self.world_pixel_scale, body.centery),
                max(1, self.world_pixel_scale),
            )
            pygame.draw.circle(
                self.screen,
    ...
    def draw_secret(self, secret: SecretCache) -> None:
        ...
        glow = pygame.Surface((34 * self.world_pixel_scale, 18 * self.world_pixel_scale), pygame.SRCALPHA)
        pygame.draw.ellipse(glow, (*color, int(34 + 46 * pulse)), glow.get_rect())
        self.screen.blit(glow, glow.get_rect(center=(sx, sy + 2 * self.world_pixel_scale)))
        if not frame.is_asset:
            pygame.draw.rect(
                self.screen,
                (35, 28, 24),
                (
                    sx - 8 * self.world_pixel_scale,
                    sy - 8 * self.world_pixel_scale,
                    16 * self.world_pixel_scale,
                    10 * self.world_pixel_scale,
                ),
            )
            pygame.draw.rect(
                self.screen,
        ...
        if frame.is_asset:
            self.blit_resolved_sprite(
                ...
                secret.x,
                secret.y,
                y_offset=FLOOR_ART_CONTACT_Y_OFFSET,
            )
        else:
            sprite = self._world_scaled_sprite(frame.surface)
            self.screen.blit(sprite, sprite.get_rect(midbottom=(sx, sy + 4 * self.world_pixel_scale)))
        if math.hypot(secret.x - self.player.x, secret.y - self.player.y) < 1.1:
            ...
            self.screen.blit(label, label.get_rect(center=(sx, sy - 31 * self.world_pixel_scale)))
            self.screen.blit(
                sublabel, sublabel.get_rect(center=(sx, sy - 18 * self.world_pixel_scale))
            )
    ...
    def draw_story_guest(self, guest: StoryGuest) -> None:
        ...
        ring = pygame.Surface((48 * self.world_pixel_scale, 25 * self.world_pixel_scale), pygame.SRCALPHA)
        ring_alpha = int(20 + 12 * pulse) if not guest.resolved else 14
        pygame.draw.ellipse(
            ring, (*color, ring_alpha), ring.get_rect(), max(1, self.world_pixel_scale)
        )
        self.screen.blit(
            ring,
            ring.get_rect(
                center=(sx, sy + int((ACTOR_FEET_Y_OFFSET - 2.0) * self.world_pixel_scale))
            ),
        )
        ...
        # tinted at draw time. Guest art anchors one source px lower than the
        # player template, hence the -1 against the shared ground line.
        y_offset = ACTOR_FEET_Y_OFFSET - 1.0 - body_lift
        if frame.is_asset:
            self.blit_resolved_sprite(frame, guest.x, guest.y, y_offset=y_offset)
        else:
            sprite = self._world_scaled_sprite(frame.surface)
            self.screen.blit(
                sprite, sprite.get_rect(midbottom=(sx, sy + y_offset * self.world_pixel_scale))
            )
        ...
        if (
            ...
            self.screen.blit(label, label.get_rect(center=(sx, sy - 55 * self.world_pixel_scale)))
            self.screen.blit(
                sublabel, sublabel.get_rect(center=(sx, sy - 42 * self.world_pixel_scale))
            )

    def draw_idle_npc(self, npc: IdleNpc) -> None:
        # Ambient flavor NPC. Generic humanoids reuse the story-guest art; bar
        # dancers, garden frogs, and the lossless soul have dedicated visuals.
        # No branch draws quest markers or labels; the soul keeper's talk
        # prompt comes from the interaction-hint HUD, not from this draw.
        ...
        elif npc.kind == "lossless_soul":
            frame = self.sprites.lossless_soul_visual(
                self.elapsed,
                direction=direction,
                moving=moving,
                dancing=not moving,
    ...
    def draw_spirit_beast_pet_indicator(
        ...
        unit = max(1, self.world_pixel_scale)
        size = 5 * unit
        center = size // 2
    ...
    def draw_familiar(self, familiar: Familiar) -> None:
        ...
        if kind == "spirit_beast":
            # Grounded quadruped: its manifest anchor sits ~9 source px below
            # the planted paws (class actors keep them within ~1 px), so +2
            # pushes the art down until the paws meet the contact shadow.
            bob = 0.0
            shadow_w = 34 if familiar.champion else 28
            y_offset = ACTOR_FEET_Y_OFFSET + 2.0
        elif kind == "bar_dancer":
            # Grounded humanoid on the idle-NPC dancer's exact feet line.
            bob = 0.0
            shadow_w = 24
            y_offset = ACTOR_FEET_Y_OFFSET - 1.0
        else:
            # The wisp deliberately hovers above its shadow — it is a spirit,
            # and the shadow gap is its flight, so it stays off the ground line.
            bob = (
        ...
        if frame.is_asset:
            ...
            rendered_sprite = self._world_scaled_sprite(sprite)
            rect = rendered_sprite.get_rect(
                midbottom=(sx, sy + y_offset * self.world_pixel_scale)
            )
            self.screen.blit(rendered_sprite, rect)
            # Match blit_sprite/blit_resolved_sprite: record the body blit so
            # draw_world_objects can ghost the familiar through walls.
            self._last_sprite_blit = (rendered_sprite, rect)
        if familiar.hp < familiar.max_hp:
            bar_w = 24 * self.world_pixel_scale
            fill_w = int(bar_w * max(0, familiar.hp) / familiar.max_hp)
            bar_h = 3 * self.world_pixel_scale
            bar_y = rect.top - 2 * self.world_pixel_scale
            pygame.draw.rect(
    ...
    def draw_shrine(self, shrine: Shrine) -> None:
        ...
        glow = pygame.Surface((50 * self.world_pixel_scale, 28 * self.world_pixel_scale), pygame.SRCALPHA)
        ...
        if not shrine.used:
            for index in range(3):
                ...
                mote_x = sx + int(math.cos(angle) * 17 * self.world_pixel_scale)
                mote_y = sy - int((16 + math.sin(angle) * 6) * self.world_pixel_scale)
                pygame.draw.circle(
                    self.screen,
                    self.shade(color, 35),
                    (mote_x, mote_y),
        ...
        if not frame.is_asset:
            pygame.draw.rect(
                ...
                (48, 42, 50),
                (
                    sx - 7 * self.world_pixel_scale,
                    sy - 24 * self.world_pixel_scale,
                    14 * self.world_pixel_scale,
                    25 * self.world_pixel_scale,
                ),
            )
            pygame.draw.rect(
                self.screen,
                color,
                (
                    sx - 4 * self.world_pixel_scale,
                    sy - 19 * self.world_pixel_scale,
                    8 * self.world_pixel_scale,
                    6 * self.world_pixel_scale,
                ),
            )
        if frame.is_asset:
            self.blit_resolved_sprite(
            ...
            sprite = self._world_scaled_sprite(frame.surface)
            self.screen.blit(sprite, sprite.get_rect(midbottom=(sx, sy + 2 * self.world_pixel_scale)))
        if (
            not shrine.used
            ...
            self.screen.blit(label, label.get_rect(center=(sx, sy - 44 * self.world_pixel_scale)))
            self.screen.blit(
                sublabel, sublabel.get_rect(center=(sx, sy - 31 * self.world_pixel_scale))
            )
    ...
    def _projectile_trail_surface(self, color: Color, alpha: int) -> pygame.Surface:
        ...
        key = (*normalized_color, int(alpha), self.world_pixel_scale)
        cache: dict[tuple[int, int, int, int, int], pygame.Surface] = getattr(
        ...
        if len(cache) >= 128:
            cache.pop(next(iter(cache)))
        trail = pygame.Surface((10 * self.world_pixel_scale, 5 * self.world_pixel_scale), pygame.SRCALPHA)
    ...
    def draw_projectile(self, projectile: Projectile) -> None:
        ...
        for step, alpha in trail_samples:
            ...
            side = math.sin(self.elapsed * 12.0 + step) * 2 * self.world_pixel_scale
            self.screen.blit(
                trail,
                trail.get_rect(
                    center=(
                        sx - int(vx * step * 9 * self.world_pixel_scale + px * side),
                        sy
                        - 12 * self.world_pixel_scale
                        - int(vy * step * 9 * self.world_pixel_scale + py * side),
                    )
                ),
            )
        pygame.draw.circle(
            ...
            (sx, sy - 12 * self.world_pixel_scale),
            max(3, int((3 + flicker * 2) * self.world_pixel_scale)),
        )
        ...
        sprite = self._world_scaled_sprite(sprite)
        rect = sprite.get_rect(center=(sx, sy - 12 * self.world_pixel_scale))
        self.screen.blit(sprite, rect)
    ...
    def draw_slash(self, slash: SlashEffect) -> None:
        ...
        sprite = self._world_scaled_sprite(self.sprites.slash).copy()
        if dx < 0:
        ...
        px, py = -vy, vx
        center = (
            sx + int(vx * (1.0 - life) * 12 * self.world_pixel_scale),
            sy - 18 * self.world_pixel_scale + int(vy * (1.0 - life) * 6 * self.world_pixel_scale),
        )
        for index, alpha in enumerate((92, 54, 26)):
            arc_offset = (index + 1) * 7 * self.world_pixel_scale
            pygame.draw.line(
                self.screen,
                (255, 235, 170, int(alpha * life)),
                (
                    center[0] - int(vx * arc_offset + px * 8 * self.world_pixel_scale),
                    center[1] - int(vy * arc_offset + py * 4 * self.world_pixel_scale),
                ),
                (
                    center[0] + int(vx * arc_offset + px * 8 * self.world_pixel_scale),
                    center[1] + int(vy * arc_offset + py * 4 * self.world_pixel_scale),
        ...
        for side in (-1, 1):
            pygame.draw.line(
                ...
                (
                    center[0]
                    + int((vx * 16 + px * side * 10) * self.world_pixel_scale * (1.0 - life)),
                    center[1]
                    + int((vy * 8 + py * side * 6) * self.world_pixel_scale * (1.0 - life)),
                ),
                max(1, self.world_pixel_scale),
            )
