...
class RenderingBaseMixin:
    ...
    def _static_menu_signature(self) -> object | None:
        """Describe every visible input to a retainable full-screen menu.

        These screens have no animation unless a text field is active. Keeping
        the signature here makes both desktop and mobile retention explicit and
        prevents a newly changed setting or scroll cursor from leaving stale
        pixels on screen.
        """

        state = getattr(self, "state", "")
        if state not in {"title", "options", "controls", "about", "chronicle", "confirm_exit"}:
            return None
        ...
        if state == "title":
            return common + (
                ...
                # 5.0: the relic diamond in the logo spins on desktop, so the
                # retained-frame signature ticks with the spin frame. Mobile
                # keeps the static logo and its draw-once battery saving.
                -1
                if getattr(self, "mobile_mode", False)
                else self.spinning_logo_frame_index(),
            )
        if state == "options":
            return common + (
                int(getattr(self, "options_cursor", 0)),
                int(getattr(self, "options_scroll", 0)),
                bool(getattr(self, "fullscreen", False)),
                getattr(self, "mobile_render_quality", ""),
                getattr(self, "difficulty_name", ""),
                bool(getattr(self, "hell_unlocked", False)),
                getattr(self, "frame_rate_cap", ""),
                ...
                bool(getattr(self, "mp_server_tls", True)),
                bool(getattr(self, "show_perf_overlay", False)),
            )
        if state == "controls":
            return common + (
        ...
        if state == "about":
            return common + (int(getattr(self, "licenses_scroll", 0)),)
        if state == "chronicle":
            return common + (
                int(getattr(self, "chronicle_selection", 0)),
                len(getattr(self, "run_history", ())),
            )
        return common

    def _mobile_static_menu_signature(self) -> object | None:
        """Compatibility alias retained for older mobile diagnostics/tests."""

        return self._static_menu_signature()

    def _paused_scene_context(self) -> tuple[bool, bool, bool, bool] | None:
        """Return the paused overlay set that can safely retain world + HUD.

        Joiners continue interpolating authoritative snapshots while their local
        menu is open, and desktop Help intentionally does not pause gameplay.
        Both cases therefore keep the live renderer.
        """

        if (
            getattr(self, "state", "") != "playing"
            or getattr(self, "story_intro_pending", False)
            or getattr(self, "active_cutscene", None) is not None
            or getattr(self, "active_mini_game", None) is not None
        ):
            return None
        is_joiner = getattr(self, "mp_is_joiner", None)
        if callable(is_joiner) and is_joiner():
            return None
        context = (
            bool(getattr(self, "inventory_open", False)),
            bool(getattr(self, "character_menu_open", False)),
            bool(getattr(self, "shop_open", False)),
            bool(
                getattr(self, "mobile_mode", False)
                and getattr(self, "show_help", False)
            ),
        )
        return context if any(context) else None

    def _paused_scene_signature(
        self, context: tuple[bool, bool, bool, bool]
    ) -> object:
        """Describe visible state that menu actions can change while paused."""

        def item_state(item: object | None) -> object:
            if item is None:
                return None
            return (
                id(item),
                getattr(item, "name", ""),
                getattr(item, "slot", ""),
                int(getattr(item, "power", 0)),
                int(getattr(item, "defense", 0)),
                int(getattr(item, "heal", 0)),
                int(getattr(item, "mana", 0)),
                getattr(item, "rarity", ""),
                bool(getattr(item, "unidentified", False)),
                bool(getattr(item, "cursed", False)),
                tuple(getattr(item, "affixes", ())),
                getattr(item, "damage_type", ""),
                getattr(item, "skill_bonus", ""),
                getattr(item, "proc_effect", ""),
            )

        def actor_state(actor: object) -> object:
            return (
                id(actor),
                round(float(getattr(actor, "x", 0.0)), 3),
                round(float(getattr(actor, "y", 0.0)), 3),
                round(float(getattr(actor, "hp", 0.0)), 2),
                bool(getattr(actor, "alive", True)),
            )

        player = self.player
        camera = getattr(self, "_cam_iso", None)
        equipment = tuple(
            (slot, item_state(item))
            for slot, item in sorted(player.equipment.items())
        )
        inventory = tuple(item_state(item) for item in player.inventory)
        safe = self.mobile_safe_rect()
        return (
            context,
            id(self.screen),
            self.screen.get_size(),
            (safe.x, safe.y, safe.width, safe.height),
            id(getattr(self, "dungeon", None)),
            int(getattr(self, "current_depth", 0)),
            round(float(getattr(self, "view_zoom", 1.0)), 4),
            (
                round(float(camera[0]), 3),
                round(float(camera[1]), 3),
            )
            ...
            bool(getattr(self, "_lighting_enabled", True)),
            bool(getattr(self, "_lighting_normal_maps", False)),
            bool(getattr(self, "minimap_visible", True)),
            bool(self.is_current_floor_dark()),
            getattr(getattr(self, "theme", None), "name", ""),
            getattr(getattr(self, "run_modifier", None), "name", ""),
            (
                round(float(player.x), 3),
                round(float(player.y), 3),
                int(player.hp),
                int(player.max_hp),
                round(float(player.mana), 2),
                int(player.max_mana),
                round(float(player.stamina), 2),
                int(player.max_stamina),
                int(player.level),
                int(player.xp),
                int(player.next_xp),
                int(player.gold),
                int(player.memory_tokens),
                tuple(player.skill_upgrades),
                int(getattr(player, "melee_bonus", 0)),
                int(getattr(player, "spell_bonus", 0)),
                int(getattr(player, "armor_bonus", 0)),
                round(float(getattr(player, "speed", 0.0)), 3),
                round(float(getattr(player, "melee_timer", 0.0)), 3),
                round(float(getattr(player, "bolt_timer", 0.0)), 3),
                round(float(getattr(player, "class_skill_timer", 0.0)), 3),
                round(float(getattr(player, "dash_timer", 0.0)), 3),
                round(float(getattr(player, "bighit_timer", 0.0)), 3),
                tuple(
                    sorted(
                        (
                            str(name),
            ...
            tuple(actor_state(actor) for actor in getattr(self, "players", ())),
            tuple(actor_state(enemy) for enemy in getattr(self, "enemies", ())),
            tuple(
                actor_state(npc)
                for npc in getattr(self, "friendly_npcs", ())
            ),
            tuple(
                (
                    item_state(item),
                    round(float(getattr(item, "x", 0.0)), 3),
                    round(float(getattr(item, "y", 0.0)), 3),
                )
                for item in getattr(self, "items", ())
            ),
        )

    def _cached_paused_scene(
        self, context: tuple[bool, bool, bool, bool] | None
    ) -> tuple[object | None, pygame.Surface | None]:
        if context is None:
            self._paused_scene_cache = None
            return None, None
        signature = self._paused_scene_signature(context)
        cached = getattr(self, "_paused_scene_cache", None)
        if cached is not None and cached[0] == signature:
            return signature, cached[1]
        return signature, None

    def _remember_paused_scene(self, signature: object | None) -> None:
        if signature is None:
            self._paused_scene_cache = None
            return
        # The cache is created only on overlay entry/invalidation; steady frames
        # perform one opaque blit instead of rebuilding the dungeon and HUD.
        self._paused_scene_cache = (signature, self.screen.copy())
    ...
    def draw(self) -> None:
        ...
        menu_draw = {
            ...
            "chronicle": self.draw_chronicle_screen,
        ...
        mini_game_owns_display = bool(
            menu_draw is None and getattr(self, "active_mini_game", None) is not None
        )
        modal_owns_display = cutscene_owns_display or mini_game_owns_display
        menu_signature = self._static_menu_signature() if menu_draw else None
        if (
            menu_signature is not None
            and getattr(self, "_static_menu_last_signature", None) == menu_signature
        ):
            # Desktop windows still present the retained framebuffer so expose/
            # compositor events cannot leave a stale backing surface. Android's
            # retained texture path already owns presentation for this case.
            if not mobile:
        ...
        paused_context = self._paused_scene_context() if menu_draw is None else None
        paused_signature, paused_surface = self._cached_paused_scene(paused_context)
        story_intro_owns_display = bool(
            menu_draw is None
            and getattr(self, "story_intro_pending", False)
            and self.story_intro_lines()
        )
        if mobile:
        ...
        started = time.perf_counter()
        if (
            not modal_owns_display
            and paused_surface is None
        ):
        ...
        if menu_draw is not None:
            ...
            if menu_signature is not None:
                retained_signature = self._static_menu_signature()
                self._static_menu_last_signature = retained_signature
                # Preserve the old field for mobile memory tooling and external
                # diagnostics that pre-date desktop retention.
                self._mobile_static_menu_last_signature = retained_signature
            return

        # Cutscenes and their mini-games own the full display with an opaque
        # background. Rendering the dungeon world + HUD underneath is wasted
        # work, particularly on Android's software-alpha path.
        mini_game_active = getattr(self, "active_mini_game", None) is not None
        cutscene_active = self.active_cutscene is not None
        if paused_surface is not None:
            started = time.perf_counter()
            self.screen.blit(paused_surface, (0, 0))
            if performance is not None:
                performance.record_phase(
                    "world", time.perf_counter() - started
                )
        elif (
            not cutscene_active
            and not mini_game_active
            and not story_intro_owns_display
        ):
            started = time.perf_counter()
            ...
            self._remember_paused_scene(paused_signature)

        started = time.perf_counter()
        mini_game_active = getattr(self, "active_mini_game", None) is not None
        cutscene_active = self.active_cutscene is not None
        if mini_game_active:
            # Mini-games need a larger, directly tappable board than the
            # letterboxed stage and take input precedence over that stage.
            with self.mobile_full_render_target():
                self.draw_mini_game_overlay()
    ...
    def _render_world_view(self) -> None:
        ...
        World detail is selected from bounded render-resolution buckets
        (1.0/1.2/1.4/1.6). The layer is sized
        ``screen_size * bucket / zoom`` and the final composite applies only
        the residual ``zoom / bucket``. At maximum zoom the 1.6 bucket makes
        the layer exactly display-sized, so authored 512 px tiles are blitted
        natively without a 512 -> 320 -> 512 round trip.
        ...
        - residual zoom OUT (zoom/bucket below 1): the world layer is larger than the
        ...
        - residual zoom IN (zoom/bucket above 1): the world layer is smaller than the
    ...
    def _render_world_target(self) -> None:
        ...
        render_scale = self.world_render_scale_bucket(zoom)
        residual_zoom = zoom / render_scale
        use_layer = abs(residual_zoom - 1.0) > 1e-3
        # The bucket controls world render resolution; the residual controls
        # the one final whole-layer composite.  Shade whichever target is
        # smaller.  At maximum zoom both are 1.6, residual is exactly one, and
        # the native-resolution world is drawn directly to the display.
        shade_post = residual_zoom <= 1.0
        self._shade_post_composite = shade_post
        real_screen = self.screen
        layer: pygame.Surface | None = None
        self._activate_world_render_scale(render_scale)
        if use_layer:
            layer = self._world_layer_surface(real_screen, residual_zoom)
            layer.fill((10, 10, 14))
            self.screen = layer
        ...
        try:
            try:
                ...
                if not shade_post:
                    # Residual zoom-in: shade the smaller world layer before
                    # its final nearest-neighbour composite.
                    self._shade_world()
            finally:
                if use_layer:
                    ...
                    # (camera_iso, visible_bounds, frame_lights) so a
                    # post-composite shade pass reuses layer-derived bounds.
                    self._frame_cache.pop("screen_size", None)
            if shade_post:
                # Residual zoom-out or identity: shade the smaller (or only)
                # display after the composite.
                self._shade_world()
        finally:
            # UI/HUD and input projection are display-space concerns.  Do not
            # leak the world render bucket beyond this single painter pass.
            self._active_world_render_scale = None

    def _activate_world_render_scale(self, render_scale: float) -> None:
        """Pin one render bucket and retire stale front-cache descriptors.

        The asset library retains bounded, bucket-keyed derived surfaces in its
        LRU.  The game-level caches retain only the active bucket so repeatedly
        pinching across all four resolutions cannot hold four complete dungeon
        atlases (especially expensive on mobile).
        """

        render_scale = self.world_render_scale_bucket(render_scale)
        self._active_world_render_scale = render_scale
        if getattr(self, "_world_tile_cache_bucket", None) == render_scale:
            return
        for attr in (
            "tile_cache",
            "door_tile_cache",
        ...
        self._world_tile_cache_bucket = render_scale
    ...
    def _world_layer_surface(
        self, real_screen: pygame.Surface, residual_zoom: float
    ) -> pygame.Surface:
        rw, rh = real_screen.get_size()
        lw = max(320, int(round(rw / residual_zoom)))
        lh = max(240, int(round(rh / residual_zoom)))
    ...
    def _composite_world_layer(
        ...
        size = dest.get_size()
        # World art is deliberately pixel-authored. Nearest-neighbour
        # composition preserves hard texel edges and avoids resampling a
        # screen-sized layer through the substantially costlier smooth filter.
