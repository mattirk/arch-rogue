...
from ..combat._utils import BIGHIT_CHARGE_TIME, BIGHIT_COMMIT_FRACTION
from ..combat.class_skills import CLASS_SKILLS, _DEFAULT_CLASS_SKILL
...
class RenderingActorMixin:
    WORLD_SCALED_SPRITE_CACHE_MAX = 192
    WORLD_SCALED_SPRITE_CACHE_MAX_BYTES = 48 * 1024 * 1024

    def _world_scaled_sprite(self, source: pygame.Surface) -> pygame.Surface:
        """Return a bounded bucket-scaled world sprite.

        Actor sources keep their established resolution at bucket 1.0. Higher
        render-density buckets enlarge both the surface and its contact anchor
        before the small residual whole-layer downsample. Keeping this cache
        separate from the atlas avoids multiplying every animation family by
        four while still preventing per-frame transforms for visible actors.
        """
        ...
        raw_cache = getattr(self, "_world_scaled_sprite_cache", None)
        if isinstance(raw_cache, OrderedDict):
            ...
            cache = OrderedDict()
            self._world_scaled_sprite_cache = cache
            self._world_scaled_sprite_cache_bytes = 0
        ...
        scaled = pygame.transform.scale(
            source,
            (
                max(1, round(source.get_width() * render_scale)),
                max(1, round(source.get_height() * render_scale)),
            ),
        )
        scaled = optimize_immutable_alpha_surface(scaled)
        cache[key] = (source, scaled)
        cache.move_to_end(key)
        cache_bytes = int(
            getattr(self, "_world_scaled_sprite_cache_bytes", 0)
        ) + scaled.get_pitch() * scaled.get_height()
        while (
            len(cache) > self.WORLD_SCALED_SPRITE_CACHE_MAX
            or cache_bytes > self.WORLD_SCALED_SPRITE_CACHE_MAX_BYTES
        ):
            _old_key, (_old_source, old_scaled) = cache.popitem(last=False)
            cache_bytes -= old_scaled.get_pitch() * old_scaled.get_height()
        self._world_scaled_sprite_cache_bytes = max(0, cache_bytes)
        return scaled
    ...
    def draw_shopkeeper(self, shopkeeper: Shopkeeper) -> None:
        ...
        self.screen.blit(
            ring,
            ring.get_rect(
                center=(sx, sy + int((ACTOR_FEET_Y_OFFSET - 1.0) * scale))
            ),
        )
        ...
        y_offset = ACTOR_FEET_Y_OFFSET - body_lift
        if frame.is_asset:
    ...
    def draw_jointed_limb(
        ...
        pygame.draw.lines(
            surface, outline, False, rounded_points, width + max(1, self.world_pixel_scale)
        )
    ...
    def draw_humanoid_run_layer(
        ...
        if not self.is_humanoid(actor):
            return
        scale = self.world_pixel_scale * self.humanoid_run_scale(actor)
    ...
    def blit_sprite(
        ...
        turned_sprite = self._world_scaled_sprite(turned_sprite)
        rect = turned_sprite.get_rect(
            midbottom=(
                round(sx + x_offset * self.world_pixel_scale),
                round(sy + y_offset * self.world_pixel_scale),
            )
        )
        self.screen.blit(turned_sprite, rect)
        # draw_world_objects reads this right after draw_player/draw_enemy to
        # re-blit the exact on-screen body sprite for the wall-ghost pass.
        self._last_sprite_blit = (turned_sprite, rect)
        ...
        *,
        ...
        rect = sprite.get_rect(
            topleft=(
                round(
                    sx
                    + x_offset * self.world_pixel_scale
                    - frame.anchor[0] * render_scale
                ),
                round(
                    sy
                    + y_offset * self.world_pixel_scale
                    - frame.anchor[1] * render_scale
                ),
            )
        )
        ...
        # draw_world_objects reads this right after draw_player/draw_enemy to
        # re-blit the exact on-screen body sprite for the wall-ghost pass.
        self._last_sprite_blit = (sprite, rect)
        return rect
    ...
    def draw_hit_flash_overlay(
        ...
        overlay = pygame.Surface(
            (width + 12 * self.world_pixel_scale, height + 10 * self.world_pixel_scale), pygame.SRCALPHA
        )
        ...
        for index in range(4):
            ...
            pygame.draw.line(
                overlay, (*color, int(96 * life)), start, end, max(1, self.world_pixel_scale)
            )
        self.screen.blit(
            overlay,
            overlay.get_rect(
                center=(
                    sx,
                    sy + int(ACTOR_FEET_Y_OFFSET * self.world_pixel_scale) - height // 2,
                )
            ),
        )
    ...
    def draw_windup_telegraph(self, enemy: Enemy, sx: int, sy: int) -> None:
        ...
        base = 10 * self.world_pixel_scale
        radius = int(base + (1.0 - life) * 10 * self.world_pixel_scale)
        alpha = int(120 * life)
        if getattr(self, "mobile_mode", False):
            radius = max(base, round(radius / self.world_pixel_scale) * self.world_pixel_scale)
            alpha = max(0, min(120, round(alpha / 8) * 8))
        size = (radius * 2 + 8 * self.world_pixel_scale, radius * 2 + 8 * self.world_pixel_scale)
        line_width = max(1, self.world_pixel_scale * 2)
        if getattr(self, "mobile_mode", False):
    ...
    def draw_bighit_charge_ring(self, player: Player, sx: int, sy: int) -> None:
        """Big Hit windup telegraph: a tightening ring while the player charges.

        The player-side sibling of ``draw_windup_telegraph`` — same feet
        anchor and cached-overlay machinery, opposite motion: the ring grows
        with charge progress and brightens, then flips to the gold committed
        color once the halfway point locks the blow in. Driven by the
        replicated ``_charging`` status, so a co-op partner's windup shows on
        both screens.
        """
        ttl = player.status_effects.get("_charging", 0.0)
        if ttl <= 0.0:
            return
        progress = max(0.0, min(1.0, 1.0 - ttl / BIGHIT_CHARGE_TIME))
        # Quantize the animation so the shared overlay caches stay small
        # (twelve states cover the 0.9 s charge at readable cadence).
        progress = round(progress * 12.0) / 12.0
        committed = progress >= BIGHIT_COMMIT_FRACTION
        skill = CLASS_SKILLS.get(player.class_name, _DEFAULT_CLASS_SKILL)
        color = (235, 205, 120) if committed else skill.color
        base = 8 * self.world_pixel_scale
        radius = int(base + progress * 12 * self.world_pixel_scale)
        alpha = max(0, min(120, int(50 + 70 * progress)))
        size = (
            radius * 2 + 8 * self.world_pixel_scale,
            radius * 2 + 8 * self.world_pixel_scale,
        )
        line_width = max(1, self.world_pixel_scale * 2)
        if getattr(self, "mobile_mode", False):
    ...
    def draw_garden_heal_glow(self, sx: int, sy: int, width: int, height: int) -> None:
        ...
        aura_w = width + int(18 * self.world_pixel_scale) + int(6 * self.world_pixel_scale * pulse)
        aura_h = height + int(14 * self.world_pixel_scale) + int(4 * self.world_pixel_scale * pulse)
        ...
        pygame.draw.ellipse(
            ...
            ring_rect,
            max(1, self.world_pixel_scale * 2),
        )
        ...
        for index in range(4):
            ...
            spark_h = int(10 * self.world_pixel_scale * (0.6 + 0.4 * pulse))
            pygame.draw.line(
                overlay,
    ...
    def draw_player(self, player: Player) -> None:
        ...
        if frame.is_asset:
            ...
            rect = self.blit_resolved_sprite(
                frame, player.x, player.y, y_offset=ACTOR_FEET_Y_OFFSET
            )
            sx = rect.centerx
            ...
            sx, sy = self.blit_sprite(
                ...
                player.x,
                player.y,
                y_offset=ACTOR_FEET_Y_OFFSET - bob,
                x_offset=sway,
                stretch=stretch,
                lean=lean,
                base_sprite=self.sprites.legacy_player_base(player.class_name),
            )
        rendered_sprite, _rendered_rect = self._last_sprite_blit
        self.draw_bighit_charge_ring(player, sx, sy)
        self.draw_hit_flash_overlay(
            sx,
            sy,
            rendered_sprite.get_width(),
            rendered_sprite.get_height(),
            hit_flash_ttl,
            (255, 110, 90),
        )
        if is_local:
            ...
            self.screen.blit(
                ...
                (
                    ...
                    sy - rendered_sprite.get_height() - label.get_height() - 4,
                ),
            )
    ...
    def draw_aim_cone(self) -> None:
        ...
        # Floor telegraph: fan out from the player's ground line so the cone
        # apex sits at the feet rather than behind the lowered sprite.
        sy += int(ACTOR_FEET_Y_OFFSET * self.world_pixel_scale)
        ...
        if cached is None:
            ...
            def arc_points(
                ...
                radius = length * self.world_pixel_scale
            ...
            pad = (3 if mobile_fast else 18) * self.world_pixel_scale
        ...
        # original absolute-space cone (vx*14*self.world_pixel_scale, -8*self.world_pixel_scale+vy*6).
        blit_x = int(sx + vx * 14 * self.world_pixel_scale - anchor_x)
        blit_y = int(sy - 8 * self.world_pixel_scale + vy * 6 * self.world_pixel_scale - anchor_y)
    ...
    def draw_enemy(self, enemy: Enemy) -> None:
        ...
        y_off = (
            ACTOR_FEET_Y_OFFSET + 4.0 if big_boss else ACTOR_FEET_Y_OFFSET
        ) - bob
        if frame.is_asset:
            rect = self.blit_resolved_sprite(
                frame,
        ...
        rendered_sprite, rect = self._last_sprite_blit
        self.draw_hit_flash_overlay(
            sx,
            sy,
            rendered_sprite.get_width(),
        ...
        bar_h = (7 if big_boss else 4) * self.world_pixel_scale
        bar_y = rect.top - (4 if big_boss else 2) * self.world_pixel_scale
        status_entries = [
            status
        ...
        if status_entries:
            ...
            pip_y = bar_y - 5 * self.world_pixel_scale
            pip_spacing = 7 * self.world_pixel_scale
            ...
            for index, status in enumerate(status_entries[:5]):
                ...
                pygame.draw.circle(
                    ...
                    max(2, 3 * self.world_pixel_scale),
                )
        if enemy.telegraph == "lured":
            lure_color = self.damage_type_color("shadow")
            pulse = 0.5 + 0.5 * math.sin(self.elapsed * 10.0 + enemy.x)
            marker_y = sy - (30 if big_boss else 22) * self.world_pixel_scale
            pygame.draw.circle(
                self.screen,
                (*lure_color, int(80 + pulse * 80)),
                (sx, marker_y),
                max(3, int((3 + pulse * 2) * self.world_pixel_scale)),
                max(1, self.world_pixel_scale),
            )
            pygame.draw.line(
                self.screen,
                self.shade(lure_color, 45),
                (sx - 4 * self.world_pixel_scale, marker_y - 2 * self.world_pixel_scale),
                (sx + 4 * self.world_pixel_scale, marker_y - 2 * self.world_pixel_scale),
                max(1, self.world_pixel_scale),
            )
        if enemy.elite_modifier or enemy.kind == "miniboss":
            pulse = 0.5 + 0.5 * math.sin(self.elapsed * 5.2)
            marker_w = (96 if big_boss else 46) * self.world_pixel_scale
            marker_h = (34 if big_boss else 20) * self.world_pixel_scale
            marker = self._cached_ellipse_overlay(
                "elite_marker",
                (marker_w, marker_h),
                enemy.color,
                (int(28 + pulse * 48) // 8) * 8,
                outer_width=max(1, self.world_pixel_scale),
            ...
            self.screen.blit(
                marker,
                marker.get_rect(
                    center=(sx, sy - (22 if big_boss else 14) * self.world_pixel_scale)
                ),
            )
            ...
            self.screen.blit(
                label, label.get_rect(center=(sx, bar_y - 8 * self.world_pixel_scale))
            )
        if (
            ...
            tell_size = (64 if big_boss else 42) * self.world_pixel_scale
            telegraph = self._cached_circle_overlay(
                ...
                max(
                    3,
                    int((7 if big_boss else 5) * self.world_pixel_scale + pulse * 3 * self.world_pixel_scale),
                ),
                max(1, self.world_pixel_scale),
            )
            self.screen.blit(
                telegraph,
                telegraph.get_rect(
                    center=(sx, sy - (28 if big_boss else 18) * self.world_pixel_scale)
                ),
            )
            ...
            self.screen.blit(
                label,
                label.get_rect(
                    center=(sx, sy - (52 if big_boss else 35) * self.world_pixel_scale)
                ),
            )
        if state == "cast":
            cast_color = (
            ...
            pygame.draw.circle(
                self.screen,
                (*cast_color, 112),
                (sx, sy - 24 * self.world_pixel_scale),
                max(3, 4 * self.world_pixel_scale),
                max(1, self.world_pixel_scale),
            )
        if enemy.kind == "boss" or big_boss:
            pulse = 0.5 + 0.5 * math.sin(self.elapsed * 4.2)
            aura_w = (132 if big_boss else 74) * self.world_pixel_scale
            aura_h = (52 if big_boss else 30) * self.world_pixel_scale
            ...
            self.screen.blit(
                aura,
                aura.get_rect(center=(sx, sy - (28 if big_boss else 18) * self.world_pixel_scale)),
            )
