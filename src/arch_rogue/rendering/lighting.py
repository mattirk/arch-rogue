...
from ..constants import (
    ...
    LIGHT_ROOM_FLASH_ATTACK,
    LIGHT_ROOM_FLASH_COLOR,
    LIGHT_ROOM_FLASH_FADE,
    LIGHT_ROOM_FLASH_PEAK,
...
from ..models import Color, LightSource, Tile
...
class LightingMixin:
    ...
    def _shade_params(self) -> tuple[float, Callable[[float, float], tuple[int, int]]]:
        ...
        # shading the layer pre-composite, positions and sprite sizes use the
        # active render-resolution bucket. Returns
        # (effective_render_scale, projection_callable).
        if getattr(self, "_shade_post_composite", True):
            return getattr(self, "view_zoom", 1.0), self.world_to_display
        return self.world_render_scale, self.world_to_screen

    def _dark_ambient_sprite(
        self, semi_x: int, color: Color, flat_frac: float
    ) -> pygame.Surface:
        """Elliptical dark-floor ambient wash, feathered to zero at its rim.

        The lantern visibility disc is a circle in world space; under the 2:1
        projection that is an exact screen ellipse with semi-axes
        ``R * sqrt(2) * TILE_W / 2`` by half that, so the wash can match the
        tile-cull boundary precisely. The profile is flat at full ``color``
        out to ``flat_frac`` of the radius (the old visibility edge — the
        interior look is unchanged) and smoothsteps to black at the rim (the
        widened draw edge), so terrain entering the drawn disc is multiplied
        to black. Built like ``_radial_light_sprite``: 1px circle bands at a
        bounded resolution, then one bilinear upscale to the elliptical size.
        """

        semi_x = max(16, (int(semi_x) // 8) * 8)
        key = (semi_x, hashable_color(color), round(flat_frac, 3))
        cache = getattr(self, "_dark_ambient_sprite_cache", None)
        if cache is None:
            cache = {}
            self._dark_ambient_sprite_cache = cache
        cached = cache.get(key)
        if cached is not None:
            return cached
        build_size = 160
        build_r = build_size // 2
        grad = pygame.Surface((build_size, build_size))
        cx = cy = build_r
        for r in range(build_r, 0, -1):
            t = r / build_r
            if t <= flat_frac:
                ...
                u = (t - flat_frac) / (1.0 - flat_frac)
                factor = 1.0 - u * u * (3.0 - 2.0 * u)
            band = (
                int(color[0] * factor),
                int(color[1] * factor),
                int(color[2] * factor),
            )
            pygame.draw.circle(grad, band, (cx, cy), r)
        sprite = pygame.transform.smoothscale(grad, (semi_x * 2, semi_x))
        try:
    ...
    def _stamp_ambient(self, buffer: pygame.Surface, scale: int) -> None:
        ...
        if self.is_current_floor_dark():
            # Dark floor: near-black themed wash, feathered to zero across the
            # widened draw band (run_flow._visibility_scan_params) so terrain
            # crossing the tile-cull boundary surfaces multiplied to black
            # instead of popping in at ambient brightness. Inside the old
            # visibility disc the wash keeps its flat authored level.
            buffer.fill((0, 0, 0, 255))
            eff_zoom, project = self._shade_params()
            outer = DARK_LEVEL_LIGHT_RADIUS + DARK_VISIBILITY_OUTER_MARGIN
            draw_radius = outer + DARK_VISIBILITY_DRAW_MARGIN
            semi_x = int(
                draw_radius * 0.7071 * TILE_W * eff_zoom
            ) // max(1, scale)
            sprite = self._dark_ambient_sprite(
                semi_x,
                shade_color(theme_color, level),
                outer / draw_radius,
            )
            px, py = project(self.player.x, self.player.y)
            buffer.blit(
                sprite,
                (
                    px // scale - sprite.get_width() // 2,
                    py // scale - sprite.get_height() // 2,
                ),
        ...
        if self._smooth_fog_reveal_active():
            # 4.10.1 HD: ambient * revelation field, bilinearly smooth across
            # the fog frontier (rendering/fog.py). Interior tiles receive the
            # same full-ambient multiply as the rect fills below.
            self._stamp_smooth_fog(buffer, scale, ambient)
            return
        ...
        wall_top_reach_px = int(
            (48 * WORLD_SCALE + TILE_H * 0.5) * eff_zoom
        ) // scale
        wall_extra_px = max(0, wall_top_reach_px - tile_w_px // 2)
        min_x, max_x, min_y, max_y = self.visible_bounds()
        ...
        for x in range(min_x, max_x + 1):
            for y in range(min_y, max_y + 1):
                ...
                elevated = (
                    tiles[x][y] in (Tile.WALL, Tile.CLOSED_DOOR, Tile.OPEN_DOOR)
                )
                top_extra = wall_extra_px if elevated else 0
                buffer.fill(
                    ambient,
                    pygame.Rect(
                        rx,
    ...
    def _room_flash_state(
        self,
    ) -> tuple[dict[tuple[int, int], int], float, float] | None:
        """The active Frost Nova flash as (tiles, elapsed, fade), else None.

        ``tiles`` maps region tiles to wavefront BFS distances from the
        caster, ``elapsed`` is the time since the cast, and ``fade`` is the
        global end-of-life dimming factor (1.0 while the flash holds, easing
        to 0 across the final ``LIGHT_ROOM_FLASH_FADE`` window so the whole
        lit region fades out together).
        """
        tiles = getattr(self, "room_flash_tiles", None)
        ttl = getattr(self, "room_flash_ttl", 0.0)
        max_ttl = getattr(self, "room_flash_max_ttl", 0.0)
        if not tiles or ttl <= 0.0 or max_ttl <= 0.0:
            return None
        fade = min(1.0, ttl / LIGHT_ROOM_FLASH_FADE) ** 1.2
        if fade <= 0.0:
            return None
        return tiles, max(0.0, max_ttl - ttl), fade

    def _stamp_room_flash(self, buffer: pygame.Surface, scale: int) -> None:
        """Additively stamp the Frost Nova flash wave into the light buffer.

        The light originates at the caster: each region tile brightens over
        ``LIGHT_ROOM_FLASH_ATTACK`` once the wavefront (traveling at
        ``LIGHT_ROOM_FLASH_WAVE_SPEED`` along BFS distances) reaches it,
        stays lit, and the whole region fades out together at the end. The
        per-tile rect approximation mirrors ``_stamp_ambient`` so the flash
        composites exactly like the ambient wash and stops at the region's
        walls and doors. On light floors the desktop path skips
        never-revealed tiles, matching the fog-of-war ambient rules; on
        dark floors the flash deliberately lights the whole chamber — the
        nova illuminating a dark room is the payoff of the max effect.
        """
        state = self._room_flash_state()
        if state is None:
            return
        tiles, elapsed, fade = state
        color = getattr(self, "room_flash_color", LIGHT_ROOM_FLASH_COLOR)
        peak = LIGHT_ROOM_FLASH_PEAK * fade
        eff_zoom, project = self._shade_params()
        tile_w_px = max(1, int(TILE_W * eff_zoom) // scale)
        min_x, max_x, min_y, max_y = self.visible_bounds()
        revealed = (
        ...
        # Under the HD revelation field the frontier ambient is a gradient;
        # damp the additive flash by the same field so its rects cannot
        # re-brighten the fog band with hard tile edges.
        fog_field = (
            getattr(self, "_fog_field", None)
            if revealed is not None and self._smooth_fog_reveal_active()
            else None
        )
        for (x, y), dist in tiles.items():
            if not (min_x <= x <= max_x and min_y <= y <= max_y):
                continue
            if revealed is not None and (x, y) not in revealed:
                continue
            local = elapsed - dist / LIGHT_ROOM_FLASH_WAVE_SPEED
            if local <= 0.0:
                continue
            strength = peak * min(1.0, local / LIGHT_ROOM_FLASH_ATTACK)
            if fog_field is not None:
                strength *= fog_field.get((x, y), 1.0)
            sx, sy = project(x + 0.5, y + 0.5)
            rx = sx // scale - tile_w_px // 2
            ry = sy // scale - tile_w_px // 2
            buffer.fill(
                (
                    max(0, min(255, int(color[0] * strength))),
                    max(0, min(255, int(color[1] * strength))),
                    max(0, min(255, int(color[2] * strength))),
                    0,
                ),
                pygame.Rect(rx, ry, tile_w_px, tile_w_px),
    ...
    def draw_lighting(self) -> None:
        ...
        # The Frost Nova flash wave joins the retained-frame signature so the
        # accelerated mobile path rebuilds the buffer while the wave expands
        # and fades instead of freezing its first frame. Elapsed time is
        # quantized to ~30 Hz buckets — finer than the alternating-frame
        # dynamic-light rebuild cadence, so it never blocks reuse on its own.
        flash_state = self._room_flash_state()
        flash_signature: tuple[int, int, Color, int] | None = None
        if flash_state is not None:
            flash_tiles, flash_elapsed, flash_fade = flash_state
            flash_signature = (
                len(flash_tiles),
                int(flash_elapsed * 30),
                hashable_color(
                    getattr(self, "room_flash_color", LIGHT_ROOM_FLASH_COLOR)
                ),
                max(0, min(255, ((int(255 * flash_fade) + 8) // 16) * 16)),
            )
        frame_signature = (
            buffer.get_size(),
            ambient_color,
        ...
        if not reuse_light_frame:
            ...
            # 2b) Frost Nova max effect: wash the caster's whole room in
            # light, clipped to the room's tiles so the flash stops at its
            # walls and doors.
            self._stamp_room_flash(buffer, scale)
        ...
        # Nearest-neighbour upscale on every platform. The buffer already
        # holds smooth radial gradients, so 2x pixel duplication under a
        # BLEND_RGB_MULT composite is visually indistinguishable from the
        # previous full-screen smoothscale while costing roughly half as much
        # (measured 2.3 ms -> 1.15 ms at 2560x1440); mobile shipped nearest
        # from quarter resolution since 4.3.10 for the same reason.
    ...
    def _lit_tint_map(
        ...
        # How strongly this light shapes the sprite. The scene light buffer
        # already owns overall brightness, so a dim or distant dominant light
        # fades the relief toward the neutral multiply (1.0) instead of
        # scaling the whole sprite toward black: the old ``* intensity``
        # shade let a faint co-located fixture (e.g. a soul-prop aura at
        # 0.3) crush its own prop to ~15% brightness, and the prop then
        # snapped 2.5x whenever the player's lantern took over dominance.
        # ``intensity * atten`` mirrors the ``_dominant_light`` score, so at
        # a dominance handoff the winner and loser carry nearly equal
        # strength and the tint changes without a visible brightness pop.
        strength = max(0.0, min(1.0, light.intensity * atten))
            ...
            for py in range(sh):
                for px in range(sw):
                    ...
                    if lambert < 0.0:
                        ...
                        lambert = 1.0
                    relief = 0.32 + 0.68 * lambert
                    shade = 1.0 + (relief - 1.0) * strength
                    tr = max(
                        0,
                        min(
                            255,
                            int((255 + (light_color[0] - 255) * strength) * shade),
                        ),
                    )
                    tg = max(
                        0,
                        min(
                            255,
                            int((255 + (light_color[1] - 255) * strength) * shade),
                        ),
                    )
                    tb = max(
                        0,
                        min(
                            255,
                            int((255 + (light_color[2] - 255) * strength) * shade),
                        ),
                    )
