...
from .costs import player_recovery_rates
...
class _PlayerCombatMixin:
    ...
    def update_player(self, dt: float) -> None:
        ...
        move_speed = (
            PLAYER_MOVE_SPEED
            # 5.0: archetype rating + gear + discipline nodes + Haste Shrine,
            # clamped in one channel (combat/costs.player_move_speed).
            * (1.0 + self.player_move_speed())
            * (0.82 if self.player_status("chilled") > 0 else 1.0)
            # Big Hit windup: lumber at half speed while charging. Mirrored in
            # the host partner sim and joiner prediction (net/mixin.py).
        ...
        # Big Hit cooldown + charge tick (fires the blow on charge completion).
        ...
        stamina_regen, mana_regen = player_recovery_rates(self.player)
