...
class _FamiliarsCombatMixin:
    ...
    # 4.8.7 immortal Bar Dancer: the final-depth reward for toasting every bar
    # the run generated. Class-independent, so her numbers live here rather
    # than scaling with any discipline path; ``unkillable`` (the Eternal-Crows
    # contract: HP floors at 1 and regenerates) is what makes her immortal.
    BAR_DANCER_FAMILIAR_HP = 46
    BAR_DANCER_FAMILIAR_DAMAGE = 7
    BAR_DANCER_FAMILIAR_SPEED = 3.5
    ...
    def familiar_variant_for_index(self, index: int) -> int:
        ...
        crow once the Acolyte has learned Spirit Call (``acolyte_spirit_call``).
        Deeper Spirit nodes (Crow Companion / Twin Crows / Crow Lord / Eternal
        Crows) scale stats and count but no longer change the silhouette —
        the big familiar is always the crow.
        """
        if self.player.has_upgrade("acolyte_spirit_call"):
            return 1
        return 0
    ...
    def familiar_damage_type(self, familiar: Familiar | None = None) -> str:
        if familiar is not None and familiar.kind == "spirit_beast":
            if self.player.has_upgrade("ranger_spirit_companion"):
                return "arcane"
            return "physical"
        if familiar is not None and familiar.kind == "bar_dancer":
    ...
    def maybe_summon_bar_dancer_companion(self) -> None:
        """Summon the immortal Bar Dancer on the final depth if she was earned.

        The pact is strict: at least one bar must have rolled during the run
        and the tapped barrel of every one of them been drunk from
        (``bars_toasted`` covers ``bars_visited``). She joins as a familiar on
        the shared follow/attack AI, owned by the local player, and persists
        through save/load via the normal familiar serialization. Idempotent:
        an already-present dancer is never duplicated.
        """
        stats = self.run_stats
        if stats.bars_visited <= 0 or stats.bars_toasted < stats.bars_visited:
            return
        if any(f.kind == "bar_dancer" and f.alive for f in self.familiars):
            return
        spawn = self._spirit_beast_spawn_position()
        fx, fy = spawn if spawn is not None else (self.player.x, self.player.y)
        self.familiars.append(
            Familiar(
                x=fx,
                y=fy,
                max_hp=self.BAR_DANCER_FAMILIAR_HP,
                hp=self.BAR_DANCER_FAMILIAR_HP,
                damage=self.BAR_DANCER_FAMILIAR_DAMAGE,
                speed=self.BAR_DANCER_FAMILIAR_SPEED,
        ...
        accent = (224, 126, 72)
        self.add_impact(
            fx, fy, accent, ttl=0.48, radius=0.72, kind="burst"
        )
        self.floaters.append(
            FloatingText(
                "The Bar Dancer joins you — every bar toasted!",
                self.player.x,
                self.player.y - 0.6,
                accent,
                ttl=2.2,
    ...
    def _familiar_attack(self, familiar: Familiar, enemy: Enemy) -> None:
        ...
        # value, since it draws the first blows). Eternal Crows makes the
    ...
    def _familiar_take_damage(
        ...
        if familiar.unkillable:
            # Eternal Crows: the host cannot die; damage floors at 1 and
    ...
    def _familiar_regen(self, familiar: Familiar, dt: float) -> None:
        # Eternal Crows familiars slowly regenerate, and unkillable ones
