...
class _DisciplinesCombatMixin:
    def skill_names(self) -> tuple[str, str, str, str]:
        # Slot 1 is the archetype's Big Hit since 4.10 (auto-melee is
        # nameless); the old melee names (Shield Bash, Backstab, ...) retired
        # from the HUD with it.
        names = {
        ...
        return names.get(self.player.class_name, ("Smash", "Bolt", "Nova", "Dash"))
    ...
    def choose_discipline(self, key: str, reason: str = "chosen") -> bool:
        """Apply a specific discipline by key, spending one memory token.

        Returns False (without spending a token) if the discipline is unknown, belongs
        to another archetype, is already acquired, has unmet prerequisites, is
        in a path locked by the commitment limit, or the player has no memory
        tokens to spend.
        """
        ...
        if is_path_locked(
            ...
            return False
        if self.player.memory_tokens <= 0:
            return False
        self.player.memory_tokens -= 1
        self._apply_discipline(node, reason)
        self._apply_combo_bonus_delta(node)
        return True

    def grant_memory_token(self, amount: int = 1, reason: str = "reward") -> None:
        """Award memory tokens from run rewards (shrines, altars, story)."""
        if amount <= 0:
            return
        self.player.memory_tokens += amount
        self.floaters.append(
            FloatingText(
                f"+{amount} Memory Token{'s' if amount != 1 else ''}",
                self.player.x,
                self.player.y - 0.6,
                self.skill_color(),
                ttl=1.6,
    ...
    def grant_discipline(self, reason: str = "level up") -> bool:
        ...
        NOT spend the player's banked memory tokens (level-up tokens are spent
