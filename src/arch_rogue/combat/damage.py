...
from ._utils import KNOCKBACK_DECAY_RATE, KNOCKBACK_SPEED
from ..content import BOSS_LAST_WORDS
from ..models import (
    Enemy,
    FloatingText,
)
...
class DamageContext:
    ...
    source: str = ""
    is_crit: bool = False
    # Per-hit knockback speed override (tiles/sec). ``None`` keeps the global
    # KNOCKBACK_SPEED nudge; Big Hit's hurl passes tiles * KNOCKBACK_DECAY_RATE
    # so the decay integral lands the target the designed distance away.
    knockback_speed: float | None = None
...
class _DamageCombatMixin:
    ...
    def damage_enemy(self, ctx: DamageContext) -> None:
        ...
        # 4.8.9 damage aggro: getting hurt always engages (and pack-alerts),
        # so sniping from outside the aggro radius is no longer free. The
        # acting player is the attacker in co-op remote contexts too.
        self.alert_enemy(enemy, self.player.x, self.player.y)
        amount = ctx.amount
        ...
        if length > 0.001:
            ...
            # stunned enemies). Total displacement ~= speed / decay rate.
            speed = (
                KNOCKBACK_SPEED
                if ctx.knockback_speed is None
                else ctx.knockback_speed
            )
            enemy.knockback_vx = (kx / length) * speed
            enemy.knockback_vy = (ky / length) * speed
        if enemy.hp <= 0:
            if ctx.knockback_speed is not None and length > 0.001:
                # A hurled kill removes the body this same frame, so the
                # 2-tile flight would be invisible. Paint the launch itself:
                # a short trail of bursts marching down the throw path.
                nx, ny = kx / length, ky / length
                tiles = ctx.knockback_speed / KNOCKBACK_DECAY_RATE
                for step in (0.35, 0.7):
                    self.add_impact(
                        enemy.x + nx * tiles * step,
                        enemy.y + ny * tiles * step,
                        hit_color,
                        ttl=0.26 + 0.16 * step,
                        radius=0.38,
                        kind="burst",
    ...
    def _float_boss_last_words(self, enemy: Enemy) -> None:
        """4.9: every Stalled dies mid-office; each gets one last word.

        A single floater — "…finished." and its variants — is the cheapest
        possible payoff of the boss-as-unfinished-official framing.
        """
        last_words = BOSS_LAST_WORDS.get(enemy.name, "")
        if not last_words:
            # Elite prefixes and themed tyrant titles decorate the base name;
            # match on containment, with the final boss as the boss-kind
            # fallback (every themed Gate Tyrant is still the Toll-Keeper).
            last_words = next(
                (
                    words
                    for base_name, words in BOSS_LAST_WORDS.items()
                    if base_name in enemy.name
                ),
                BOSS_LAST_WORDS["Dread Gate Tyrant"]
                if enemy.kind == "boss"
                else "",
            )
        if not last_words:
    ...
    def kill_enemy(self, enemy: Enemy) -> None:
        ...
        # 4.8.10: an NPC ally chasing this enemy may have built a nav field
        # toward it; forget it with the corpse.
        self._drop_nav_field_for(enemy)
        ...
        if enemy.kind == "boss":
            ...
            self._float_boss_last_words(enemy)
        ...
        if enemy.kind in ("boss", "miniboss") or enemy.elite_modifier:
            # Boss and kill-count achievements pop at the kill, not at the
            # eventual run end. Ordinary kills skip this: the method saves to
            # disk when progress changed, and rank-and-file deaths never do.
        ...
        if self.player.gain_xp(xp_gain):
            # Milestone 3.3: level-ups award a memory token (handled inside
        ...
        # The previous 0.45 base made most kills spit out an item; 4.2 trimmed
        # it to 0.36. 4.9.23 runs a second across-the-board rarity pass and
        # trims the on-kill roll to 0.28. Elites and bosses still grant their
        # existing gold bonuses.
        ...
        # Boss outcomes are critical progression boundaries and remain
        # immediately durable. Routine kills coalesce behind a short checkpoint
        # window so a combat burst cannot block one frame per corpse on fsync.
        if enemy.kind in ("boss", "miniboss"):
            self.save_run()
        else:
            self.request_run_checkpoint()
