...
class PixelSpriteAtlas:
    ...
    def __init__(self) -> None:
        ...
        raw_items = {
            ...
            "remove_curse": self._remove_curse_scroll(),
            "weapon": self._weapon(),
            "armor": self._armor(),
        ...
        self.lossless_soul_sprite = self._scale_actor(self._lossless_soul())
        lossless_soul_accent = (142, 214, 205)
        self.lossless_soul_animation_frames = self._actor_animation_frames(
            self.lossless_soul_sprite, lossless_soul_accent
        )
        self.lossless_soul_animation_frames["dance"] = [
            self._actor_pose_frame(
                self.lossless_soul_sprite, lossless_soul_accent, "dance", frame
            )
            for frame in range(8)
        ]
    ...
    def lossless_soul_frame(
        ...
        *,
        ...
        state = "walk" if moving else "dance" if dancing else "idle"
        frames = self.lossless_soul_animation_frames[state]
        if clip_progress is not None:
            return self._frame_from(frames, (clip_progress % 1.0) * len(frames))
        rate = 5.6 if moving else 3.6 if dancing else 3.0
        return self._frame_from(frames, elapsed, rate=rate)
    ...
    def familiar_frame(
        ...
        *,
        ...
        ``variant`` selects small wisp (0), big crow (1), or Spirit Beast (2).
    ...
    def _remove_curse_scroll(self) -> pygame.Surface:
        s = self._surface(18, 16)
        paper = (224, 207, 166)
        paper_hi = (245, 235, 200)
        edge = (132, 91, 52)
        ...
        self._rect(s, 3, 4, 12, 8, paper)
        self._hline(s, 3, 4, 12, paper_hi)
        # broken-chain sigil: two open gold links with a gap between them
        self._rect(s, 5, 6, 3, 3, sigil)
        self._dot(s, 6, 7, paper)
        self._rect(s, 10, 8, 3, 3, sigil)
        self._dot(s, 11, 9, paper)
        self._dot(s, 5, 6, sigil_hi)
        self._dot(s, 10, 8, sigil_hi)
        # curse mist wisping off the tear
        self._dot(s, 9, 5, mist)
        self._dot(s, 13, 6, mist)
        return s
    ...
    def _lossless_soul(self) -> pygame.Surface:
        s = self._surface(self.RAW_ACTOR_W, self.RAW_ACTOR_H)
        outline = (18, 18, 26)
        skin = (226, 218, 222)
        skin_hi = (244, 240, 242)
        ...
        # Deep hood framing a pale face, long silver hair spilling forward.
        self._rect(s, 7, 1, 12, 10, outline)
        self._rect(s, 8, 2, 10, 8, robe)
        self._hline(s, 8, 2, 10, robe_hi)
        self._rect(s, 9, 4, 8, 5, skin)
        self._hline(s, 9, 4, 8, skin_hi)
        self._dot(s, 11, 6, (52, 60, 72))
        self._dot(s, 14, 6, (52, 60, 72))
        self._vline(s, 8, 5, 7, silver)
        self._vline(s, 17, 5, 7, silver)
        self._vline(s, 7, 8, 6, silver_hi)
        self._vline(s, 18, 8, 6, silver_hi)

        # Layered night-blue robe with a tarnished-silver clasp line.
        self._rect(s, 5, 11, 16, 14, outline)
        self._rect(s, 6, 12, 14, 12, robe)
        self._vline(s, 7, 12, 12, robe_hi)
        self._vline(s, 18, 12, 12, robe_lo)
        self._vline(s, 13, 12, 12, silver)
        for y in (13, 16, 19, 22):
            self._dot(s, 13, y, teal)
        self._rect(s, 4, 13, 3, 7, robe)
        self._rect(s, 19, 13, 3, 7, robe)
        self._rect(s, 4, 19, 3, 2, skin)
        self._rect(s, 19, 19, 3, 2, skin)

        # Trailing hem dissolving into soul-flame wisps instead of feet.
        self._rect(s, 5, 25, 16, 5, outline)
        self._rect(s, 6, 25, 14, 4, robe_lo)
        self._hline(s, 6, 25, 14, robe)
        for x, y in ((6, 30), (9, 31), (13, 30), (16, 31), (19, 30)):
            self._dot(s, x, y, teal)
        for x in (7, 11, 15, 18):
            self._dot(s, x, 29, teal_hi)
        self._dot(s, 5, 28, teal)
        self._dot(s, 20, 28, teal)
        return s
    ...
    # post-Spirit-Call crow. The Ranger Spirit Beast fallback follows below.
    # ------------------------------------------------------------------
    ...
    def _familiar_crow(self) -> pygame.Surface:
        # Big crow spirit (the post-Spirit-Call familiar state). A round
        ...
        # big eye discs (lighter than the body so they read as crow eyes)
