...
class MenuStateOverlayMixin:
    ...
    def _draw_state_overlay_content(self) -> None:
        ...
        # 4.9: Nim Rue closes the entry — the ending stamp on victory, her
        # road-shaded margin note on death. One line, ellipsized like the
        # subtitle; absent when there is no story state to speak for.
        story_line = ""
        if victory and hasattr(self.g, "story_victory_line"):
            story_line = self.g.story_victory_line()
        elif not victory and hasattr(self.g, "story_death_line"):
            story_line = self.g.story_death_line()
        story_h = 0
        if story_line:
            story_h = self.g.small_font.get_height() + self.u(4)
            self.draw_text(
                f"“{story_line}” — N. Rue",
                self.g.small_font,
                self.shade(color, 46),
                pygame.Rect(inner.x, sub_y + sub_h, inner.width, story_h),
                align="center",
            )

        # Run-stats table inside a recessed sub-panel.
        table_top = sub_y + sub_h + story_h + self.u(14)
    ...
    def _run_stats_sections(
        ...
        section_indices = (
            ("Run", (0, 1, 2, 3, 4, 10)),
            ("Combat", (5, 6, 7, 8, 9, 12)),
            ("Exploration", (11, 13, 14, 15, 16, 17, 18)),
            ("Legacy", (19, 20, 21, 22, 23, 24)),
        )
        return [
    ...
    def _run_stats_rows(self, victory: bool) -> list[tuple[str, str]]:
        ...
        return [
            ...
            (
                "Bars toasted",
                f"{g.run_stats.bars_toasted} / {g.run_stats.bars_visited}",
            ),
