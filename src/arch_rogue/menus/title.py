...
class AboutDocument(NamedTuple):
    """Cached, width-specific layout for the long About/license document."""

    entries: tuple[AboutEntry, ...]
    prefix_heights: tuple[int, ...]
    scroll_max: int
...
class MenuTitleMixin:
    def draw_title_menu(self) -> None:
        ...
        rendered_rows = self.draw_menu_rows(
            ...
            row_assets=(None, "menu.row.two_descend", None, None, None, None),
        )
    ...
    def draw_about_screen(self) -> None:
        ...
        ai_notice = ai_provenance_text()
        apache_license = license_text()
        third_party_notice = notice_text()
        pygame_license = pygame_lgpl_text()
        intro = (
        ...
        cache_key = (
            ...
            id(pre_font),
            tuple(self.accent()),
            int(getattr(self.g, "ui_scale", 1)),
            hash(ai_notice),
            hash(apache_license),
            hash(third_party_notice),
            hash(pygame_license),
        )
        cached = getattr(self, "_about_document_cache", None)
        if cached is not None and cached[0] == cache_key:
            ...
            def add_paragraph(text: str) -> None:
                for i, line in enumerate(self.wrap_text(text, body_font, wrap_width)):
                    entries.append(
                        AboutEntry(
                            line,
                            body_font,
                            self.TEXT,
                            (paragraph_gap if i == 0 else 0) + line_gap,
                            (paragraph_gap if i == 0 else 0) + body_vcenter,
                            0,
                            -1,
                        )
                    )
            ...
            def add_labeled(label: str, body: str) -> None:
                entries.append(
                    AboutEntry(
                        ...
                        self.accent(),
                        item_gap + line_gap,
                        item_gap + body_vcenter,
                        0,
                        -1,
                    )
                )
                for line in self.wrap_text(
                    body, body_font, wrap_width - label_indent
                ):
                    entries.append(
                        AboutEntry(
                            line,
            ...
            def add_preformatted(text: str) -> None:
                ...
                for src_line in text.split("\n"):
                    for line in self.wrap_text(src_line, pre_font, wrap_width):
                        leading = paragraph_gap if first else 0
                        entries.append(
                            AboutEntry(
                                line,
                                pre_font,
                                self.MUTED,
                                leading + pre_line_gap,
                                leading + pre_vcenter,
                                0,
                                -1,
                            )
            ...
            add_paragraph(ai_notice)
            add_section("Third-Party Notices")
            add_preformatted(third_party_notice)
            add_section("pygame-ce — GNU LGPL 2.1-or-later")
            add_preformatted(pygame_license)
            add_section("Arch Rogue — Apache License 2.0")
            add_preformatted(apache_license)

            prefix = [0]
            for entry in entries:
                prefix.append(prefix[-1] + entry.height)
            scroll_max = self._about_scroll_max(
                tuple(prefix), len(entries), content.height
            )
            document = AboutDocument(
                tuple(entries), tuple(prefix), scroll_max
            )
            self._about_document_cache = (cache_key, document)

        entries = document.entries
        prefix = document.prefix_heights
        scroll_max = document.scroll_max
        n = len(entries)

        def visible_end(start: int) -> int:
            return self._about_visible_end(prefix, start, n, content.height)
    ...
    @staticmethod
    def _about_visible_end(
        prefix: tuple[int, ...],
        start: int,
        count: int,
        viewport_height: int,
    ) -> int:
        """Return the last entry fitting the variable-height About viewport."""

        if count <= 0:
            return 0
        lo, hi = min(start + 1, count), count
        while lo < hi:
            mid = (lo + hi + 1) // 2
            if prefix[mid] - prefix[start] <= viewport_height:
                lo = mid
            else:
                hi = mid - 1
        return lo
    ...
    @classmethod
    def _about_scroll_max(
        cls,
        prefix: tuple[int, ...],
        count: int,
        viewport_height: int,
    ) -> int:
        """Find the first entry index whose viewport reaches document end."""

        if count <= 0 or prefix[count] <= viewport_height:
            return 0
        lo, hi = 0, count - 1
        while lo < hi:
            mid = (lo + hi) // 2
            if cls._about_visible_end(prefix, mid, count, viewport_height) >= count:
                hi = mid
            else:
                lo = mid + 1
        return lo
    ...
    def _draw_help_overlay_fitted(self) -> None:
        ...
        lines = [
            ...
            "Minimap: Ctrl+M toggles the top-right map. The bar, garden, and stairs appear once discovered; the relic's guiding light traces its path.",
            "Graphics: choose Legacy, Modern, or HD in Options; Ctrl+Alt+L toggles Legacy and your last authored tier.",
        ]
