...
class MenuBaseMixin:
    ...
    _MENU_TEXT_CACHE_MAX_ENTRIES = 1024
    _MENU_TEXT_CACHE_MAX_BYTES = 8 * 1024 * 1024
    ...
    def __init__(
        ...
        self._menu_text_cache_bytes = 0
        self._menu_text_size_cache: OrderedDict[
            tuple[int, str], tuple[int, int]
        ] = OrderedDict()
        self._menu_ellipsize_cache: OrderedDict[
            tuple[int, str, int], str
        ] = OrderedDict()
        self._menu_wrap_cache: OrderedDict[
            tuple[int, str, int], tuple[str, ...]
        ] = OrderedDict()

    @staticmethod
    def _remember_lru(cache: OrderedDict, key: object, value: object, limit: int):
        """Store one derived menu value without allowing UI caches to grow forever."""
    ...
    def clear_render_caches(self) -> None:
        """Drop display/font-dependent menu caches through the game's cache seam."""

        self._menu_font_cache.clear()
        self._menu_backdrop_cache = None
        self._menu_text_cache.clear()
        self._menu_text_cache_bytes = 0
        self._menu_text_size_cache.clear()
        self._menu_ellipsize_cache.clear()
        self._menu_wrap_cache.clear()
        self._about_document_cache = None
        self._discipline_tree_cache = {}
        self._discipline_grid_layout_cache = {}
        self._discipline_name_fit_cache = {}
        self._discipline_description_font_choice = None
        self._discipline_panel_bounds_choice = None

    def _text_size(self, font: pygame.font.Font, text: str) -> tuple[int, int]:
        key = (id(font), text)
        cached = self._menu_text_size_cache.get(key)
        if cached is not None:
            self._menu_text_size_cache.move_to_end(key)
            return cached
        size = font.size(text)
        return self._remember_lru(
            self._menu_text_size_cache, key, size, 4096
        )

    def _remember_text_surface(
        self,
        key: tuple[int, str, Color, int],
        surface: pygame.Surface,
    ) -> pygame.Surface:
        """Retain rendered text within a conservative, byte-aware memory cap."""

        size_bytes = (
            surface.get_width() * surface.get_height() * surface.get_bytesize()
        )
        if size_bytes > self._MENU_TEXT_CACHE_MAX_BYTES:
            return surface
        self._menu_text_cache[key] = surface
        self._menu_text_cache.move_to_end(key)
        self._menu_text_cache_bytes += size_bytes
        while (
            len(self._menu_text_cache) > self._MENU_TEXT_CACHE_MAX_ENTRIES
            or self._menu_text_cache_bytes > self._MENU_TEXT_CACHE_MAX_BYTES
        ):
            _, evicted = self._menu_text_cache.popitem(last=False)
            self._menu_text_cache_bytes -= (
                evicted.get_width()
                * evicted.get_height()
                * evicted.get_bytesize()
            )
        return surface
    ...
    def _animated_title_logo(
        self, size: tuple[int, int]
    ) -> pygame.Surface | None:
        """The title logo with its relic diamond spinning (5.0, desktop only).

        Every (frame, size) variant is served from a small LRU on the Game so
        a retained-frame repaint never recomposes or rescales — the warm-cache
        contract (test_ui_assets) requires the second draw of a menu to add
        zero asset builds/decodes.
        """

        frame_index = self.g.spinning_logo_frame_index()
        cache = getattr(self.g, "_animated_logo_cache", None)
        if cache is None:
            cache = OrderedDict()
            self.g._animated_logo_cache = cache
        key = (frame_index, size)
        surface = cache.get(key)
        if surface is not None:
            cache.move_to_end(key)
            return surface
        composed = self.g.compose_spinning_logo(frame_index)
        if composed is None:
            return None
        surface = pygame.transform.scale(composed, size)
        surface = optimize_immutable_alpha_surface(surface)
        cache[key] = surface
        # 16 spin frames x the (rare) size changes; keep a lap and a half.
        while len(cache) > 24:
            cache.popitem(last=False)
        return surface
    ...
    def ellipsize(self, text: str, font: pygame.font.Font, max_width: int) -> str:
        ...
        cached = self._menu_ellipsize_cache.get(key)
        if cached is not None:
            self._menu_ellipsize_cache.move_to_end(key)
            return cached
        if self._text_size(font, text)[0] <= max_width:
            result = text
            return self._remember_lru(
                self._menu_ellipsize_cache, key, result, 2048
            )
        suffix = "…"
        low = 0
        high = len(text)
        while low < high:
            length = (low + high + 1) // 2
            if self._text_size(font, text[:length] + suffix)[0] <= max_width:
                low = length
            else:
                high = length - 1
        result = text[:low] + suffix if low else suffix
        return self._remember_lru(
            self._menu_ellipsize_cache, key, result, 2048
        )
    ...
    def wrap_text(self, text: str, font: pygame.font.Font, max_width: int) -> list[str]:
        max_width = max(1, max_width)
        key = (id(font), text, max_width)
        cached = self._menu_wrap_cache.get(key)
        if cached is not None:
            self._menu_wrap_cache.move_to_end(key)
            return list(cached)
        if not text:
            return [""]
        lines: list[str] = []
        ...
        self._remember_lru(self._menu_wrap_cache, key, result, 4096)
        return list(result)
    ...
    def draw_text(
        ...
        surface = self._menu_text_cache.get(key)
        if surface is None:
            surface = font.render(
                self.ellipsize(text, font, rect.width), True, color
            )
            self._remember_text_surface(key, surface)
        else:
            self._menu_text_cache.move_to_end(key)
        if align == "center":
        ...
        *,
        ...
        def fits(candidate: pygame.font.Font) -> bool:
            return candidate.get_height() <= max_height and all(
                self._text_size(candidate, text)[0] <= max_width
                for text in samples
            )
        ...
        *,
        ...
        if title_asset and modern_header:
            ...
            if (
                ...
                # 5.0: the title menu spins the logo's relic diamond (the
                # loading screen and website hero already do). Mobile keeps
                # the static logo — per-frame menu repaints would defeat the
                # retained-frame battery saving there.
                if title_asset == "menu.logo.title" and not getattr(
                    self.g, "mobile_mode", False
                ):
                    title_surface = self._animated_title_logo(logo_size)
        ...
        *,
        ...
        for index, (key, label, value) in enumerate(rows):
            ...
            if row_asset is None and row_asset_key != "menu.row":
                row_asset_key = "menu.row"
                row_asset = self.ui_asset("menu.row", row_rect.size)
            row_safe = (
                self.ui_content_rect(row_asset_key, row_rect)
