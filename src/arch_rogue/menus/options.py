...
class MenuOptionsMixin:
    def draw_options_menu(self) -> None:
        ...
        rows: list[MenuRow] = []
        if not self.g._options_skip_fullscreen():
            display_row: MenuRow = (
        ...
        if not self.g._options_skip_render_resolution():
            rows.append(
                (
                    "R",
                    "Render resolution",
                    self.g.desktop_render_resolution_label(),
                )
            )
        rows.extend([
        ...
        if not self.g._options_skip_fullscreen():
            rows.append(("0 / + / -", "UI scale", self.g.ui_scale_label()))
        rows.extend([
            (
                "G / Left / Right",
                "Graphics",
                self.g.graphics_tier_label(),
            ),
            ...
            # 4.11.0: the music row is parked until the real tracks land; the
            # option (and its M shortcut) returns with them.
        ...
        # Visual grouping: (row_index, section_title). Section start indices
        # use the OPTIONS_ROW_* properties so they auto-shift when the
        # fullscreen row is hidden on Steam Deck. An optional Diagnostics row
        # (desktop only) and the Back row are intentionally ungrouped at the
        # bottom.
        sections = [
            (self.g.OPTIONS_ROW_DIFFICULTY, "Display"),
            (self.g.OPTIONS_ROW_CONTROLS, "Controls"),
            (self.g.OPTIONS_ROW_AUDIO, "Audio"),
            (self.g.OPTIONS_ROW_LIGHTING, "Lights"),
            (self.g.OPTIONS_ROW_MP_HOST, "Multiplayer"),
        ]
        if not self.g.mobile_mode:
        ...
        if note_h:
            ...
            self.draw_wrapped_text(
                ...
                "Hell appears after your first clear. Options persist in the "
                "Arch Rogue folder of your user data directory.",
                detail_font,
                self.MUTED,
                note_rect,
            )
