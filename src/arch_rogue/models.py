...
class Discipline:
    ...
    Milestone 3.3 — memory tokens and combo trees:
...
class RunStats:
    ...
    # 4.8.7 bar pilgrimage: bars generated on this run's floors vs. bars whose
    # tapped barrel the party drank from. Toasting every bar that rolled earns
    # the immortal Bar Dancer companion on the final depth.
    bars_visited: int = 0
    bars_toasted: int = 0
    cause_of_death: str = ""
...
@dataclass(frozen=True)
class SpecialRoomPlacement:
    """Declarative spawn policy for one special-room kind.

    ``chance`` is the probability the room appears on an eligible floor.
    ``rng_stream`` selects which RNG drives the roll and the room pick:

    * ``"shared"`` — the dungeon's main generation stream. Consuming it is part
      of the historical layout contract, so shared-stream kinds must keep their
      position in the definition order.
    * ``"guest"`` — the layout-seeded story-guest stream (one draw per floor).
    * ``"flavor"`` — the layout-seeded flavor stream consumed, in definition
      order, by every flavor room. Appending new kinds preserves the rolls of
      earlier ones, so existing floors keep their bar/garden layouts.

    ``requires_flag`` names a ``Dungeon`` attribute that must be truthy for the
    room to be considered at all (e.g. ``"guest_room"`` for story-beat floors).
    """

    chance: float = 1.0
    rng_stream: str = "flavor"
    requires_flag: str = ""
...
class SpecialRoomDefinition:
    ...
    min_depth: int = 1
    max_depth: int = 0
    placement: SpecialRoomPlacement = SpecialRoomPlacement()
...
class Item:
    ...
    def label(self) -> str:
        ...
        if self.slot in ("identify", "remove_curse"):
...
class Trap:
    ...
    damage: int
    active: bool = True
    # Traps spawn invisible. ``revealed`` flips (permanently for the floor)
    # when any living player closes within TRAP_REVEAL_RADIUS, and
    # ``reveal_progress`` ramps 0→1 to drive the renderer's materialize fade.
    # Defaults keep older saves loading as hidden, un-walked traps.
    revealed: bool = False
...
class StoryBeat:
    ...
    resolved_choice: str = ""
    outcome: str = ""
    # 4.9.x: the dilemma's plain-spoken reveal sentence (default keeps older
...
class StoryGuest:
    ...
    resolved_choice: str = ""
    met: bool = False
    # 4.8.10 "NPCs join the fight": once the guest's dialogue resolves it takes
    # up arms beside the descenders. ``aggressive`` is DERIVED from ``resolved``
    # (at resolve time and again on load) rather than being new story state;
    # ``max_hp == 0`` means combat stats are not rolled yet (rolled lazily from
    # the per-kind ally table). All fields appended with defaults so positional
    # construction and pre-4.8.10 saves are untouched.
    max_hp: int = 0
    hp: int = 0
    damage: int = 0
    attack_range: float = 1.1
    attack_cooldown: float = 1.0
    combat_speed: float = 1.9
    aggressive: bool = False
    greeter_id: str = ""
    home_x: float = 0.0
    home_y: float = 0.0
    # Transient combat state — never serialized (see _TRANSIENT_NPC_FIELDS).
    attack_timer: float = field(default=0.0, repr=False, compare=False)
    nav_latch: float = field(default=0.0, repr=False, compare=False)
    combat_active: bool = field(default=False, repr=False, compare=False)

    @property
    def alive(self) -> bool:
        # An NPC whose stats were never rolled (``max_hp == 0``) is alive.
        return self.max_hp <= 0 or self.hp > 0


@dataclass
class IdleNpc:
    # Ambient actor placed in flavor special rooms. ``kind`` selects the flavor
    # or a dedicated visual subtype such as ``garden_frog``. Most kinds are
    # decorative; the ``lossless_soul`` keeper offers a dialogue cutscene and
    # either resident Garden frog can open its room's once-only mini-game.
    # Per-room outcomes stay on ``SpecialRoom.state`` rather than on the actor.
    x: float
    y: float
    kind: str = ""
    name: str = ""
    ...
    # 4.8.10 "NPCs join the fight": greeted bar/garden/dancer NPCs (and the
    # keeper once her reflection resolves) become killable combat allies.
    # ``max_hp == 0`` means "stats not rolled yet" — rolled lazily on first
    # greeting from the per-kind ally table scaled by depth and difficulty.
    # ``greeter_id`` routes kill XP/gold to the greeting player, and
    # ``home_x``/``home_y`` anchor the 9-tile combat leash. All appended with
    # defaults, so pre-4.8.10 saves load as pacifist props (no schema bump).
    max_hp: int = 0
    hp: int = 0
    damage: int = 0
    attack_range: float = 1.1
...
@dataclass
class StormChainCharge:
    """Shared one-shot Storm payload for every projectile in one Arc Bolt cast."""
...
class Projectile:
    ...
    #   * `homing` in 0..1 steers the projectile toward the nearest enemy in
    #     direct projectile line of sight each frame (0 disables homing).
    #     Applied by the combat loop, not here, so the model stays free of
    #     enemy-list references.
    pierce: int = 0
    homing: float = 0.0
    ...
    # Every projectile from one Arc Bolt cast shares this one-shot payload.
    # Whichever projectile hits first spends it, so a missed fan shard cannot
    # discard Storm while another shard lands. This reference is transient:
    # player projectiles are neither run-saved nor authoritative on guests.
    storm_chain_charge: StormChainCharge | None = field(
        default=None,
        repr=False,
        compare=False,
    )
...
class Enemy:
    ...
    # 4.8.9 machine intelligence: persisted AI identity. ``tactic`` overrides
    # the role-derived movement doctrine when set; ``ability_keys`` lists
    # authored EnemyAbility keys (empty = synthesized default from ``kind``,
    # matching pre-4.8.9 behavior). Both are identity, not state, so they
    # ride saves and multiplayer spawn payloads like ``role``/``damage_type``.
    tactic: str = ""
    ...
    # ``alert_timer`` runs down after the target leaves aggro range); the
    # strafe side flankers/skirmishers drift toward; and the last authored
    # ability fired (drives the boss rotation's "not the same one twice").
    ability_timers: dict[str, float] = field(
        default_factory=dict, repr=False, compare=False
    )
    ai_state: str = field(default="", repr=False, compare=False)
    alert_timer: float = field(default=0.0, repr=False, compare=False)
    # Wall-stall latch: seconds remaining during which movement prefers
    # distance-field descent because a planned advance step barely moved.
    nav_latch: float = field(default=0.0, repr=False, compare=False)
    # Chokepoint yield: seconds remaining during which this enemy backs off
    # instead of advancing, so a packmate closer to the target can take a
    # contested doorway (transient, host-session-only like nav_latch).
    yield_timer: float = field(default=0.0, repr=False, compare=False)
    # Stuck probe: reference position + window for the net-displacement
    # check that catches oscillating wedges per-frame moves can't see.
    stuck_probe_x: float = field(default=0.0, repr=False, compare=False)
    stuck_probe_y: float = field(default=0.0, repr=False, compare=False)
    stuck_probe_timer: float = field(default=0.0, repr=False, compare=False)
    memory_x: float = field(default=0.0, repr=False, compare=False)
    memory_y: float = field(default=0.0, repr=False, compare=False)
    strafe_sign: float = field(default=0.0, repr=False, compare=False)
    last_ability: str = field(default="", repr=False, compare=False)
...
class Player:
    ...
    stamina: float = 100
    speed: float = 4.6
    # 5.0: Haste Shrine's real, capped move-speed blessing. ``speed`` above is
    # the legacy display rating kept for save/wire compatibility; locomotion
    # reads combat.costs.player_move_speed() instead.
    shrine_move_bonus: float = 0.0
    melee_bonus: int = 0
    spell_bonus: int = 0
    armor_bonus: int = 0
    level: int = 1
    ...
    # 4.10 Big Hit (action slot 1) cooldown. The charge itself lives in
    # ``status_effects["_charging"]`` so it replicates/saves for free.
    bighit_timer: float = 0.0
    ...
    def melee_damage(self) -> int:
        ...
        # 4.10: base trimmed 12 -> 10 alongside Big Hit's arrival — auto-melee
        # is now the trash-clearing baseline under the slot-1 heavy blow.
        return (
            10
            + self.level * 2
            + self.melee_bonus
    ...
    def spell_damage(self) -> int:
        """Base bolt damage: the spell-power analog of :meth:`melee_damage`.

        This is the number ``player_cast_bolt`` starts from (equipment skill
        bonuses and class riders stack on top there); the character sheet
        shows it so spell_bonus picks — Arcanist paths, Warden Guard Bolt,
        the 4.11.0 Rogue Marksman rebalance — read as a visible stat.
        """

        return 14 + self.level * 2 + self.spell_bonus
    ...
    def gain_xp(self, amount: int) -> bool:
        ...
        # Milestone 3.3: level-ups award a memory token the player spends in the
