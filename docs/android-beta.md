...
## Graphics tier and render quality

Android exposes two independent visual settings:

- **Graphics** selects the art and world-rendering path. **Legacy** uses the
  procedural renderer, **Modern** keeps the original lower-resolution authored
  world sprites, and **HD** uses the high-resolution world masters and
  native-detail zoom pipeline. Modern and HD share the authored actors, props,
  menus, and HUD.
- **Render quality** selects the logical framebuffer resolution sent to the
  display: Performance, Balanced, or Native.

HD is the default graphics tier and the target for future visual development.
Modern and Legacy remain supported compatibility paths for older or constrained
devices. Graphics tier does not force a render-quality mode: HD can run with a
540p or 720p cap, and Modern or Legacy can run at Native resolution. Existing
option files preserve the former procedural choice as Legacy and the former
authored-asset choice as Modern; fresh installations start in HD.
...
telemetry. The first Options row cycles to capped modes for slower devices:
...
- **Native · full resolution** — diagnostic/high-end mode; can be dramatically
  ...
  quarter-resolution lighting as the capped modes; the cheaper local-tint path
...
To compare mobile CPU render-quality modes without an Android device, use the
