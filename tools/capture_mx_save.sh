#!/usr/bin/env bash
# Build once, then record deterministic MX-save Chronicle/recovery captures.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
OUT="$ROOT/build/mx-save-captures"
...
if [[ "$display_ready" != true ]]; then
  if command -v xvfb-run >/dev/null 2>&1; then
    ...
    if [[ "$display_ready" != true ]]; then
      cat "$OUT-xvfb.log" >&2
      printf 'capture_mx_save.sh: build-local Xvfb did not become ready\n' >&2
      exit 2
    fi
  else
    printf 'capture_mx_save.sh: no reachable display; install xorg-server-xvfb or run from a graphical session\n' >&2
    exit 2
  fi
fi
...
if len(data) < 24 or data[:8] != b"\x89PNG\r\n\x1a\n" or data[12:16] != b"IHDR":
    raise SystemExit(f"capture_mx_save.sh: {path} is not a canonical PNG")
actual = struct.unpack(">II", data[16:24])
if actual != expected:
    raise SystemExit(
        f"capture_mx_save.sh: {path.name} is {actual[0]}x{actual[1]}, "
...
capture() {
  ...
  local width="$2"
  local height="$3"
  local relative="build/mx-save-captures/${scenario}_${width}x${height}.png"
  local log="$OUT/${scenario}_${width}x${height}.log"
  printf 'MX-save capture %-10s %sx%s\n' "$scenario" "$width" "$height"
  if ! (
    cd "$ROOT"
    env -u ARCH_ROGUE_LIGHTING -u ARCH_ROGUE_ZOOM -u ARCH_ROGUE_REVEAL -u ARCH_ROGUE_DEV \
      -u ARCH_ROGUE_PLAY -u ARCH_ROGUE_OPEN -u ARCH_ROGUE_BAG -u ARCH_ROGUE_SPAWN -u ARCH_ROGUE_DEPTH \
      -u ARCH_ROGUE_MX6_CAPTURE -u ARCH_ROGUE_MX7_CAPTURE -u ARCH_ROGUE_MX_STORY_CAPTURE -u ARCH_ROGUE_MX7_PERF \
      -u ARCH_ROGUE_CAPTURE_THEME -u ARCH_ROGUE_CAPTURE_DARK -u ARCH_ROGUE_CAPTURE_DOOR \
      XDG_CONFIG_HOME="$OUT/config" \
      XDG_DATA_HOME="$OUT/config" \
      ARCH_ROGUE_SEED="$SEED" \
      ARCH_ROGUE_MX_SAVE_CAPTURE="$scenario" \
  ...
  if [[ ! -s "$ROOT/$relative" ]]; then
    printf 'capture_mx_save.sh: missing or empty screenshot %s\n' "$ROOT/$relative" >&2
    exit 1
  fi
...
scenarios=(empty populated victories fallen recovery abandon)
resolutions=(
  "640 480"
  "1280 720"
  "1280 800"
...
for scenario in "${scenarios[@]}"; do
  for resolution_entry in "${resolutions[@]}"; do
    ...
    capture "$scenario" "$width" "$height"
  done
done

if command -v montage >/dev/null 2>&1; then
  montage "${captures[@]}" -thumbnail 640x400 -tile 4x -geometry +4+4 "$OUT/mx-save-montage.png"
  printf 'Montage: %s\n' "$OUT/mx-save-montage.png"
fi
