...
// The Odin Android subtarget intentionally reports ODIN_OS as Linux, so this
// branch must remain before the desktop Linux/X11 branch below.
when RAYLIB_ANDROID {
	#assert(ODIN_OS == .Linux, "RAYLIB_ANDROID requires Odin's Linux Android subtarget")
	#assert(!RAYLIB_SHARED, "The vendored Android raylib contract is static-only")
	when ODIN_ARCH == .arm64 {
		@(extra_linker_flags="-Wl,--wrap=fopen")
		foreign import lib {
			"android/arm64-v8a/libraylib.a",
			"system:android",
			"system:log",
			"system:EGL",
			"system:GLESv2",
		...
		@(extra_linker_flags="-Wl,--wrap=fopen")
		foreign import lib {
			"android/armeabi-v7a/libraylib.a",
			"system:android",
			"system:log",
			"system:EGL",
			"system:GLESv2",
		...
		#assert(false, "No vendored raylib Android archive exists for this architecture")
	}
...
// Arch Rogue's reviewed NativeActivity bridge is compiled into the vendored
// Android archive. It intentionally stays unavailable to every desktop build.
when RAYLIB_ANDROID {
	foreign lib {
		ArchRogueAndroidDrainLifecycleEvents :: proc() -> c.uint ---
		ArchRogueAndroidDrainBackPresses     :: proc() -> c.uint ---
		ArchRogueAndroidInternalDataPath     :: proc() -> cstring ---
		ArchRogueAndroidGetSafeInsets        :: proc(left, top, right, bottom: ^c.int) -> c.int ---
		ArchRogueAndroidIsInteractive        :: proc() -> c.int ---
		ArchRogueAndroidApiLevel             :: proc() -> c.int ---
		ArchRogueAndroidLog                  :: proc(message: cstring) ---
	}
}
